...
namespace Imedisoft.Data.CrudGenerator.Generator
{
    public static class EntityClassGenerator
    {
		...
		public static string Generate(Table table, string ns)
        {
			...
			if (table.Type.Namespace != ns)
            {
				AddNamespace(table.Type.Namespace);
            }
		...
		private static void GenerateSelectOne(StringBuilder stringBuilder, Table table)
        {
			...
			stringBuilder.AppendLine(@"		/// </summary>");
			stringBuilder.AppendLine(@"		/// <param name=""command"">The SELECT command to execute.</param>");
			stringBuilder.AppendLine(@"		/// <param name=""parameters"">The (optional) command parameters.</param>");
		...
		private static void GenerateSelectOnePK(StringBuilder stringBuilder, Table table)
        {
            ...
            stringBuilder.AppendLine(@"		/// </summary>");
			stringBuilder.AppendLine($@"		/// <param name=""{param}"">The primary key of the <see cref=""" + table.Type.Name + @"""/> to select.</param>");
			stringBuilder.AppendLine($"		public static {table.Type.Name} SelectOne({TypeName(table.PrimaryKey.Type.Name)} {param})");
			...
			else // If the table uses a non numeric primary key use a parameterized query...
            {
                ...
                stringBuilder.AppendLine($"			=> SelectOne({command},");
                stringBuilder.AppendLine($"				{GenerateParameterDirect(table.PrimaryKey, param)});");
            }
        ...
        {
			if (table.AllowedOperations.HasFlag(TableOperations.Create))
			{
				GenerateInsert(stringBuilder, table);
				//GenerateUpdateCompare(stringBuilder, table);
			}

			if (table.AllowedOperations.HasFlag(TableOperations.Update))
			{
				GenerateUpdate(stringBuilder, table);
				//GenerateUpdateCompare(stringBuilder, table);
			}

			if (table.AllowedOperations.HasFlag(TableOperations.Delete))
			{
		...
		private static void GenerateInsert(StringBuilder stringBuilder, Table table)
        {
			...
			foreach (var column in table.Columns)
			{
				if ((column.IsPrimaryKey && column.IsAutoIncrement) || column.AutoGenerated)
                {
					continue;
                }
			...
			stringBuilder.AppendLine(@"		/// </summary>");
			stringBuilder.AppendLine($@"		/// <param name=""{param}"">The <see cref=""" + table.Type.Name + @"""/> to insert into the database.</param>");

			if (table.PrimaryKey.IsNumeric && table.PrimaryKey.IsAutoIncrement)
            {
		...
		private static void GenerateUpdate(StringBuilder stringBuilder, Table table)
        {
			...
			foreach (var column in table.Columns)
			{
				if (column.IsPrimaryKey || column.ReadOnly || column.AutoGenerated)
				{
					continue;
				}
			...
			stringBuilder.AppendLine(@"		/// </summary>");
			stringBuilder.AppendLine($@"		/// <param name=""{param}"">The <see cref=""" + table.Type.Name + @"""/> to update.</param>");
			stringBuilder.AppendLine($"		private static void ExecuteUpdate({table.Type.Name} {param})");
			stringBuilder.AppendLine("			=> Database.ExecuteNonQuery(");
			stringBuilder.AppendLine($"				{queryBuilder},");
        ...
        private static void GenerateParameters(StringBuilder stringBuilder, Table table, string paramName, bool includePrimaryKeys = false)
        {
            ...
            {
                var column = table.Columns[i];
                if ((column.IsPrimaryKey && column.IsAutoIncrement && !includePrimaryKeys) || column.ReadOnly)
                {
                    continue;
                }
        ...
        private static void GenerateUpdateCompare(StringBuilder stringBuilder, Table table)
        {
			...
			stringBuilder.AppendLine(@"		/// Updates the specified <see cref=""" + table.Type.Name + @"""/> in the database.");
			stringBuilder.AppendLine(@"		/// </summary>");
			stringBuilder.AppendLine($"		private static bool ExecuteUpdate({table.Type.Name} {paramNew}, {table.Type.Name} {paramOld})");
			stringBuilder.AppendLine(@"		{");
		...
		private static void GenerateDelete(StringBuilder stringBuilder, Table table)
        {
			...
			stringBuilder.AppendLine(@"		/// </summary>");
			stringBuilder.AppendLine($@"		/// <param name=""{param}"">The primary key of the <see cref=""" + table.Type.Name + @"""/> to delete.</param>");
			stringBuilder.AppendLine($"		private static void ExecuteDelete({TypeName(table.PrimaryKey.Type.Name)} {param})");
			...
			stringBuilder.AppendLine(@"		/// </summary>");
			stringBuilder.AppendLine($@"		/// <param name=""{param}"">The <see cref=""" + table.Type.Name + @"""/> to delete.</param>");
			stringBuilder.AppendLine($"		private static void ExecuteDelete({table.Type.Name} {param})");
            stringBuilder.AppendLine($"			=> ExecuteDelete({param}.{table.PrimaryKey.FieldName});");
        }


		private static string GenerateParameterDirect(Column column,string paramName)
			=> $"new MySqlParameter(\"{column.Name}\", {Pack($"{paramName}", column)})";
