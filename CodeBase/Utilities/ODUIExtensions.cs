...
namespace CodeBase
{
    public static class ODUIExtensions
	{
		...
		/// The items in the ComboBox must be ODBoxItems.
		/// </summary>
		...
		/// </summary>
		/// <param name="fSelectItem">A func that takes an object that is the same type as the ODBoxItems Tags and returns a bool, i.e. x => x.ClinicNum==0.</param>
		public static void SetSelectedItem<T>(this ListBox listBox, Func<T, bool> fSelectItem)
		{
			for (int i = 0; i < listBox.Items.Count; i++)
			{
                if (!(listBox.Items[i] is ODBoxItem<T> odBoxItem))
                {
                    continue;
                }
		...
		/// <summary>
		/// Sets the items for the listBox to the given items list.
		/// </summary>
		/// <param name="fItemToString">
		/// Func that takes an object that is the same type as the ODBoxItems Tags and returns a string to be displayed for this item, 
		/// i.e. x => x.Abbr.
		/// </param>
		/// <param name="fSelectItem">
		/// Optional func that takes an object that is the same type as the ODBoxItems Tags and returns a bool, 
		/// i.e. x => x.ClinicNum==0. Pass null if you don't want to set initial item(s) to be selected.
		/// </param>
		...
		/// <summary>
		/// Gets all the Tags for all ODBoxItems added to Items.
		/// </summary>
		public static List<T> AllTags<T>(this ListBox listBox)
		{
			return listBox.Items.OfType<ODBoxItem<T>>().Select(x => x.Tag).ToList();
		}

		/// <summary>
		/// Returns true if a SelectedItem exists and the SelectedItem's Tag is of type T.
		/// </summary>
		...
		/// <summary>
		/// Returns the tag of the selected item. The items in the ListBox must be ODBoxItems.
		/// </summary>
		...
		/// <summary>
		/// Returns the tags of the selected items. The items in the ListoBox must be ODBoxItems.
		/// </summary>
		...
		/// <summary>
		/// Takes a number of elements from the end of the enumerable.
		/// </summary>
		public static IEnumerable<T> TakeLast<T>(this IEnumerable<T> source, int count) 
			=> source.Skip(Math.Max(0, source.Count() - count));

		/// <summary>
		/// Sometimes ODProgress can cause other forms to open up behind other applications.
		/// Call this method to force this form to the front.
		/// </summary>
