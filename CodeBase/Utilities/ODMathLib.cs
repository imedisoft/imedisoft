...
namespace CodeBase
{
    public class ODMathLib
	{
		...
		/// Returns no floats is there is no intersection and returns 2 floats if the intersection is a 
		/// segment (although both segment points may be the same).
		/// </summary>
		...
		/// <summary>
		/// Returns the intersection of the given two rectangles as a set of 4 floats in the order 
		/// (x,y,w,h), or returns an array of 0 floats if the two rectangles do not intersect.  
		/// 
		/// jordan Probably obsolete and needs to be removed soon.
		/// </summary>
		public static float[] IntersectRectangles(float x1, float y1, float w1, float h1, float x2, float y2, float w2, float h2)
		{
			...
			// No intersection?
			if (xIntersect.Length == 0 || yIntersect.Length == 0) return new float[0];
