...
namespace CodeBase
{
	...
	public class ODThread
	{
		...
		public static bool MakeThread(IODThread threadClass, bool isAutoStart = false, string groupName = "")
		{
            ...
            {
                Name = threadClass.GetThreadName()
            };
            if (groupName != "")
			{
			...
			bool doRunOnCurrentThreadIf1Processor = false)
		{
			...
			if (threadCount == 1 && doRunOnCurrentThreadIf1Processor)
			{
				foreach (Action action in listActions)
				{
					...
					{
						exceptionFirst ??= ex;//First exception in will get thrown.
					}
				}
				HandleException();
				return;
			...
			for (int i = 0; i < listActions.Count; i++)
			{
				...
				if (listActionsCur.Count == actionsPerThread || i == (listActions.Count - 1))
				{
                    ...
                    {
                        Tag = new List<Action>(listActionsCur),
	...
	/// <summary>
	/// This interface is needed for the MakeThread method so that we don't have to copy additional files to the UpdateFileCopier project.
	/// </summary>
