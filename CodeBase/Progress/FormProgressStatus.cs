...
namespace CodeBase
{
	...
	/// Invoke CloseGracefully() in order to have the window gracefully close (as to not rely on thread abort).
	/// </summary>
	public partial class FormProgressStatus : FormProgressBase
	{
		/// <summary
		/// >Indicates that this progress form is in "history" mode which will show a text box with all 
		/// messages it takes action on and then will stay open forcing the user to click Close.
		/// </summary>
		private bool _hasHistory;

		/// <summary>
		/// The date and time of the most recent event that this form processed.
		/// </summary>
		private DateTime _dateTimeLastEvent;

		/// <summary>
		/// The date and time that this form was initialized. Used to help calculate the total time.
		/// </summary>
		private DateTime _dateTimeInit;

		/// <summary>
		/// Do not instatiate this class.  It is not meant for public use. Use ODProgress.ShowProgressStatus() instead.
		...
		/// eventType must be a Type that contains an event called Fired.
		/// </summary>
		public FormProgressStatus(
			EventCategory odEventType = EventCategory.Undefined, Type eventType = null, bool hasHistory = false, bool hasMinimize = true, string startingMessage = "", ProgressBarStyle progStyle = ProgressBarStyle.Marquee) : base(odEventType, eventType)
		{
			InitializeComponent();
		...
		/// <summary>
		/// Gets a user friendly elapsed time message to display in the history text box.
		/// </summary>
		...
		private void CopyButton_Click(object sender, EventArgs e)
		{
			try
			{
				ODClipboard.Text = textHistoryMsg.Text;
