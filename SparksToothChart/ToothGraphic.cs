...
namespace SparksToothChart
{
    ...
    public class ToothGraphic
	{
		...
		/// Performed before any other rotation or translation.
		/// Positive numbers are clockwise as viewed from occlusal.
		/// </summary>
		public float Rotate;
		
		/// <summary>
		/// Rotation to the buccal or lingual about the X axis in degrees.
		/// Positive numbers are to buccal. Happens 2nd, after rotateY.
		/// Not common. Usually 0.
		/// </summary>
		public float TipB;
		...
		/// Aka tipping. 
		/// Positive numbers are mesial. 
		/// Happens last after all other rotations.
		/// </summary>
		public float TipM;
		
		/// <summary>
		/// Shifting mesially or distally in millimeters. 
		/// Mesial is always positive, distal is negative. 
		/// Happens after all three rotations.
		/// </summary>
		public float ShiftM;
		
		/// <summary>
		/// Shifting vertically in millimeters.
		/// Aka supereruption. 
		/// Happens after all three rotations. 
		/// A positive number is super eruption, whether on the maxillary or mandibular.
		/// </summary>
		public float ShiftO;
		...
		/// <summary>
		/// This gets set to true if this tooth has an RCT.
		/// The purpose of this flag is to cause the root to be painted transparently on top of the RCT.
		/// </summary>
		public bool IsRCT;

        /// <summary>
		/// The X indicates that an extraction is planned.
		/// </summary>
		...
		/// <summary>
		/// True if this tooth is set to show the primary letter in addition to the perm number.
		/// This can only be true for 20 of the 32 perm teeth.
		/// A primary tooth would never have this value set.
		/// </summary>
		...
		/// <summary>
		/// This will be true if the tooth is a crown or retainer.
		/// The reason it's needed is so that crowns will show on missing teeth with implants.
		/// </summary>
		public bool IsCrown;
		
		/// <summary>
		/// True if the tooth is a bridge pontic, or a tooth on a denture or RPD.
		/// This makes the crown visible from the F view, but not the occlusal. 
		/// It also makes the root invisible.
		/// </summary>
		...
		/// <summary>
		/// For perio, 1, 2, or 3.  It's a string because we'll later allow +.
		/// The text gets drawn directly on the front of the tooth.
		/// </summary>
		...
		/// <summary>
		/// Shallow copy. If the toothgraphic is being copied for use on a different DirectX device,
		/// then pass in the toothgraphic for the other device in order to maintain the model data on the destination device.
		/// If toothGraphicOtherDevice is not null, then return value will include shallow copies of vb and Groups from toothGraphicOtherDevice.
		/// </summary>
		public ToothGraphic Copy(ToothGraphic toothGraphicOtherDevice = null)
		{
			...
			ToothGraphic toothGraphicCopy = (ToothGraphic)MemberwiseClone();
		...
		/// <summary>
		/// Only called from ToothChartWrapper.ResetTeeth or from ToothChartOpenGL.ResetTeeth when program first loads.
		/// Constructor requires passing in the toothID.
		/// Exception will be thrown if not one of the following: 1-32 or A-T.
		/// Always loads graphics data from local resources even if in simplemode.
		/// </summary>
        ...
        /// <summary>
		/// Set true to hide the number of the tooth from displaying.
		/// Visible should also be set to false in this case.
		/// </summary>
		...
		/// <summary>
		/// Resets this tooth graphic to original location, visiblity, and no restorations.
		/// If primary tooth, then Visible=false.
		/// </summary>
		public void Reset()
		{
		...
		/// <summary>
		/// Used in constructor and also in Reset.
		/// Also sets visibility of all groups to true except RCT visible false.
		/// </summary>
		...
		/// <summary>
		/// Strict validator for toothID. 
		/// Will not handle any international tooth nums since all internal toothID's are in US format.
		/// True if 1-32 or A-T.  Since supernumerary not currently handled, 51-82 and AS-TS return false.
		/// Any invalid values also return false.
		/// </summary>
		public static bool IsValidToothID(string tooth_id)
		{
			if (!Tooth.IsValidDB(tooth_id)) return false;

			if (Tooth.IsSuperNum(tooth_id)) return false;
			
			return true;
		}

		/// <summary>
		/// The tooth_id should be validated before coming here, but it won't crash if invalid.
		/// Primary or perm are ok. 
		/// Empty and null are also ok.
		/// Result is always 1-32 or -1 if invalid tooth_id.
		/// Supernumerary not allowed
		/// </summary>
		...
		/// Used to lay out the teeth on the screen.
		/// Value taken from Wheelers Dental Anatomy.
		/// Used in the orthographic projection.
		/// Also used when building the teeth in the first place.
		/// The perspective projection will need to adjust these numbers because of the curvature of the arch.
		/// Only the widths of permanent teeth are used for layout.  If a primary tooth is passed in, it will give an error.
		/// </summary>
		...
		public static float GetWidth(int tooth_num) 
			=> GetWidth(tooth_num.ToString());

		/// <summary>
		/// The x position of the center of the given tooth, with midline being 0.
		...
		public static bool IsMaxillary(string tooth_id) 
			=> IsValidToothID(tooth_id) && Tooth.IsMaxillary(tooth_id);
		
		public static bool IsAnterior(string tooth_id) 
			=> IsValidToothID(tooth_id) && Tooth.IsAnterior(tooth_id);
		...
		/// <summary>
		/// Tries to find an existing VertexNormal in the list for this tooth.
		/// If it can, then it returns that index.
		/// If it can't then it adds this VertexNormal to the list and returns the last index.
		/// </summary>
