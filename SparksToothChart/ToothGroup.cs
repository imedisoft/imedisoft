...
namespace SparksToothChart
{
    ...
    /// If a group does not apply, such as a F on a posterior tooth, then that tooth will not have that group.
    /// The code must be resiliant enough to handle missing groups.  We might add more groups later and subdivide existing groups.
    /// For instance pits, grooves, cusps, cervical areas, etc.
    /// Over the years, this could get quite extensive and complex.
    /// We would have to add a table to the database to handle additional groups painted by user.
    /// </summary>
    public class ToothGroup
	{
		...
		/// <summary>
		/// Corresponds to the number of indicies referenced by the facesDirectX IndexBuffer.
		/// This relates to triangles, not to Polygons as in the Faces list. Must be a multiple of 3.
		/// </summary>
		...
		public void PrepareForDirectX(Device device)
		{
			...
			int[] arrayOptimizedIndices = D3DX.OptimizeFaces(listTriangles.ToArray(), listTriangles.Count / 3, uniqueIndicies.Count);
		...
		public override string ToString() 
			=> GroupType.ToString() + ". Faces:" + Faces.Count.ToString();
	}
	...
	public enum ToothGroupType
	{
		...
		/// <summary>
		/// Class V. In addition to B or F.
		/// </summary>
		V,

		/// <summary>
		/// The pulp chamber and post or buildup. 
		/// Because our teeth are not yet transparent, these faces need to be translated forward.
		/// </summary>
		Buildup,
		
		/// <summary>
		/// Only defined on some anterior teeth.  The small bit of enamel on the F that is not included in any filling.
		/// </summary>
		EnamelF,
		
		/// <summary>
		/// Only defined on some anterior teeth.  The F portion of the D filling.
		/// </summary>
		DF,
		
		/// <summary>
		/// Only defined on some anterior teeth.  The F portion of the M filling.
		/// </summary>
		MF,
		
		/// <summary>
		/// Only defined on some anterior teeth.  The F portion of the I filling.
		/// </summary>
		IF,
