...
namespace UnitTestsCore {
	public class PaymentT {
		...
		public static PaymentEdit.IncomeTransferData BalanceAndIncomeTransfer(long patNum,Family fam=null,Payment regularTransferPayment=null
			,List<PayPlanCharge> payPlanCharges=null,bool doIncludeHidden=false) 
		{
			if(fam==null) {
			...
			PaymentEdit.ConstructResults results=PaymentEdit.ConstructAndLinkChargeCredits(fam.ListPats.Select(x => x.PatNum).ToList(),
				patNum,new List<PaySplit>(),regularTransferPayment,new List<AccountEntry>(),isIncomeTxfr:true,doShowHiddenSplits:doIncludeHidden);
			#endregion
			//begin transfer loops - Does not insert objects into database from this method. Testing method needs to insert.
			if(!PaymentEdit.TryCreateIncomeTransfer(results.ListAccountCharges,DateTimeOD.Today,out PaymentEdit.IncomeTransferData incomeTransferData)) {
				throw new ODException(incomeTransferData.SummaryText);
			}
