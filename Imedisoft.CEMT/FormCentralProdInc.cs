	...
	public partial class FormCentralProdInc : Form
	{
		...
		private void RunDaily()
		{
			...
			for (int i = 0; i < listThreads.Count; i++)
			{
				...
				if (data == null)
				{
					stringFailedConns += Connections.GetConnectionString(connection) + "\r\n";
				}
				else
				{
					listReportData.Add(new CEMTReportData(data, (Connection)obj[1]));
				}
		...
		private void RunProvider()
		{
			...
			for (int i = 0; i < listThreads.Count; i++)
			{
				...
				if (data == null)
				{
					strFailedConn += Connections.GetConnectionString(connection) + "\r\n";
				}
				else
				{
					listReportData.Add(new CEMTReportData(data, connection, listConnProvs));
				}
				listThreads[i].QuitSync(Timeout.Infinite);
			}
			...
			foreach (CEMTReportData reportDataCur in listReportData)
			{
				...
				foreach (Provider provCur in reportDataCur.Providers)
				{
					DataRow row = dtTotal.NewRow();
					row[0] = provCur.Abbr;
					production = 0;
		...
		{
			Connection conn = (Connection)odThread.Parameters[0];
		...
		/// <summary>
		/// Stores data needed to run the Cemt P&I reports.
		/// </summary>
		private class CEMTReportData
		{
			/// <summary>
			/// Stores the report's DataSet.
			/// </summary>
			public DataSet Data { get; }

			/// <summary>
			/// The CentralConnection associated to the Data.
			/// </summary>
			public Connection Connection { get; }

			public List<Provider> Providers { get; }

			public CEMTReportData(DataSet data, Connection connection, List<Provider> providers = null)
			{
				Data = data;
			...
			public string ConnectionString 
				=> Connections.GetConnectionString(Connection);

			public bool HasClinicBreakdown 
				=> Connection != null && Connection.HasClinicBreakdownReports;
		}
	}
}