    ...
    public class DataConnection : IDisposable
	{
		...
		public static int CommandTimeout
		{
			...
			set
			{
				...
				{
					ConnectionString = BuildSimpleConnectionString(ServerName, Database, MysqlUser, MysqlPass);
				}
			}
		}
		...
		public static int DefaultPortNum() => 3306;
		...
		/// Does not query the database.
		/// </summary>
		public static string GetDatabaseName() => Database;
		...
		/// <summary>
		/// This flag indicates that the connection will try to reconnect if connection is lost.
		/// Thread specific.
		/// </summary>
		...
		public DataConnection(string database) : this(ServerName, database, MysqlUser, MysqlPass) { }
		...
		/// <summary>
		/// Instantiates a new database connection utilizing the connection variables passed in. 
		/// connectStr will be used unless it is null or empty AND serverName is not null.
		/// </summary>
		...
		/// connectStr is never used when isLow is true; however, connectStr will be used unless it is null or empty AND serverName is not null.
		/// <summary>
		public DataConnection(string connectStr, string serverName, string database, string mysqlUser, string mysqlPass, string mysqlUserLow, string mysqlPassLow, bool isLow)
		{
			...
			if (ODThread.SetDatabaseContextChild == null)
			{
				...
				ODThread.SetDatabaseContextChild = (databaseContext) =>
				{
					...
					{
						dataConn.SetDbT((string)databaseContext, "", true, true);
					}
				};
			}
			#endregion
		...
		/// <summary>
		/// Returns a connection string constructed with the lower privileged MySQL user and password.
		/// </summary>
		public static string GetLowConnectionString() 
			=> BuildSimpleConnectionString(ServerName, Database, MysqlUserLow, MysqlPassLow);

		public static string GetReportConnectionString(string pServer, string pDatabase, string pUserID, string pPassword)
		{
			DataConnection rdcon = new DataConnection(pServer, pDatabase, pUserID, pPassword);
		...
		/// <summary>
		/// This method sets all the thread specific variables for the DataConnection.
		/// It will leave all normal static connection variables.  
		/// Should be called before connecting to a database from a thread outside of the main thread.
		/// Can optionally validate both high and low permission connections by running an arbitrary query against each.
		/// </summary>
		public void SetDbT(string server, string db, string user, string password, string userLow, string passLow, bool skipValidation = false)
		{
			string connectStr = BuildSimpleConnectionString(server, db, user, password);
			string connectStrLow = "";
			if (userLow != "")
		...
		/// E.g. new threads need to preserve parent database context but shouldn't crash the program if the database is currently unavailable.
		/// </summary>
		public void SetDbT(string connectStr, string connectStrLow, bool skipValidation = false, bool skipTestConnection = false)
		{
		...
		/// If true, then InsertID will be set to the value of the primary key of the newly inserted row.
		...
		public long NonQ(string commands, bool getInsertID, params MySqlParameter[] parameters) 
			=> NonQ(commands, getInsertID, "", "", true, parameters);
		...
		/// Uses datareader instead of datatable, so faster.
		...
		/// <summary>
		/// A method of bulk copying the data from the passed in datatable into the database table of the given tableName.
		/// This is primarily used as a way of transferring data from one server to another, as we cannot backup data across 
		/// servers as easily as for the same server.
		/// Exports the datatable to a temporary ^ separated value file and then importing it to the database using MySqlBulkLoader.
		/// </summary>
		...
		/// </summary>
		private void RunMySqlAction(Action actionDb) => RunDbAction(actionDb, _con, _cmd);
		...
		private static void RunDbAction(Action actionDb, System.Data.Common.DbConnection connection, System.Data.Common.DbCommand cmd)
		{
			try
			{
				QueryMonitor.RunMonitoredQuery(actionDb, cmd);
			}
		...
		/// <summary>
		/// Returns true if this MySQL command can be safely retried.
		/// </summary>
		...
		private bool StartConnectionErrorRetry(string connectionString, DataConnectionEventType errorType)
		{
			...
			ODThread threadRetry = new ODThread(500, (o) =>
			{
				...
				{
					//Connection has not been restored yet.  Wait a little bit and then try again.
				}
			});
