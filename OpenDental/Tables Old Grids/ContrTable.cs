...
namespace OpenDental
{
    public class ContrTable : UserControl
	{
		...
		private UserControl panelTable;
		...
		private void InitializeComponent()
		{
			...
			this.panelTable = new UserControl();
		...
		public void LayoutTables(bool preserveScroll)
		{
			...
			if (ColWidth[MaxCols - 1] != 0)
			{
				...
				else panelTable.Height = rowPos[MaxRows - 1] + RowHeight[MaxRows - 1];
		...
		protected void panelHead_Paint(object sender, PaintEventArgs pea)
		{
			...
			for (int j = 0; j < MaxCols; j++)
			{
				offset = (int)((ColWidth[j] - grfx.MeasureString(Fields[j], myFont).Width) / 2);
	...
	{
        public CellEventArgs(int col, int row)
		{
			Row = row;
			Col = col;
		}
