...
//		private System.Windows.Forms.PrintDialog printDialog2;
    ...
    private ArrayList ALPreAuth;
		...
		public void InitializeOnStartup() {
			...
			checkShowCompleted.Checked=Preferences.GetBool(PreferenceName.TreatPlanShowCompleted);
			if(PrefC.RandomKeys) {//random PKs don't get the option or sorting by order entered
				tabShowSort.TabPages.Remove(tabPageSort);
			}
			else {
				radioTreatPlanSortTooth.Checked=Preferences.GetBool(PreferenceName.TreatPlanSortByTooth);
			}
			//checkShowIns.Checked=Prefs.GetBool(PrefName.TreatPlanShowIns");
			//checkShowDiscount.Checked=Prefs.GetBool(PrefName.TreatPlanShowDiscount");
		...
		public void LayoutToolBar(){
			...
			ToolBarMain.Buttons.Add(new ODToolBarButton("PreAuthorization",-1,"","PreAuth"));
			ToolBarMain.Buttons.Add(new ODToolBarButton("Discount",-1,"","Discount"));
			ToolBarMain.Buttons.Add(new ODToolBarButton("Update Fees",1,"","Update"));
			//ToolBarMain.Buttons.Add(new ODToolBarButton("Save TP",3,"","Create"));
			ToolBarMain.Buttons.Add(new ODToolBarButton("Print TP",2,"","Print"));
			ToolBarMain.Buttons.Add(new ODToolBarButton("Email TP",-1,"","Email"));
			ToolBarMain.Buttons.Add(new ODToolBarButton("Sign TP",-1,"","Sign"));
      ...
      FillPreAuth();
		...
		private void FillPlans(bool doRefreshData=true){
			...
			col=new GridColumn("Status",50);
			gridPlans.Columns.Add(col);
			col=new GridColumn("Heading",230);
			gridPlans.Columns.Add(col);
			col=new GridColumn("Signed",76,HorizontalAlignment.Center);
			gridPlans.Columns.Add(col);
			...
			//row.Cells.Add("");//date empty
			//row.Cells.Add("Current Treatment Plans");
			...
			for(int i=0;i<_listTreatPlans.Count;i++){
				...
				if(treatPlanCur.ResponsParty!=0){
					str+="\r\n"+"Responsible Party: "+Patients.GetLim(treatPlanCur.ResponsParty).GetNameLF();
				}
				row.Cells.Add(str);
		...
		private void FillMainData() {
			...
			if((_listTreatPlans[gridPlans.SelectedIndices[0]].TPStatus==TreatPlanStatus.Saved 
				...
				&& _listTreatPlans[gridPlans.SelectedIndices[0]].Heading=="Unassigned"))
			{
			...
			for(int i=0;i<RowsMain.Count;i++) {
				...
				for(int j=0;j<fields.Count;j++) {
					switch(fields[j].InternalName) {
						...
						case "Fee":
							if(checkShowFees.Checked) {
								if(Preferences.GetBool(PreferenceName.TreatPlanItemized) || RowsMain[i].Description.ToString()=="Total"
						...
						case DisplayFields.InternalNames.TreatmentPlanModule.CatPercUCR:
							if(Preferences.GetBool(PreferenceName.TreatPlanItemized) 
								|| RowsMain[i].Description.ToString().In("Total","Subtotal")) 
							{
								row.Cells.Add(RowsMain[i].CatPercUCR.ToString("F"));
							}
							else {
    ...
    private void FillPreAuth(){
			...
			GridColumn col=new GridColumn("Date Sent",80);
			gridPreAuth.Columns.Add(col);
			col=new GridColumn("Carrier",100);
			gridPreAuth.Columns.Add(col);
			col=new GridColumn("Status",53);
			gridPreAuth.Columns.Add(col);
      ...
      for(int i=0;i<ALPreAuth.Count;i++){
		...
		private void SetPriority(Definition selectedPriority,TreatPlan selectedTp,List<ProcTP> listSelectedProcTps) {
			...
			for(int i=0;i<gridMain.Rows.Count;i++) {
				if(gridMain.Rows[i].Tag!=null && listSelectedProcNums.Contains(((ProcTP)gridMain.Rows[i].Tag).ProcNumOrig)) {
					gridMain.SetSelected(i,true);
				}
			}
		}
		...
		private void ToolBarMainPrint_Click() {
			...
			if(Preferences.GetBool(PreferenceName.FuchsOptionsOn)) {
				if(checkShowDiscount.Checked || checkShowIns.Checked) {
					if(MessageBox.Show(this,string.Format("Do you want to remove insurance estimates and discounts from printed treatment plan?"),"Open Dental",MessageBoxButtons.YesNo,MessageBoxIcon.Question) != DialogResult.No) {
						checkShowDiscount.Checked=false;
						checkShowIns.Checked=false;
						FillMain();
					}
			...
			#endregion
			if(_listTreatPlans[gridPlans.SelectedIndices[0]].TPStatus==TreatPlanStatus.Saved
				&& Preferences.GetBool(PreferenceName.TreatPlanSaveSignedToPdf)
			  && _listTreatPlans[gridPlans.SelectedIndices[0]].Signature!=""
		...
		private void ToolBarMainEmail_Click() {
			...
			if(Preferences.GetBool(PreferenceName.FuchsOptionsOn)) {
				if(checkShowDiscount.Checked || checkShowIns.Checked) {
					if(MessageBox.Show(this,string.Format("Do you want to remove insurance estimates and discounts from e-mailed treatment plan?"),"Open Dental",MessageBoxButtons.YesNo,MessageBoxIcon.Question) != DialogResult.No) {
						checkShowDiscount.Checked=false;
						checkShowIns.Checked=false;
						FillMain();
					}
			...
			if(gridPlans.SelectedIndices[0]>0 //not the default plan.
				&& Preferences.GetBool(PreferenceName.TreatPlanSaveSignedToPdf) //preference enabled
			...
			message.Subject="Treatment Plan";
				...
				if(PatPlanList.Count>0) { //Primary
					...
					if(claimproc==null || claimproc.EstimateNote.Contains("Frequency Limitation")) {
						...
						if(checkShowMaxDed.Checked) { //whether visible or not
							...
							if(ded>0) {
								showPriDeduct="\r\n"+"Pri Deduct Applied: "+ded.ToString("c");
							}
						}
						else {
				...
				if(PatPlanList.Count>1) { //Secondary
					...
					if(claimproc==null) {
						...
						if(checkShowMaxDed.Checked) {
							...
							if(ded>0) {
								showSecDeduct="\r\n"+"Sec Deduct Applied: "+ded.ToString("c");
							}
						}
						else {
		...
		private TreatPlan GetCurrentTPHelper() {
			...
			retVal.Heading="Proposed Treatment Plan";
			retVal.DateTP=DateTimeOD.Today;
			retVal.PatNum=PatCur.PatNum;
			retVal.Note=Preferences.GetString(PreferenceName.TreatmentPlanNote);
		...
		private MigraDoc.DocumentObjectModel.Document CreateDocument(){
			...
			if(checkShowIns.Checked) {
				...
				col=new GridColumn("Primary",70,HorizontalAlignment.Right);
				gridFamIns.Columns.Add(col);
				col=new GridColumn("Secondary",70,HorizontalAlignment.Right);
				gridFamIns.Columns.Add(col);
				gridFamIns.Rows.Clear();
				GridRow row;
				//Annual Family Max--------------------------
				row=new GridRow();
				row.Cells.Add("Family Maximum");
				...
				//Family Deductible--------------------------
				row=new GridRow();
				row.Cells.Add("Family Deductible");
				...
				par.AddFormattedText("Family Insurance Benefits",totalFontx);
				...
				par=section.AddParagraph();
				par.Format.Alignment=ParagraphAlignment.Center;
				par.AddFormattedText("Individual Insurance Benefits",totalFontx);
				MigraDocHelper.InsertSpacer(section,2);
			...
			for(int i=0;i<ProcListFiltered.Count;i++) {
				...
				switch(ProcedureCodes.GetById(proc.CodeNum).PaintType) {
					...
					case ToothPaintingType.Watch:
						_toothChartRelay.SetWatch(proc.ToothNum,cDark);
						break;
				}
			}
		}
		...
		private void ToolBarMainCreate_Click(){//Save TP
			...
			if(Preferences.GetBool(PreferenceName.TreatPlanPromptSave)) { 
				...
				InputBox inputHeadingName=new InputBox("Save Treatment Plan as",treatPlanHeading);
		...
		private void ToolBarMainSign_Click() {
			...
			if(Preferences.GetBool(PreferenceName.TreatPlanSaveSignedToPdf) //preference enabled
			   && _listTreatPlans[gridPlans.SelectedIndices[0]].Signature!="" //and document is signed
			   && Documents.DocExists(_listTreatPlans[gridPlans.SelectedIndices[0]].DocNum)) //and file exists
			{
		...
		private List<Document> SaveTPAsDocument(bool isSigSave,Sheet sheet=null) {
			...
			if(isSigSave && categories.Count==0 && Preferences.GetBool(PreferenceName.TreatPlanSaveSignedToPdf)) {
      ...
      FormInsPlanSelect FormIPS=new FormInsPlanSelect(PatCur.PatNum); 
			...
			if(FormIPS.SelectedPlan.IsMedical) {
				ClaimCur.MedType=Preferences.GetBool(PreferenceName.ClaimMedTypeIsInstWhenInsPlanIsMedical) ? EnumClaimMedType.Institutional : EnumClaimMedType.Medical;
			}
      ...
      //FormClaimEdit2.IsPreAuth=true;
		...
		private void butPlannedAppt_Click(object sender,EventArgs e) {
			...
			List<long> listSelectedProcNums=gridMain.SelectedRows
		...
		private void UpdateToolbarButtons() {
			if(PatCur!=null && _listTreatPlans.Count>0) {
				...
				if(PatPlanList.Count==0) {//patient doesn't have insurance
					...
					if(!Preferences.GetBool(PreferenceName.EasyHideInsurance)) {//if insurance isn't hidden
						checkShowMaxDed.Visible=true;
