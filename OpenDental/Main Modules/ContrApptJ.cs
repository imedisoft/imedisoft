...
namespace OpenDental
{
    ...
    public partial class ContrAppt : UserControl
	{
		...
		private void contrApptPanel_ApptDoubleClicked(object sender, UI.ApptEventArgs e)
		{
			...
			{//appt already saved or deleted inside that window
				...
				if (apt != null)
				{
					...
					if (!Preferences.GetBool(PreferenceName.ApptsAllowOverlap, true))
					{
		...
		private void contrApptPanel_ApptMainAreaDoubleClicked(object sender, UI.ApptMainClickEventArgs e)
		{
			...
			if (_patCur != null)
			{
				formPatientSelect.InitialPatientId = _patCur.PatNum;
			}
			...
			if (_patCur == null || formPatientSelect.SelectedPatientId != _patCur.PatNum)
			{//if the patient was changed
				RefreshModuleDataPatient(formPatientSelect.SelectedPatientId);
		...
		private void contrApptPanel_ApptMovedToPinboard(object sender, UI.ApptDataRowEventArgs e)
		{
			...
			if (Preferences.GetBool(PreferenceName.BrokenApptRequiredOnMove))
			{
		...
		private void ContrApptPanel_ApptRightClicked(object sender, UI.ApptRightClickEventArgs e)
		{
			...
			menuApt.MenuItems.RemoveByKey(MenuItemNames.SendComeInText);
			menuApt.MenuItems.RemoveByKey(MenuItemNames.OrthoChart);
			MenuItem menuItem;
			if (Preferences.GetBool(PreferenceName.ApptModuleShowOrthoChartItem))
			{
		...
		private void toolBarPrint_Click()
		{
			...
			List<long> listVisOpNums = contrApptPanel.ListOpsVisible.Select(x => x.Id).ToList();
		...
		private void butBreak_Click(object sender, System.EventArgs e)
		{
			if (Preferences.GetBool(PreferenceName.BrokenApptAdjustment)
				&& Preferences.GetLong(PreferenceName.BrokenAppointmentAdjustmentType) == 0)
			{
		...
		private void butComplete_Click(object sender, System.EventArgs e)
		{
			...
			if (appt.AptStatus != ApptStatus.PtNote && appt.AptStatus != ApptStatus.PtNoteCompleted  //Ptnote cannot have procs attached
				&& !Preferences.GetBool(PreferenceName.ApptAllowEmptyComplete)//Appointments must have at least 1 proc
				&& listProcs.Count == 0)
			{
			...
			if (listProcs.Count > 0 && appt.AptDateTime.Date > DateTime.Today.Date && !Preferences.GetBool(PreferenceName.FutureTransDatesAllowed))
			{
		...
		private void butDelete_Click(object sender, System.EventArgs e)
		{
			...
			if (sender == this)
			{//this method was called from somewhere else in this class rather than the delete button.
		...
		private void butUnsched_Click(object sender, System.EventArgs e)
		{
			...
			if (Preferences.GetBool(PreferenceName.UnscheduledListNoRecalls) && Appointments.IsRecallAppointment(appt))
			{
		...
		private void pinBoard_ApptMovedFromPinboard(object sender, UI.ApptFromPinboardEventArgs e)
		{
			...
			try
			{
				...
				if (apptCur.AptStatus != ApptStatus.PtNote && apptCur.AptStatus != ApptStatus.PtNoteCompleted)
				{
					...
					if ((assignedDent != 0 && assignedDent != apptCur.ProvNum) || (assignedHyg != 0 && assignedHyg != apptCur.ProvHyg))
					{
						if (MsgBox.Show(MsgBoxButtons.YesNo, "Change provider?"))
						{
							...
							if (assignedHyg != 0 || Preferences.GetBool(PreferenceName.ApptSecondaryProviderConsiderOpOnly))
							...
							if (apptCur.Pattern != calcPattern && !Preferences.GetBool(PreferenceName.AppointmentTimeIsLocked))
							{
				...
				if (Preferences.GetBool(PreferenceName.InsChecksFrequency))
				{
		...
		private void pinBoard_ModuleNeedsRefresh(object sender, EventArgs e)
		{
			...
			{
				listOpNums = contrApptPanel.ListOpsVisible.Select(x => x.Id).ToList();
		...
		private void GridEmpOrProv_DoubleClick(object sender, EventArgs e)
		{
			...
			FormScheduleDayEdit formScheduleDayEdit = new FormScheduleDayEdit(contrApptPanel.DateSelected, Clinics.Active.Id);
		...
		private void gridReminders_MouseDown(object sender, MouseEventArgs e)
		{
			...
			{//check tasks off
				...
				{
					long userNumInbox = TaskLists.GetMailboxUserNum(reminderTask.TaskListId);
					...
					if (TaskUnreads.IsUnread(Security.CurrentUser.Id, reminderTask))
					{
						...
						//reminderTask.Status=TaskStatus.Viewed;
						...
						long signalNum = Signalods.SetInvalid(InvalidType.Task, KeyType.Task, reminderTask.Id);
						UserControlTasks.RefillLocalTaskGrids(reminderTask, new List<long>() { signalNum });
					}
					//if already read, nothing else to do.  If done, nothing to do
				}
				else
				{
					if (reminderTask.Status == TaskStatus.New)
					{
		...
		private void menuApt_Popup(object sender, EventArgs e)
		{
			...
			MenuItem item = null;
			bool dontAllowUnscheduled = Preferences.GetBool(PreferenceName.UnscheduledListNoRecalls)
				&& Appointments.IsRecallAppointment(Appointments.GetOneApt(contrApptPanel.SelectedAptNum));
			BrokenApptProcedure brokenApptProcs = (BrokenApptProcedure)PrefC.GetInt(PreferenceName.BrokenApptProcedure);
		...
		{
			if (!Preferences.GetBool(PreferenceName.WebSchedAsapEnabled) || Appointments.RefreshASAP(0, 0, appt.ClinicNum, new List<ApptStatus>()).Count == 0
		...
		private void menuApt_Click(object sender, System.EventArgs e)
		{
			switch (((MenuItem)sender).Name)
			{
				...
				case MenuItemNames.SendConfirmationText:
					...
					if (wasTextSent)
					{
						Appointments.SetConfirmed(appt, Preferences.GetLong(PreferenceName.ConfirmStatusTextMessaged));
					}
					break;
		...
		private void menuBlockAdd_Click(object sender, EventArgs e)
		{
			...
			FormScheduleBlockEdit formScheduleBlockEdit = new FormScheduleBlockEdit(schedule, Clinics.Active.Id, listDefsBlockoutTypes);
		...
		private void menuBlockEdit_Click(object sender, EventArgs e)
		{
			...
			FormScheduleBlockEdit FormSB = new FormScheduleBlockEdit(scheduleClicked, Clinics.Active.Id, listUserBlockoutDefs);
			FormSB.ShowDialog();
			RefreshPeriodSchedules();
		}
		...
		private void menuBlockPaste_Click(object sender, EventArgs e)
		{
			...
			{
				if (!Preferences.GetBool(PreferenceName.ReplaceExistingBlockout) || !Schedules.IsAppointmentBlocking(sched.BlockoutType))
				{
		...
		{
			List<Appointment> listAppts = Appointments.GetForPeriodList(contrApptPanel.DateSelected, contrApptPanel.DateSelected, Clinics.Active.Id)
		...
		private void menuTextApptsForDayView_Click(object sender, EventArgs e)
		{
			...
			for (int i = 0; i < contrApptPanel.TableAppointments.Rows.Count; i++)
			{
				...
				if (!contrApptPanel.ListOpsVisible.Any(y => y.Id == opNum))
		...
		private void ButAdvSearch_Click(object sender, EventArgs e)
		{
			...
			foreach (ODBoxItem<Provider> prov in providersListBox.Items)
			{
			...
			for (int i = 0; i < listProvidersShort.Count; i++)
			{
				if (contrApptPanel.ListApptViewItems.Exists(x => x.ProvNum == listProvidersShort[i].Id))
				{
					if (!listProvidersShort[i].IsSecondary)
					{
						...
						providersListBox.Items.Add(new ODBoxItem<Provider>(listProvidersShort[i].Abbr, listProvidersShort[i]));
					}
				}
			}
			if (pinBoard.SelectedIndex == -1)
		...
		private void ProvidersButton_Click(object sender, EventArgs e)
		{
            ...
            {
                SelectedProviders = _listProvidersSearch
            };
			...
			{
				providersListBox.Items.Add(new ODBoxItem<Provider>(formProvidersMultiPick.SelectedProviders[i].Abbr, formProvidersMultiPick.SelectedProviders[i]));
			}
		...
		public void InitializeOnStartup()
		{
			...
			SetWeeklyView(Preferences.GetBool(PreferenceName.ApptModuleDefaultToWeek));
		...
		public void ModuleSelected(long patNum, List<long> listPinApptNums = null, List<long> listOpNums = null, List<long> listProvNums = null)
		{
			...
			contrApptPanel.MinPerIncr = PrefC.GetInt(PreferenceName.AppointmentTimeIncrement);
			...
			Color colorTimeLine = PrefC.GetColor(PreferenceName.AppointmentTimeLineColor);
			contrApptPanel.SetColors(colorOpen, colorClosed, colorHoliday, colorBlockText, colorTimeLine);
			contrApptPanel.SizeFont = float.Parse(Preferences.GetString(PreferenceName.ApptFontSize));
			contrApptPanel.WidthProvOnAppt = float.Parse(Preferences.GetString(PreferenceName.ApptProvbarWidth));
		...
		public void FunctionKeyPress(Keys keys)
		{
			...
			{//Check for valid F key.(-1 to ignore "None" view).
				return;
			}
		...
		private void SetReminderGridRow(GridRow row, Task reminderTask)
		{
			...
			row.Cells.Clear();
			string dateStr = "";
			if (reminderTask.RepeatDate.HasValue)
			{
				if (reminderTask.RepeatInterval == TaskRepeatInterval.Daily)
				{
			...
			else if (reminderTask.DateStart.HasValue)
			{
				dateStr += reminderTask.DateStart.Value.ToShortDateString() + " " + reminderTask.DateStart.Value.ToShortTimeString() + " - ";
			}
			string objDesc = "";
			...
			else if (reminderTask.AppointmentId.HasValue)
			{
				Appointment appointment = Appointments.GetOneApt(reminderTask.AppointmentId.Value);
				if (appointment != null)
				{
			...
			if (!reminderTask.Description.StartsWith("==") && reminderTask.UserId != 0)
			{
				objDesc += Users.GetUserName(reminderTask.UserId) + " - ";
			}
			if (Preferences.GetBool(PreferenceName.TasksNewTrackedByUser))
			{//The new way
				if (reminderTask.Status == TaskStatus.Done)
		...
		private void RefreshSchedulesIfNeeded(DateTime dateStart, DateTime dateEnd, List<long> listOpNums, bool isRefreshNeeded = false)
		{
			...
			contrApptPanel.TableEmpSched = Schedules.GetPeriodEmployeeSchedTable(dateStart, dateEnd, Clinics.Active.Id);
			contrApptPanel.TableProvSched = Schedules.GetPeriodProviderSchedTable(dateStart, dateEnd, Clinics.Active.Id);
		...
		private void FillProduction(DateTime start, DateTime end)
		{
			...
			if (Preferences.GetBool(PreferenceName.ApptModuleProductionUsesOps))
			{
		...
		private void FillWaitingRoom()
		{
			...
			if (Preferences.GetBool(PreferenceName.WaitingRoomFilterByView))
			{
			...
			DateTime waitTime;
			GridRow row;
			int waitingRoomAlertTime = PrefC.GetInt(PreferenceName.WaitingRoomAlertTime);
			Color waitingRoomAlertColor = PrefC.GetColor(PreferenceName.WaitingRoomAlertColor);
			for (int i = 0; i < table.Rows.Count; i++)
			{
		...
		public void RefreshModuleScreenPeriod()
		{
			...
			if (PrefC.HasClinicsEnabled && Clinics.ClinicId > 0)
			{
				opNums = Operatories.GetOpsForClinic(Clinics.Active.Id).Select(x => x.Id).ToList();
			}
		...
		public void TickRefresh()
		{
			...
			{
				//dates already set
				if (Preferences.GetBool(PreferenceName.ApptModuleRefreshesEveryMinute))
				{
					...
					else
					{
						...
						List<Signalod> listSignals = Signalods.RefreshTimed(Signalods.ApptSignalLastRefreshed, new string[] { SignalName.Appointment, SignalName.Schedules }).ToList();
						List<long> listOpNumsVisible = contrApptPanel.ListOpsVisible.Select(x => x.Id).ToList();
						List<long> listProvNumsVisible = contrApptPanel.ListProvsVisible.Select(x => x.Id).ToList();
		...
		private void DoSearch()
		{
			...
			long[] providers = new long[providersListBox.Items.Count];
			List<long> providerNums = new List<long>();
			for (int i = 0; i < providers.Length; i++)
			{
				providers[i] = _listProvidersSearch[i].Id;
				providerNums.Add(_listProvidersSearch[i].Id);
			...
			if (PrefC.HasClinicsEnabled)
			{
				...
				{//not HQ
					...
					listOpNums = Operatories.GetOpsForClinic(Clinics.Active.Id).Select(x => x.Id).ToList();//get ops for the currently selected clinic only
				}
				else
				{//HQ
					ApptView viewCur = GetApptViewCur();
		...
		private void ShowSearch()
		{
			...
			Plugins.HookAddCode(this, "ContrAppt.ShowSearch_end", providersListBox, aptNum);
		...
		{
			if (Preferences.GetBool(PreferenceName.BrokenApptRequiredOnMove) && appt.AptStatus == ApptStatus.Scheduled)
			{
		...
		private ApptView GetApptViewForUser()
		{
			...
			UserodApptView userodApptViewCur = UserodApptViews.GetOneForUserAndClinic(Security.CurrentUser.Id, Clinics.Active.Id);
			...
			if (apptView == null //if no entry in the ComputerPref table (or "none" view in ComputerPref table)
				...
				&& (!_hasInitializedOnStartup //and ContrAppt has NOT been initialized yet.
					|| (Preferences.GetBool(PreferenceName.EnterpriseNoneApptViewDefaultDisabled) && comboView.SelectedIndex == -1)))
		...
		private void LayoutPanels()
		{
			...
			contrApptPanel.Width = ClientSize.Width - panelCalendar.Width - 2;
			contrApptPanel.Height = ClientSize.Height;
		...
		public void MoveAppointments(List<Appointment> listAppts, List<Appointment> listApptsOld, Operatory operatoryCur)
		{
			...
			if (((ApptSchedEnforceSpecialty)PrefC.GetInt(PreferenceName.ApptSchedEnforceSpecialty)).In(ApptSchedEnforceSpecialty.Block, ApptSchedEnforceSpecialty.Warn))
			{
		...
		private void MoveAppointment(Appointment appt, Appointment apptOld, List<Schedule> listSchedsForOp = null, bool isOpUpdate = false)
		{
			...
			if (appt.AptStatus != ApptStatus.PtNote && appt.AptStatus != ApptStatus.PtNoteCompleted)
			{
				...
				if ((assignedDent != 0 && assignedDent != appt.ProvNum) || (assignedHyg != 0 && assignedHyg != appt.ProvHyg))
				{
					...
					if (isOpUpdate || MsgBox.Show(MsgBoxButtons.YesNo, "Change provider?"))
					{//Short circuit logic.  If we're updating op through right click, never ask.
						...
						if (!isOpUpdate)
						{
							...
							if (appt.Pattern != calcPattern && !Preferences.GetBool(PreferenceName.AppointmentTimeIsLocked))
			...
			#region Detect Frequency Conflicts
			//Detect frequency conflicts with procedures in the appointment
			if (!isOpUpdate && Preferences.GetBool(PreferenceName.InsChecksFrequency))
			{
				procsForSingleApt = Procedures.GetProcsForSingle(appt.AptNum, false);
				string frequencyConflicts = "";
				try
			...
			{
				string prompt;
				if (Preferences.GetBool(PreferenceName.ApptConfirmAutoEnabled))
				{
					prompt = "Do you want to resend the eConfirmation?";
				}
				else if (Preferences.GetBool(PreferenceName.ApptThankYouAutoEnabled))
				{
				...
				if (Preferences.GetBool(PreferenceName.ApptThankYouAutoEnabled))
				{
		...
		private void pd2_PrintApptCard(object sender, PrintPageEventArgs ev)
		{
			...
			{//Otherwise use practice information
				...
				str += Preferences.GetString(PreferenceName.PracticeCity) + "  "
		...
		private void SendTextMessages(List<long> listPatNums)
		{
			...
			Clinic curClinic = Clinics.GetById(Clinics.Active.Id) ?? Clinics.GetDefaultForTexting() ?? Clinics.GetPracticeAsClinicZero();
			...
			FormTxtMsgMany formTxtMsgMany = new FormTxtMsgMany(listPatComms, "", Clinics.Active.Id, SmsMessageSource.DirectSms);
			...
			{ //None view NOT selected.
				...
				{//Scroll start time at the earliest scheduled operatory or appointment
				 ...
				 //Get the schedules that have any operatory visible
					...
					{
						if (sched.Ops.Any(x => contrApptPanel.ListOpsVisible.Exists(y => x == y.Id))//The schedule is linked to a visible operatory
					...
					long schedProvUnassinged = Preferences.GetLong(PreferenceName.ScheduleProvUnassigned);
					...
					foreach (Operatory op in contrApptPanel.ListOpsVisible)
					{
						...
						if (contrApptPanel.ListSchedules.Any(x => x.Ops.Contains(op.Id)))
						{
					...
					foreach (DataRow row in contrApptPanel.TableAppointments.Rows)
					{
						...
						//todo:
						if (!contrApptPanel.ListOpsVisible.Exists(x => x.Id == opNum) //The appointment is in a visible operatory
		...
		private void SetView(long apptViewNum, bool saveToDb)
		{
			...
			if (saveToDb)
			{
				...
				ComputerPrefs.LocalComputer.ClinicNum = Clinics.Active.Id;
				ComputerPrefs.Update(ComputerPrefs.LocalComputer);
				UserodApptViews.InsertOrUpdate(Security.CurrentUser.Id, Clinics.Active.Id, apptViewNum);
			}
			if (_patCur == null)
			{
				ModuleSelected(0, listOpNums: ApptViewItems.GetOpsForView(apptViewNum), listProvNums: ApptViewItems.GetProvsForView(apptViewNum));
