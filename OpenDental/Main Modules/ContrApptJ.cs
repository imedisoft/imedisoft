...
namespace OpenDental {
	...
	public partial class ContrAppt:UserControl {
		...
		private void contrApptPanel_ApptMainAreaDoubleClicked(object sender,UI.ApptMainClickEventArgs e) {
			...
			if(_patCur==null || formPatientSelect.SelectedPatientId!=_patCur.PatNum) {//if the patient was changed
				...
				if(apptCur.AptStatus!=ApptStatus.PtNote&&apptCur.AptStatus!=ApptStatus.PtNoteCompleted) {
					...
					if((assignedDent!=0&&assignedDent!=apptCur.ProvNum)||(assignedHyg!=0&&assignedHyg!=apptCur.ProvHyg)) {
						if(MsgBox.Show(MsgBoxButtons.YesNo,"Change provider?")) {
							...
							if(assignedHyg!=0||Preferences.GetBool(PreferenceName.ApptSecondaryProviderConsiderOpOnly)) {//the hygienist will only be changed if the spot has a hygienist.
		...
		private void menuTextApptsForDayView_Click(object sender,EventArgs e) {
			...
			for(int i=0;i<contrApptPanel.TableAppointments.Rows.Count;i++){
				...
				if(!contrApptPanel.ListOpsVisible.Any(y => y.Id==opNum)){//Make sure the appointments are visible in the current view.
					continue;
				}
				if(PIn.Date(contrApptPanel.TableAppointments.Rows[i]["AptDateTime"].ToString()).Date!=dateClicked){
					continue;
		...
		private void SetReminderGridRow(GridRow row,Task reminderTask) {
			...
			if(!reminderTask.Description.StartsWith("==") && reminderTask.UserId!=0) {
				objDesc+=Userods.GetName(reminderTask.UserId)+" - ";
			}
			if(Preferences.GetBool(PreferenceName.TasksNewTrackedByUser)) {//The new way
				if(reminderTask.Status==TaskStatus.Done) {
					row.Cells.Add("1");
				}
				else {
		...
		public void TickRefresh(){
			try {
				...
				if(Preferences.GetBool(PreferenceName.ApptModuleRefreshesEveryMinute)) {
					if(Preferences.GetLong(PreferenceName.ProcessSigsIntervalInSecs)==0) {//Signal processing is disabled.
						RefreshPeriod();
					}
					else {
				...
				if((assignedDent!=0 && assignedDent!=appt.ProvNum) || (assignedHyg!=0 && assignedHyg!=appt.ProvHyg)) {
					...
					if(isOpUpdate || MsgBox.Show(MsgBoxButtons.YesNo,"Change provider?")) {//Short circuit logic.  If we're updating op through right click, never ask.
						...
						if(!isOpUpdate) { 
							...
							if(appt.Pattern!=calcPattern && !Preferences.GetBool(PreferenceName.AppointmentTimeIsLocked)) {//Updating op provs will not change apt lengths.
								if(appt.TimeLocked) {
									if(MsgBox.Show(MsgBoxButtons.YesNo,"Appointment length is locked.  Change length for new provider anyway?")) {
