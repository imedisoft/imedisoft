    ...
    private FormSmsTextMessaging _formSmsTextMessaging;
		...
		private void RefreshLocalDataPostCleanup(params InvalidType[] arrayITypes) {//This is where the flickering and reset of windows happens
			...
			if(arrayITypes.Contains(InvalidType.ToolButsAndMounts) || isAll) {
				...
				if(PrefC.GetBoolSilent(PrefName.ImagesModuleUsesOld2020,false)){
					if(ContrImages2!=null){//can be null on startup
		...
		private void LayoutControls(){
			...
			if(userControlTasks1.Visible) {
				if(menuItemDockBottom.Checked) {
					if(panelSplitter.Height>8) {//docking needs to be changed from right to bottom
						panelSplitter.Height=7;
		...
		private void allNeutral(){
			...
			if(PrefC.GetBoolSilent(PrefName.ImagesModuleUsesOld2020,false)){
				...
				if(ContrImagesJ2!=null){//can be null on startup
		...
		public static void S_SetPhoneStatusAvailable() {
			...
			_formOpenDentalS.phoneSmall.SetAvailable(Employees.GetEmp(Security.CurUser.EmployeeNum).IsWorkingHome);
		}
		...
		private void menuItem_DrawItem(object sender,DrawItemEventArgs e) {
			...
			if((e.State & DrawItemState.HotLight)==DrawItemState.HotLight	|| (e.State & DrawItemState.Selected)==DrawItemState.Selected){
				using(SolidBrush brushBackground=new SolidBrush(Color.FromArgb(224,240,255))){//light blue
					e.Graphics.FillRectangle(brushBackground,e.Bounds);
				}
				using(Pen pen=new Pen(Color.FromArgb(0,173,254))){//med blue line
