...
namespace OpenDental {
	public partial class FormJobManager:ODForm {
		...
		private void FormJobManager_Load(object sender,EventArgs e) {
			...
			if(!JobPermissions.IsAuthorized(JobPerm.Concept,true) && !JobPermissions.IsAuthorized(JobPerm.QueryTech,true) && !JobPermissions.IsAuthorized(JobPerm.DesignTech,true)) {
		...
		private void LoadJob(Job job,bool doRefreshUI,LoadJobAction loadAction=LoadJobAction.Select) {
			if(job==null || JobUnsavedChangesCheck() || QueryUnsavedChangesCheck() || MarketingUnsavedChangesCheck()) {
				return;
			}
			...
			if(userControlJobEdit.Visible) {
				...
				jobNumCur=userControlMarketingEdit.GetJob()?.JobNum??0;
			}
			switch(loadAction) {
			...
			else if(job.Category==JobCategory.MarketingDesign) {
				userControlQueryEdit.Visible=false;
				userControlMarketingEdit.Visible=true;
				userControlJobEdit.Visible=false;
				userControlMarketingEdit.LoadJob(job,Jobs.GetJobTree(job,_listJobsAll),_listJobsAll);
				butRefresh.Enabled=true;
			}
			else {
				userControlQueryEdit.Visible=false;
		...
		private void RefreshControlsAndCacheWithJobs(Dictionary<long,Job> dictRefreshJobs,bool doRefreshUI=true) {
			...
			//Get the job currently loaded into the UserControlQueryEdit control.
			jobCur=userControlMarketingEdit.GetJob();
			//Check to see if the current job is included in the list of jobs we need to refresh (could be due to processing a signal).
			if(jobCur!=null && dictRefreshJobs.ContainsKey(jobCur.JobNum)) {
				//Set jobCur to the new version of the job that was just retrieved from the database.
				jobCur=dictRefreshJobs[jobCur.JobNum];//Could be null if someone else just deleted this job...
		...
		private void FillActiveTabGrid() {
			...
			else if(tabControlNav.SelectedTab==tabTree && !checkResults.Checked) {
				FilterAndFill();
			}
		...
		private void FillGridTesting() {
			...
			List<Job> listTestingJobs=_listJobsAll.FindAll(x => !x.Category.In(JobCategory.Query,JobCategory.Research,JobCategory.Conversion,JobCategory.MarketingDesign)
		...
		private void FillGridPatternReview() {
			...
			if(tabControlNav.TabPages.Contains(tabPatternReview)) {
				...
				jobCount=0;
				foreach(KeyValuePair<JobCategory,List<Job>> kvp in dictCategories) {
					if(kvp.Key.In(JobCategory.Query,JobCategory.Conversion,JobCategory.MarketingDesign)) {
						continue;
					}
					List<Job> listJobsSorted=kvp.Value.OrderBy(x => _listJobPriorities.FirstOrDefault(y => y.DefNum==x.Priority).ItemOrder).ToList();
		...
		private void FillGridMarketing() {
			...
			long selectedJobNum=0;
			if(userControlMarketingEdit.GetJob()!=null) {
				selectedJobNum=userControlMarketingEdit.GetJob().JobNum;
			}
			...
			//Sort jobs into action dictionary
			List<Job> listJobsSorted=_listJobsAll.Where(x => x.Category==JobCategory.MarketingDesign).ToList();
			...
			foreach(KeyValuePair<JobPhase,List<Job>> kvp in dictPhases) {
				...
				foreach(Job job in kvp.Value) {
					Color backColor=Color.White;
					if(selectedJobNum>0 
						&& job.ListJobQuotes.FirstOrDefault().PatNum==userControlMarketingEdit.GetJob().ListJobQuotes.FirstOrDefault().PatNum) 
					{
						backColor=Color.LightBlue;
					}
					Def jobPriority=_listJobPriorities.FirstOrDefault(y => y.DefNum==job.Priority);
			...
			//RESELECT JOB
			if(selectedJobNum>0 && selectedJobNum==userControlMarketingEdit.GetJob().JobNum) {
		...
		private void treeJobs_DragDrop(object sender,DragEventArgs e) {
			...
			if(userControlJobEdit.IsChanged||userControlQueryEdit.IsChanged||userControlMarketingEdit.IsChanged) {
		...
		private void butRefresh_Click(object sender,EventArgs e) {
			if(userControlJobEdit.Visible) {
				...
				GoToJob(userControlMarketingEdit.GetJob().JobNum);
			}
		}
		...
		private void userControlMarketingEdit_SaveClick(object sender,EventArgs e) {
			Job jobNew=userControlMarketingEdit.GetJob();
			Job jobStale=_listJobsAll.FirstOrDefault(x=>x.JobNum==jobNew.JobNum);
			if(jobStale==null) {
				_listJobsAll.Add(jobNew);
			}
		...
		private void butQueriesRefresh_Click(object sender,EventArgs e) {
			...
			if(checkShowQueryComplete.Checked || checkShowQueryCancelled.Checked) {
				List<Job> listJobs=Jobs.GetForQueries(_listJobsAll.Select(x => x.JobNum).ToList());
				if(!listJobs.IsNullOrEmpty()) {
					Jobs.FillInMemoryLists(listJobs);
					_listJobsAll.AddRange(listJobs);
				}
		...
		private void butMarketingRefresh_Click(object sender,EventArgs e) {
			...
			if(checkShowDesignComplete.Checked || checkShowDesignCancelled.Checked) {
				List<Job> listJobs=Jobs.GetForMarketing(_listJobsAll.Select(x => x.JobNum).ToList());
				if(!listJobs.IsNullOrEmpty()) {
					Jobs.FillInMemoryLists(listJobs);
					_listJobsAll.AddRange(listJobs);
				}
		...
		private bool MarketingUnsavedChangesCheck() {
			if(userControlMarketingEdit.IsChanged) {
				switch(MessageBox.Show("Save changes to current job?","",MessageBoxButtons.YesNoCancel)) {
					case System.Windows.Forms.DialogResult.OK:
					case System.Windows.Forms.DialogResult.Yes:
		...
		private void CheckinMarketing() {
			Job jobCur=userControlMarketingEdit.GetJob();
			if(jobCur==null) {
				return;
			}
			if(jobCur.UserNumCheckout==Security.CurUser.UserNum) {
