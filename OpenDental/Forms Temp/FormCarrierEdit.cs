			...
			if(IsNew){
				...
				try{
					...
					if(carrierOld.CarrierName!=CarrierCur.CarrierName) {
						SecurityLogs.MakeLogEntry(Permissions.InsPlanChangeCarrierName,0,"Carrier name changed in Edit Carrier window from"+" "
							+carrierOld.CarrierName+" "+"to"+" "+CarrierCur.CarrierName);
					}
				}
				catch(ApplicationException ex){
					MessageBox.Show(ex.Message);
