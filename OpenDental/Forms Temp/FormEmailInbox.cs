...
namespace OpenDental {
	public partial class FormEmailInbox:ODForm {
		...
		private void FillComboEmail() {
			...
			EmailAddress emailAddressPractice=EmailAddresses.GetOne(Preferences.GetLong(PreferenceName.EmailDefaultAddressNum));
		...
		private int GetMessages() {
			...
			while(hasMoreEmail) {
				...
				try {
					...
					MessageBox.Show("Error receiving email messages"+": "+ex.Message);
				}
		...
		private void InitEmailHideInFlags() {
			...
			for(int i=0;i<_listHideInFlags.Count;i++) {
				listShowIn.Items.Add(((HideInFlags)(_listHideInFlags[i])).GetDescription());
			}
		}
		...
		private void RefreshShowIn() {
			...
			for(int i=0;i<_listHideInFlags.Count;i++) {
				listShowIn.Items[i]=_listHideInFlags[i].GetDescription();
			...
			foreach(GridRow row in ActiveGrid.SelectedRows) {
				...
				for(int i=0;i<_listHideInFlags.Count;i++) {
					if(emailMessage.HideIn.HasFlag(_listHideInFlags[i])!=emailMessageCur.HideIn.HasFlag(_listHideInFlags[i])) {
						listShowIn.Items[i]=_listHideInFlags[i].GetDescription()+" *Settings Vary";
		...
		private void butChangePat_Click(object sender,EventArgs e) {
			...
			Cursor=Cursors.Default;
			MessageBox.Show("Email messages moved successfully"+": "+messagesMovedCount);
		}
		...
		private void butSearch_Click(object sender,EventArgs e) {
			...
			Cursor=Cursors.WaitCursor;
			_isSearching=true;
			gridInbox.Title="Currently Searching - Inbox";
			gridSent.Title="Currently Searching - Sent Messages";
		...
		private void butClear_Click(object sender,EventArgs e) {
			...
			gridInbox.Title="Inbox";
			gridSent.Title="Sent Messages";
				...
				if(EmailMessages.IsSecureWebMail(emailMessage.SentOrReceived)) {
					...
					if(!String.IsNullOrEmpty(emailMessage.BodyText)) {
						if(emailMessage.BodyText.Length > 50) {
							logText+="\r\n"+"Body Text"+": "+emailMessage.BodyText.Substring(0,49)+"... ";
						}
						else {
							logText+="\r\n"+"Body Text"+": "+emailMessage.BodyText;
						}
					}
					if(emailMessage.MsgDateTime != DateTime.MinValue) {
						logText+="\r\n"+"Date"+": "+emailMessage.MsgDateTime.ToShortDateString()+". ";
					}
					SecurityLogs.MakeLogEntry(Permissions.WebMailDelete,emailMessage.PatNum,"Webmail deleted."+" "+logText);
				}
