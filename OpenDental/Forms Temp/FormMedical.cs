...
///<summary></summary>
	public class FormMedical : ODForm {
		...
		private void FormMedical_Load(object sender, System.EventArgs e){
			...
			List<EhrMeasureEvent> listDocumentedMedEvents=EhrMeasureEvents.GetByPatient(PatCur.PatNum,EhrMeasureEventType.CurrentMedsDocumented).ToList();
		...
		private void butPrint_Click(object sender,EventArgs e) {
			...
			PrinterL.TryPrintOrDebugRpPreview(pd_PrintPage,"Medications printed",auditPatNum:PatCur.PatNum);
		...
		private void pd_PrintPage(object sender,System.Drawing.Printing.PrintPageEventArgs e) {
			...
			#region printHeading
			if(!headingPrinted) {
				text="Medications List For "+PatCur.FName+" "+PatCur.LName;
				g.DrawString(text,headingFont,Brushes.Black,center-g.MeasureString(text,headingFont).Width/2,yPos);
				yPos+=(int)g.MeasureString(text,headingFont).Height;
				text="Created "+DateTime.Now.ToString();
		...
		private void butPrintMedical_Click(object sender,EventArgs e) {
			PrinterL.TryPrintOrDebugRpPreview(pd_PrintPageMedical,
				"Medications information printed",
		...
		private void pd_PrintPageMedical(object sender,System.Drawing.Printing.PrintPageEventArgs e) {
			...
			int textHeight;
			RectangleF textRect;
			text="Medical History For "+PatCur.FName+" "+PatCur.LName;
			g.DrawString(text,headingFont,Brushes.Black,center-g.MeasureString(text,headingFont).Width/2,yPos);
			yPos+=(int)g.MeasureString(text,headingFont).Height;
			...
			text="Premedicate (PAC or other): "+(checkPremed.Checked?"Y":"N");
			...
			yPos+=textHeight;
			yPos+=10;
			text="Medical History - Complete and Detailed";
			g.DrawString(text,subHeadingFont,Brushes.Black,bounds.Left,yPos);
			yPos+=(int)g.MeasureString(text,subHeadingFont).Height;
		...
		private void FillAllergies() {
			allergyList=Allergies.GetByPatient(PatCur.PatNum,checkShowInactiveAllergies.Checked).ToList();
			...
			for(int i=0;i<allergyList.Count;i++){
				...
				AllergyDef allergyDef=AllergyDefs.GetById(allergyList[i].AllergyDefId);
				row.Cells.Add(allergyDef==null ? "MISSING ALLERGY" : allergyDef.Description);
				if(!allergyList[i].AdverseReactionDate.HasValue) {
					row.Cells.Add(allergyList[i].Reaction);
				}
				else {
					row.Cells.Add(allergyList[i].Reaction+" "+allergyList[i].AdverseReactionDate.Value.ToShortDateString());
				}
				if(allergyList[i].IsActive) {
					row.Cells.Add("Active");
				}
				else {
					row.Cells.Add("Inactive");
		...
		private void gridAllergies_CellClick(object sender,ODGridClickEventArgs e) {
			...
			List<KnowledgeRequest> listKnowledgeRequests=EhrTriggers.ConvertToKnowledgeRequests(AllergyDefs.GetById(allergyList[e.Row].AllergyDefId));
			FormInfobutton FormIB=new FormInfobutton(listKnowledgeRequests);
			FormIB.PatCur=PatCur;
			FormIB.ShowDialog();
			//Nothing to do with Dialog Result yet.
		...
		private void gridAllergies_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			var allergy = allergyList[gridAllergies.GetSelectedIndex()];
			...
			{
				FormCdsIntervention.ShowIfRequired(EhrTriggers.TriggerMatch(AllergyDefs.GetById(allergy.AllergyDefId), PatCur), false);
			}
			FillAllergies();
		}
		...
		private void butAddAllergy_Click(object sender, EventArgs e)
		{
			...
			allergy.PatientId = PatCur.PatNum;

			FormAllergyEdit formA = new FormAllergyEdit(allergy);
		...
		private void FillGridAssessments() {
			...
			foreach(EhrMeasureEvent eventCur in listEvents) {
				...
				lCur=Loincs.GetByCode(eventCur.EventCode);//TobaccoUseAssessed events can be one of three types, all LOINC codes
				row.Cells.Add(lCur!=null?lCur.LongCommonName:eventCur.Type.ToString());
				sCur=Snomeds.GetByCode(eventCur.ResultCode);
		...
		private void butOK_Click(object sender, System.EventArgs e) {
			...
			if(radioMedsDocumentedYes.Checked && _EhrMeasureEventNum==0) {
				...
				ehrMeasureEventCur.EventCode="428191000124101";//SNOMEDCT code for document current meds procedure
				ehrMeasureEventCur.EventCodeSystem="SNOMEDCT";
				EhrMeasureEvents.Save(ehrMeasureEventCur);
			}
