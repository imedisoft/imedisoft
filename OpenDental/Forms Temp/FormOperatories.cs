...
namespace OpenDental{
	...
	public class FormOperatories : ODForm {
		...
		private void butCombine_Click(object sender,EventArgs e) {
			...
			if(!Preferences.GetBool(PreferenceName.ApptsAllowOverlap, true)) { //We only want to check appt overlap and return if the customer has the relevant pref turned off
		...
		private bool CanReorderOps(Operatory op1,Operatory op2,out string strErrorMsg) {
			...
			strErrorMsg="You cannot change the order of the Operatories"+" '"+op1.Abbrev+"' "+"and"+" '"+op2.Abbrev+"' "
				+"with Clinic"+" '"+comboClinic.GetSelectedAbbr()+"' "
				+"selected because it is also a member of a Headquarters Appointment View." +" "
