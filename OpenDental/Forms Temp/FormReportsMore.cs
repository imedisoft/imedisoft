...
namespace OpenDental {
	...
	public class FormReportsMore:ODForm {
		...
		private void FillLists() {
			...
			_listReportPermissions=GroupPermissions.GetPermissionsForReports().Where(x => Users.IsInUserGroup(Security.CurrentUser.Id, x.UserGroupId)).ToList();
			...
			foreach(DisplayReport report in _listProdInc) {
				if(!_listReportPermissions.Exists(x => x.ObjectId==report.DisplayReportNum)) {
		...
		private void listProdInc_MouseDown(object sender,MouseEventArgs e) {
			...
			if(!_listReportPermissions.Exists(x => x.ObjectId==_listProdInc[selected].DisplayReportNum)) {
			...
			,List<GroupPermission> listReportPermissions=null,bool doValidatePerm=true)
		{
			if(doValidatePerm) {
				if(listReportPermissions==null) {
					listReportPermissions=GroupPermissions.GetPermissionsForReports().Where(x => Users.IsInUserGroup(Security.CurrentUser.Id, x.UserGroupId)).ToList();
				}
