...
namespace OpenDental {
	...
	public partial class FormAdjMulti:ODForm {
		...
		private List<MultiAdjEntry> FillListGridEntries(CreditCalcType calc) {
			...
			PaymentEdit.ConstructResults constructResults=PaymentEdit.ConstructAndLinkChargeCredits(_patCur.PatNum,isIncomeTxfr:!radioIncludeAll.Checked);
			...
			foreach(AccountEntry accountEntry in constructResults.ListAccountCharges.FindAll(x => x.GetType()==typeof(Procedure))) {
				double remBefore=(double)accountEntry.AmountEnd;
				double remAfter=(double)accountEntry.AmountEnd;
				MultiAdjEntry multiAdjEntry=new MultiAdjEntry((Procedure)accountEntry.Tag,remBefore,remAfter);
				...
				else if(calc.In(CreditCalcType.IncludeAll,CreditCalcType.AllocatedOnly) && (!accountEntry.AmountEnd.IsZero() || isProcSelectedInAccount)) {
