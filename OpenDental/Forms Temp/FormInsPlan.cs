	...
	public class FormInsPlan : ODForm {
		...
		public FormInsPlan(InsPlan planCur,PatPlan patPlanCur,InsSub subCur){
			...
			if(CultureInfo.CurrentCulture.Name.EndsWith("CA")) {//Canadian. en-CA or fr-CA
				labelPatID.Text="Dependant Code";
				...
				labelGroupNum.Text="Plan Number";
		...
		private void InitializeComponent(){
			...
			this.comboAllowedFeeSched = new OpenDental.UI.ComboBoxPlus();
			this.labelCopayFeeSched = new System.Windows.Forms.Label();
			this.label3 = new System.Windows.Forms.Label();
			this.comboCopay = new OpenDental.UI.ComboBoxPlus();
    ...
    "t.\n5 lines in 72 vert\n6 in 85";
		...
		private void FormInsPlan_Load(object sender,System.EventArgs e) {
			...
			if(!Security.IsAuthorized(Permissions.InsPlanEdit,true)) {
				...
				labelNoPermission.Text="No Insurance Plan Edit permission.  Patient and Subscriber Information can still be saved.";
			...
			if(PatPlanCur==null) {
				...
				for(int i=0;i<Enum.GetNames(typeof(Relat)).Length;i++) {
					comboRelationship.Items.Add(Enum.GetNames(typeof(Relat))[i]);
		...
		private void FillOrtho() {
			...
			foreach(OrthoClaimType type in Enum.GetValues(typeof(OrthoClaimType))) {
				comboOrthoClaimType.Items.Add(type.GetDescription());
			...
			foreach(OrthoAutoProcFrequency type in Enum.GetValues(typeof(OrthoAutoProcFrequency))) {
				comboOrthoAutoProcPeriod.Items.Add(type.GetDescription());
		...
		private void FillFormWithPlanCur(bool isPicked) {
			...
			comboPlanType.Items.Add("Category Percentage");
			comboPlanType.Items.Add("PPO Percentage");
		...
		private void butImportTrojan_Click(object sender,System.EventArgs e) {
			...
			/*if(resetFeeSched){
				...
				comboAllowedFeeSched.Items.Clear();
				comboAllowedFeeSched.Items.Add("none");
		...
		private void butDelete_Click(object sender,System.EventArgs e) {
			...
			if(textLinkedNum.Text!="0") {//Other subscribers are present.  
				if(_subCur==null) {//viewing from big list
					...
					logText="The subscriber"+" "+Patients.GetPat(_subCur.Subscriber).GetNameFLnoPref()+" "
		...
		private void FillBenefits() {
			...
			for(int i=0;i<benefitList.Count;i++) {
				...
				if(benefitList[i].CoverageLevel==BenefitCoverageLevel.None) {
					...
					row.Cells.Add(benefitList[i].CoverageLevel.ToString());
				}
				if(benefitList[i].BenefitType==InsBenefitType.CoInsurance && benefitList[i].Percent != -1) {
					...
					row.Cells.Add("Waiting Period");
				}
				else {
					row.Cells.Add(benefitList[i].BenefitType.ToString());
				}
				...
				if(benefitList[i].TimePeriod==BenefitTimePeriod.None) {
					...
					row.Cells.Add(benefitList[i].TimePeriod.ToString());
				}
				if(benefitList[i].Quantity>0) {
					...
					{
						row.Cells.Add(benefitList[i].Quantity.ToString()+" "+"times per year"+" ");
					}
					...
					{
						row.Cells.Add(benefitList[i].Quantity.ToString()+" "+"times in the last 12 months"+" ");
					}
					else {
						row.Cells.Add(benefitList[i].Quantity.ToString()+" "
							+benefitList[i].QuantityQualifier.ToString());
					}
				}
				else {
					row.Cells.Add("");
		...
		private void gridBenefits_DoubleClick(object sender,EventArgs e) {
			if(IsNewPlan && _planCur.PlanNum != _planOld.PlanNum) {  //If adding a new plan and picked existing plan from list
				...
				MessageBox.Show("You have picked an existing insurance plan and changes cannot be made to benefits until you have saved the plan for this new subscriber."
					+"\r\n"+"To edit, click OK and then open the edit insurance plan window again.");
				return;
			}
			long patPlanNum=0;
			if(PatPlanCur!=null) {
					...
					if(formE.ShowDialog()==DialogResult.OK) {
						...
						foreach(DTP271 date in listDates) {
							...
							if(isDependentRequest || date.Segment.Get(1)!="347"){
								...
								string note="As of "+DateTime.Today.ToShortDateString()+" - "+patName+": "+dtpDescript+", "+dtpDateStr+"\n";
							...
							&& MsgBox.Show(MsgBoxButtons.YesNo,"The plan has ended.  Would you like to drop this plan?"))
						{
								...
								&& MsgBox.Show(MsgBoxButtons.YesNo,"Would you like to add a popup to collect new insurance information from patient?"))
							{
								...
								popup.IsNew=true;
								popup.Description="Insurance expired.  Collect new insurance information.";
		...
		private bool FillPlanCurFromForm(){
			...
			if(comboCopay.SelectedIndex==0){
				...
				if(MessageBox.Show(this,"The selected "+labelCopayFeeSched.Text
					+" fee schedule has been hidden. Are you sure you want to continue?","",MessageBoxButtons.YesNo)==DialogResult.No) 
				{
					return false;
				}
		...
		private void butAuditPat_Click(object sender,EventArgs e) {
			...
			FormInsEditPatLog formInsEditPatLog=new FormInsEditPatLog(PatPlanCur);
		...
		private void butOK_Click(object sender,System.EventArgs e) {
			...
			if(_planCur.CarrierNum!=_planCurOriginal.CarrierNum) {
				...
				MessageBox.Show("Error Code 1"+".  "+"Please contact support"+"\r\n"+"\r\n"+ex.Message+"\r\n"+ex.StackTrace);
				return;
			}
			...
			if(_subOld!=null) {//Editing an old plan for a subscriber.
				if(_subOld.AssignBen!=checkAssign.Checked) {
					...
					SecurityLogs.MakeLogEntry(Permissions.InsPlanChangeAssign,_subCur.Subscriber,"Assignment of Benefits (pay dentist) changed from"
				...
				MessageBox.Show("Error Code 2"+".  "+"Please contact support"+"\r\n"+"\r\n"+ex.Message+"\r\n"+ex.StackTrace);
				return;
			}
			...
			try {
				...
				MessageBox.Show("Error Code 3"+".  "+"Please contact support"+"\r\n"+"\r\n"+ex.Message+"\r\n"+ex.StackTrace);
				return;
			}
			...
			if(Security.IsAuthorized(Permissions.InsPlanEdit,true)) {
				if(_subCur==null) {//editing from big list.  No subscriber.  'pick from list' button not visible, making logic easier.
					...
					try {
						...
						MessageBox.Show("Error Code 4"+".  "+"Please contact support"+"\r\n"+"\r\n"+ex.Message+"\r\n"+ex.StackTrace);
						return;
					}
					...
					if(IsNewPlan) {
						if(InsPlans.AreEqualValue(_planCur,_planCurOriginal)) {//New plan, no changes
							...
							try {
								...
								MessageBox.Show("Error Code 5"+".  "+"Please contact support"+"\r\n"+"\r\n"+ex.Message+"\r\n"+ex.StackTrace);
								return;
							}
							...
							if(_planCur.PlanNum != _planOld.PlanNum) {//clicked 'pick from list' button, and then made changes
								...
								try {
									...
									MessageBox.Show("Error Code 6"+".  "+"Please contact support"+"\r\n"+"\r\n"+ex.Message+"\r\n"+ex.StackTrace);
									return;
								}
							}
						...
						if(InsPlans.AreEqualValue(_planCur,_planCurOriginal)) {//If no changes
							...
							try {
								...
								MessageBox.Show("Error Code 7"+".  "+"Please contact support"+"\r\n"+"\r\n"+ex.Message+"\r\n"+ex.StackTrace);
								return;
							}
							...
							if(_planCur.PlanNum != _planOld.PlanNum) {//clicked 'pick from list' button, and then made changes
								if(radioChangeAll.Checked) {
									...
									try {
										...
										MessageBox.Show("Error Code 8"+".  "+"Please contact support"+"\r\n"+"\r\n"+ex.Message+"\r\n"+ex.StackTrace);
										return;
									}
									...
									try {
										...
										MessageBox.Show("Error Code 9"+".  "+"Please contact support"+"\r\n"+"\r\n"+ex.Message+"\r\n"+ex.StackTrace);
										return;
									}
								...
								try {
									...
									MessageBox.Show("Error Code 10"+".  "+"Please contact support"+"\r\n"+"\r\n"+ex.Message+"\r\n"+ex.StackTrace);
									return;
								}
				...
				if(_subCur!=null) {
					if(IsNewPlan) {
						...
						try {
							...
							MessageBox.Show("Error Code 5a"+".  "+"Please contact support"+"\r\n"+"\r\n"+ex.Message+"\r\n"+ex.StackTrace);
							return;
						}
			...
			try {
				...
				if(_subCur!=null) {//Update SubCur if needed
					...
					if(_subOld.Subscriber!=_subCur.Subscriber) {//If the subscriber was changed, include an audit trail entry
						...
						SecurityLogs.MakeLogEntry(Permissions.InsPlanChangeSubsc,PatPlanCur.PatNum,
							"Subscriber Changed from"+" "+dictPatNames[_subOld.Subscriber]+" #"+_subOld.Subscriber+" "
							+"to"+" "+dictPatNames[_subCur.Subscriber]+" #"+_subCur.Subscriber);
					}
				...
				MessageBox.Show("Error Code 11"+".  "+"Please contact support"+"\r\n"+"\r\n"+ex.Message+"\r\n"+ex.StackTrace);
				return;
			}
			...
			try {
				...
				MessageBox.Show("Error Code 12"+".  "+"Please contact support"+"\r\n"+"\r\n"+ex.Message+"\r\n"+ex.StackTrace);
				return;
			}
			#endregion Carrier
			...
			try {
				...
				MessageBox.Show("Error Code 13"+".  "+"Please contact support"+"\r\n"+"\r\n"+ex.Message+"\r\n"+ex.StackTrace);
				return;
			}
