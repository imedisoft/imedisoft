...
namespace OpenDental
{
	public partial class FormCCDPrint : ODForm
	{
		...
		private void Init()
		{
			...
			{ //Some transactions are not patient specific.
				...
				else
				{
					...
					provBill = Providers.GetById(claim.ProvBill);
					//Claim related info
				...
				if (provTreat == null)
				{
					provTreat = Providers.GetById(Patients.GetProvNum(patient));
				}
				if (provBill == null)
				{
					provBill = Providers.GetById(Patients.GetProvNum(patient));
				}
			...
			{
				MessageBox.Show("CCD message format too short" + ": " + MessageText);
				Close();
				return;
			}
			...
			{//Version 02
			 //Since there is no FormID field in version 02, we figure out what the formId should be based on the transaction type.
				...
				{
					MessageBox.Show("Unhandled transactionCode" + " '" + transactionCode + "' " + "for version 02 message.");
					Close();
					return;
				}
			}
			...
			{ //Show the form on screen if there are no copies to print.
				...
				if (fieldPayTo != null)
				{
					...
					{//The insurance plan is set to pay the dentist
						...
						{//The carrier has decided to pay the subscriber.
							MsgBox.Show("INFORMATION: The carrier changed the payee from the dentist to the subscriber.");//This check was required for certification.
						}
					}
					else
					{//The insurance plan is set to pay the subscriber
						...
						{//The carrier has decided to pay the dentist.
							MsgBox.Show("INFORMATION: The carrier changed the payee from the subscriber to the dentist.");//This check was required for certification.
						}
					}
				}
				...
				if (paymentAdjustmentAmount != null)
				{
					...
					{
						MessageBox.Show("Payment adjustment amount" + ": " + RawMoneyStrToDisplayMoney(paymentAdjustmentAmount.valuestr));
					}
				}
				if (autoPrint)
				{
		...
		private void butOverride_Click(object sender, EventArgs e)
		{
			...
			{
				MessageBox.Show("You can only write claim amounts with either EOBs or Predetermination EOBs.");
				return;
			}
			...
			{
				MessageBox.Show("Cannot import because the claim has previously been recieved or is attached to a payment. Set the claim not recieved and try again.");
				return;
			}

			if (!formData.HasValidPaymentLines())
			{
				MessageBox.Show("Cannot import because the number of procedures reported in the response does not match the total count of procedures listed in the response.");
				return;
			}

			if (claim == null)
			{//When claim is null then claimprocs is also null.  See Init() for when claim can be null.
				MessageBox.Show("The claim associated to this form could not be found.");
				return;
			}

			if (!MsgBox.Show(MsgBoxButtons.OKCancel, "Overwrite amounts on the claim with values from this form?\r\nCannot be reversed except manually."))
			{
				return;
			}
			...
			{//Something wrong with the database.
				MessageBox.Show("No default clearinghouse set.");
				return;
			}
		...
		public void ShowDisplayMessages()
		{
			...
			if (msg.Length > 0)
			{
				...
				msgBox.Text = "Information from Insurance Carrier";
				msgBox.ShowDialog();
			}
		}
		...
		private void PrintDentaide(Graphics g)
		{
			...
			{
				text = "";
				switch (Canadian.GetRelationshipCode(patPlanSec.Relationship))
				{//Field E06
					case "1":
						text = isFrench ? "Époux(se)" : "Spouse";
						break;
			...
			{
				text = "";
				switch (patient.CanadianEligibilityCode)
				{//Field C09
		...
		private void PrintProcedureListEOB(Graphics g)
		{
			...
			for (int p = 0; p < procedureLineNumbers.Length; p++)
			{
				...
				List<Procedure> labProcs = Procedures.GetCanadianLabFees(proc.ProcNum, procListAll);
				if (labProcs.Count > 0 && eligibleLabAmounts1.Length > 0)
				...
				if (labProcs.Count > 1 && eligibleLabAmounts2.Length > 0)
				{//In version 2 the lab fee is rolled into the procedure amount and so there is no lab section.
		...
		private SizeF PrintDentistName(Graphics g, float X, float Y)
		{
			...
			text = provTreat.LastName + ", " + provTreat.FirstName + " " + provTreat.Initials + " " + provTreat.Suffix;
		...
		private SizeF PrintDentistAddress(Graphics g, float X, float Y, float maxWidthInPixels)
		{
			...
			SizeF size2 = PrintAddress(g, X + size1.Width, Y, Preferences.GetString(PreferenceName.PracticeAddress), Preferences.GetString(PreferenceName.PracticeAddress2),
				Preferences.GetString(PreferenceName.PracticeCity) + ", " + Preferences.GetString(PreferenceName.PracticeST) + " " + Preferences.GetString(PreferenceName.PracticeZip), 150f, maxWidthInPixels);
		...
		private void PrintPracticeAddress(Graphics g, float xPos)
		{
			if (clinic == null)
			{
				...
				{
					text = Patients.GetAddressFull(Preferences.GetString(PreferenceName.PracticeBillingAddress), Preferences.GetString(PreferenceName.PracticeBillingAddress2),
						Preferences.GetString(PreferenceName.PracticeBillingCity), Preferences.GetString(PreferenceName.PracticeBillingST), Preferences.GetString(PreferenceName.PracticeBillingZip));
				}
				else
				{
					text = Patients.GetAddressFull(Preferences.GetString(PreferenceName.PracticeAddress), Preferences.GetString(PreferenceName.PracticeAddress2),
						Preferences.GetString(PreferenceName.PracticeCity), Preferences.GetString(PreferenceName.PracticeST), Preferences.GetString(PreferenceName.PracticeZip));
				}
			}
			else
			{
		...
		private void PrintMissingToothList(Graphics g)
		{
			...
			if (initialPlacementUpperField != null && initialPlacementLowerField != null &&
				...
				(initialPlacementLowerField.valuestr == "Y" || initialPlacementLowerField.valuestr == "O"))
