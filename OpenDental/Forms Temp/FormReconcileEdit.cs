...
namespace OpenDental{
	...
	public class FormReconcileEdit : ODForm {
		...
		private void FillGrid() {
			...
			gridMain.ListGridColumns.Add(col);
			col=new GridColumn("Deposits",70,HorizontalAlignment.Right);
			gridMain.ListGridColumns.Add(col);
			col=new GridColumn("Withdrawals",75,HorizontalAlignment.Right);
			gridMain.ListGridColumns.Add(col);
		...
		private MigraDoc.DocumentObjectModel.Document CreatePrintDocument(PrintDocument pd) {
			...
			pageNumParag.AddText("Page"+" ");
			pageNumParag.AddPageField();
			pageNumParag.AddText(" "+"of"+" ");
			...
			par.AddFormattedText("RECONCILE",totalFontx);
			par.AddLineBreak();
			...
			par.AddLineBreak();
			par.AddText("Reconcile Date"+": "+PIn.Date(textDate.Text).ToShortDateString());
			par.AddLineBreak();
