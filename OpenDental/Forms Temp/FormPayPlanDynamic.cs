...
namespace OpenDental {
	...
	public partial class FormPayPlanDynamic : ODForm {
		...
		private void ButAddProcedure_Click(object sender,EventArgs e) {
			...
			AddProcedures(formProcSelect.ListSelectedProcs);
		}
		...
		private void AddProcedures(List<Procedure> listProcs) {
			...
			List<PaySplit> listSplits=PaySplits.GetPaySplitsFromProcs(listProcNums);
			foreach(Procedure proc in listProcs) {
				...
				_listPayPlanProductionEntries.Add(new PayPlanProductionEntry(proc,creditAdding,listClaimProcs,listAdjustments,listSplits));
			}
			textTotalPrincipal.Text=_sumAttachedProduction.ToString("f");
			FillProduction();
			if(countSkipped>0) {
		...
		private void ButPrintProduction_Click(object sender,EventArgs e) {
			PrinterL.TryPrintOrDebugRpPreview(pd_PrintPage,"Attached PayPlan Production printed",PrintoutOrientation.Landscape);
		}
		...
		private void pd_PrintPage(object sender,PrintPageEventArgs e) {
			...
			if(yPos==-1) {
				...
				text="Total"+": "+_sumAttachedProduction.ToString("c");
		...
		private void butCloseOut_Click(object sender,EventArgs e) {
			...
			foreach(PayPlanProductionEntry entry in listProductionEntries) {
				...
				PayPlanCharge charge=PayPlanEdit.CreateDebitChargeDynamic(_payPlanCur,_famCur,entry.ProvNum,entry.ClinicNum,(double)entry.AmountRemaining,0
					,DateTime.Today,"Close Out Charge",entry.PriKey,entry.LinkType);
		...
		private void SetNote() {
			textNote.Text=_payPlanCur.Note+DateTime.Today.ToShortDateString()
				...
				+", "+"Total Amount"+": "+textTotalPrincipal.Text
				+", "+"APR"+": "+textAPR.Text
