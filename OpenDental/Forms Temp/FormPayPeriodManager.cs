...
namespace OpenDental
{
	...
	public class FormPayPeriodManager : ODForm
	{
		...
		private void FormPayPeriodManager_Load(object sender, System.EventArgs e)
		{
			...
			PayPeriodInterval payPeriodInterval = (PayPeriodInterval)PrefC.GetInt(PreferenceName.PayPeriodIntervalSetting);
			...
			int dayOfWeek = PrefC.GetInt(PreferenceName.PayPeriodPayDay);
			if (dayOfWeek != 0)
			...
			else
			{
				...
				numDaysAfterPayPeriod.Text = Preferences.GetString(PreferenceName.PayPeriodPayAfterNumberOfDays);
				checkExcludeWeekends.Checked = Preferences.GetBool(PreferenceName.PayPeriodPayDateExcludesWeekends);
				if (checkExcludeWeekends.Checked)
				{
					if (Preferences.GetBool(PreferenceName.PayPeriodPayDateBeforeWeekend))
					{
		...
		private void butOK_Click(object sender, EventArgs e)
		{
			...
			//Save payperiods
			foreach (PayPeriod payPeriod in _listPayPeriods)
			...
			{
				Preferences.Set(PreferenceName.PayPeriodIntervalSetting, (int)PayPeriodInterval.Weekly);
			}
			else if (radioBiWeekly.Checked)
			{
				Preferences.Set(PreferenceName.PayPeriodIntervalSetting, (int)PayPeriodInterval.BiWeekly);
			}
			else
			{
				Preferences.Set(PreferenceName.PayPeriodIntervalSetting, (int)PayPeriodInterval.Monthly);
			}
			Preferences.Set(PreferenceName.PayPeriodPayDay, comboDay.SelectedIndex);
			Preferences.Set(PreferenceName.PayPeriodPayAfterNumberOfDays, PIn.Int(numDaysAfterPayPeriod.Text));
			Preferences.Set(PreferenceName.PayPeriodPayDateExcludesWeekends, checkExcludeWeekends.Checked);
			if (radioPayBefore.Checked)
			{
				Preferences.Set(PreferenceName.PayPeriodPayDateBeforeWeekend, true);
			}
			else if (radioPayAfter.Checked)
			{
				Preferences.Set(PreferenceName.PayPeriodPayDateBeforeWeekend, false);
			}
