...
///<summary></summary>
	public class FormProviderSetup:ODForm {
		...
		private void FormProviderSetup_Load(object sender, System.EventArgs e) {
			...
			checkShowHidden.Checked=Preferences.GetBool(PreferenceName.EasyHideDentalSchools);
		...
		private void FillGrid(bool needsRefresh=true) {
			...
			if(!Preferences.GetBool(PreferenceName.EasyHideDentalSchools)) {
				gridMain.Columns.Add(new GridColumn("ProvNum",60,GridSortingStrategy.AmountParse));
			}
			...
			if(!Preferences.GetBool(PreferenceName.EasyHideDentalSchools)) {
				gridMain.Columns.Add(new GridColumn("Class",90));
				gridMain.Columns.Add(new GridColumn("Instructor",60,HorizontalAlignment.Center));
			}
			if(checkShowPatientCount.Checked) {
				gridMain.Columns.Add(new GridColumn("PriPats",50,HorizontalAlignment.Center,GridSortingStrategy.AmountParse));
				gridMain.Columns.Add(new GridColumn("SecPats",50,HorizontalAlignment.Center,GridSortingStrategy.AmountParse));
			}
			gridMain.Rows.Clear();
			GridRow row;
		...
		private void butAdd_Click(object sender, System.EventArgs e) {
			...
			if(!radioStudents.Checked) {
				if(radioInstructors.Checked && Preferences.GetLong(PreferenceName.SecurityGroupForInstructors)==0) {
				...
				if(radioStudents.Checked && Preferences.GetLong(PreferenceName.SecurityGroupForStudents)==0) {
		...
		private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			...
			if(!Preferences.GetBool(PreferenceName.EasyHideDentalSchools) && Providers.IsAttachedToUser(provSelected.Id)) {//Dental schools is turned on and the provider selected is attached to a user.
		...
		private void butMovePri_Click(object sender,EventArgs e) {
			...
			ODProgress.ShowAction(() => {
				...
				startingMessage:"Gathering patient data"+"...");
		...
		private void butMoveSec_Click(object sender,EventArgs e) {
			...
			if(provTo==null) {
				...
				msg="Move all secondary patients to"+" "+provTo.Abbr+" "+"from the following providers"+": "+strProvsFrom+"?";
			}
			if(MessageBox.Show(msg,"",MessageBoxButtons.OKCancel)!=DialogResult.OK) {
				return;
			}
		...
		private void butReassign_Click(object sender,EventArgs e) {
			...
			actionCloseProgress?.Invoke();
			Cursor=Cursors.Default;
			string msg="You are about to reassign"+" "+listPatProvFrom.Count+" "+"patients to different providers.  Continue?";
			if(MessageBox.Show(msg,"",MessageBoxButtons.OKCancel)!=DialogResult.OK) {
				return;
			}
			...
			ODProgress.ShowAction(() => {
					...
					List<Action> listActions=listPatProvFrom.GroupBy(x=>new { From=x.ProvNum,To=dictPatProvTo[x.PatNum] })
						...
						.Select(x => new Action(() => {
							...
							PatientEvent.Fire(EventCategory.Patient,"Reassigning patients"+": "+patsReassigned
								+" "+"out of"+" "+listPatProvFrom.Count);
		...
		private void butCreateUsers_Click(object sender,EventArgs e) {
			...
			for(int i=0;i<gridMain.SelectedIndices.Length;i++){
				if(Providers.IsAttachedToUser(((Provider)gridMain.Rows[gridMain.SelectedIndices[0]].Tag).Id)) {
		...
		private void FormProviderSelect_Closing(object sender, System.ComponentModel.CancelEventArgs e) {
			...
			if(duplicates!="" && Preferences.GetBool(PreferenceName.EasyHideDentalSchools)) {
				if(MessageBox.Show("Warning.  The following abbreviations are duplicates.  Continue anyway?\r\n"+duplicates,
