	...
	public partial class FormPaySimple:ODForm {
		...
		private void FormPaySimple_Load(object sender,EventArgs e) {
			...
			if(_patCur==null || _patCur.PatNum==0) {//Prepaid card
				...
				checkOneTimePayment.Checked=!Preferences.GetBool(PreferenceName.StoreCCtokens);
				checkOneTimePaymentACH.Checked=!Preferences.GetBool(PreferenceName.StoreCCtokens);
				...
				switch(_trantype) {
					...
					case PaySimple.TransType.AUTH:
						...
						if(paySimpleCustomerId==0) {//Insurance payment, make a new customer id every time per Nathan on 04/26/2018
							if((_patCur==null || _patCur.PatNum==0)) {
								...
								throw new ODException("Invalid PaySimple Customer Id found.");
							}
						}
						try {
					...
					case PaySimple.TransType.RETURN:
						if(string.IsNullOrWhiteSpace(textRefNumber.Text)) {
							throw new ODException("Invalid PaySimple Payment ID.");
						}
						if(!MsgBox.Show(MsgBoxButtons.OKCancel,"You are about to return a payment.  This action is irreversible.  Continue?")) {
							throw new ODException("Payment return was cancelled by user.");
						}
					...
					case PaySimple.TransType.VOID:
						...
						if(!MsgBox.Show(MsgBoxButtons.OKCancel,"You are about to void a payment.  This action is irreversible.  Continue?")) {
							throw new ODException("Payment void was cancelled by user.");
						}
		...
		private PaySimple.ApiResponse ProcessPaymentACH() {
			...
			try {
				...
				FriendlyException.Show("Error:"+" "+ex.Message,ex);
				return null;
			}
			if(!_isAddingCard) {
		...
		private void UpsertCreditCard(PaySimple.ApiResponse apiResponse,string ccNumberMasked,CreditCardSource ccSource,DateTime ccExp) {
			...
			if(_creditCardCur.IsNew) {
				...
				_creditCardCur.Procedures=Preferences.GetString(PreferenceName.DefaultCCProcs);
