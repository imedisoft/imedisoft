...
///<summary></summary>
	public class FormProcGroup:ODForm {
		...
		private void FormProcGroup_Load(object sender, System.EventArgs e){
			...
			if(GroupCur.ProcStatus==ProcStat.EC && Prefs.GetBool(PrefName.ProcLockingIsAllowed) && !GroupCur.IsLocked) {
				butLock.Visible=true;
			}
			else {
				butLock.Visible=false;
			}
			Permissions perm=Permissions.ProcCompleteEdit;//Not sure if this is enough yet.
			...
			if(GroupCur.IsLocked) {//Whether locking is currently allowed, this proc group may have been locked previously.
				...
				if(_attachedToCompletedProc) {
					...
					if(!Security.IsAuthorized(Permissions.ProcCompleteNote,GroupCur.ProcDate)) {
						butOK.Enabled=false;
						butDelete.Enabled=false;
