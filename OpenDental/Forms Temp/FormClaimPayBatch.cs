...
///<summary></summary>
	public class FormClaimPayBatch:ODForm {
		...
		private void InitializeComponent(){
			...
			// butAttach
			// 
			this.butAttach.Image = global::Imedisoft.Properties.Resources.up;
			...
			// butDetach
			// 
			this.butDetach.Image = global::Imedisoft.Properties.Resources.down;
		...
		private void FormClaimPayEdit_Load(object sender, System.EventArgs e) {
			...
			if(EobAttaches.Exists(ClaimPaymentCur.ClaimPaymentNum)) {
				textEobIsScanned.Text="Yes";
				butView.Text="View EOB";
			}
			else {
				textEobIsScanned.Text="No";
		...
		private void butOK_Click(object sender,EventArgs e) {
			...
			if(!Preferences.GetBool(PreferenceName.AllowFutureInsPayments)
		...
		private void FormClaimPayBatch_FormClosing(object sender,FormClosingEventArgs e) {
			...
			if(ClaimPaymentCur.IsPartial) {
				if(IsAmountAndTotalEqual(isSilent:true)) {
					if(gridAttached.Rows.Count > 0
						&& !Preferences.GetBool(PreferenceName.PromptForSecondaryClaim))
					{
