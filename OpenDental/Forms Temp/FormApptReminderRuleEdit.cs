...
namespace OpenDental {
	...
	public partial class FormApptReminderRuleEdit:ODForm {
		...
		private string[] _arrLanguages {
			get {
				return Preferences.GetString(PreferenceName.LanguagesUsedByPatients).Split(',');
			}
		}
		...
		private void FormApptReminderRuleEdit_Load(object sender,EventArgs e) {
			...
			if(_arrLanguages.Length==0 
				|| (_arrLanguages.Length==1 && _arrLanguages[0].ToLower().Trim()==Preferences.GetString(PreferenceName.LanguagesIndicateNone).ToLower().Trim())) 
			{
		...
		private void CheckMultipleEConfirms() {
			...
			string confStatusEConfirmSent=Definitions.GetDef(DefinitionCategory.ApptConfirmed,Preferences.GetLong(PreferenceName.ApptEConfirmStatusSent)).Name;
			List<string> listExclude=Preferences.GetString(PreferenceName.ApptConfirmExcludeESend)
			...
			if(ApptReminderRuleCur.TypeCur==ApptReminderType.ConfirmationFutureDay
				...
				&& listExclude.Contains(Preferences.GetString(PreferenceName.ApptEConfirmStatusSent))
			...
			{
				listExclude.RemoveAll(x => x==Preferences.GetString(PreferenceName.ApptEConfirmStatusSent));
				IsPrefsChanged|=Preferences.Set(PreferenceName.ApptConfirmExcludeESend,string.Join(",",listExclude));
			}
		}
		...
		private void butLanguage_Click(object sender,EventArgs e) {
			...
			foreach(string language in _arrLanguages) {
				if(language.IsNullOrEmpty() || language.ToLower().Trim()==Preferences.GetString(PreferenceName.LanguagesIndicateNone).ToLower().Trim()) {
					continue;
				}
