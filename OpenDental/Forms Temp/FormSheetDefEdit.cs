...
namespace OpenDental {
	public partial class FormSheetDefEdit:ODForm {
		...
		///<summary>The currently selected languages three letter abbr from comboLanguages.
		///When 'Default' is selected an empty string is returned.
		///When 'Add New' is selected NULL is returned.</summary>
		private string _selectedLanguageThreeLetters {
			get {
		...
		///<summary>The currently selected languages display string from comboLanguages.
		///When 'Default' is selected an empty string is returned.
		///When 'Add New' is selected NULL is returned.</summary>
		private string _selectedLanguageDisplay {
			get {
				if(comboLanguages.SelectedIndex<0) {
					//Selection is forced to 1 in InitTranslations(...). Only not set for internal sheet defs or dynamic layout defs.
					return "";
		...
		///<summary>Returns a list of all translations that are currently being used in memory.
		///If user selects a translation but it isn't in the DB yet it will be included in this list.</summary>
		private List<string> _listUsedTranslations {
			get {
		...
		///The initial layout will never be a treatment plan layout because the Treatment Plan checkbox is always disabled in Chart initially.
		///Returns true if current _sheetDefCur.SheetTYpe is associated to a dynamic layout SheetType, otherwise false.</summary>
		...
		///<summary>
		///<para>Refreshes local _listUsedLanguages and _listUnusedLanguages data.</para>
		...
		public void RefreshLanguages(string selectedThreeLetterLanguage=null) {
			...
			_listUnusedLanguages=listAllLanguages.FindAll(x => !listUsedLanguageThreeLetters.Contains(x))
				.Select(x => new SheetDefLanguage(x,(MiscUtils.GetCultureFromThreeLetter(x)?.DisplayName??x)))
				.OrderBy(x => x.Display).ToList();
			if(selectedThreeLetterLanguage!=null && selectedThreeLetterLanguage.In(_listUnusedLanguages.Select(x => x.ThreeLetters))) {
		...
		private void comboLanguages_SelectionChangeCommitted(object sender,EventArgs e) {
			string selectedThreeLetterLanguage=_selectedLanguageThreeLetters;
		...
		private bool TryAddTranslation(out string selectedLangauge) {
			...
			InputBox formIB=new InputBox(listParams);
		...
		private void FillFieldList() {
			...
			string selectedLanguage=_selectedLanguageThreeLetters;
			...
			sheetEditMobile.UpdateLanguage(selectedLanguage);//This must be called before sheetEditMobile.SheetDef
		...
		private void DrawFields(SheetDef sheetDef,Graphics g,bool onlyDrawImages,bool isHighlightEligible=true) {
			...
			string selectedLangauge=_selectedLanguageThreeLetters;
		...
		private void DrawGridHelper(SheetFieldDef sheetFieldDef,Graphics g,bool isSelected) {
			...
			switch(sheetFieldDef.FieldName) {
				...
				case "EraClaimsPaid":
					...
					yPosGrid+=heightGridTitle;
					/*---------------------------------------------------------------------------------------------*/
					odGrid.SheetDrawHeader(g,sheetFieldDef.XPos,yPosGrid);
					yPosGrid+=heightGridHeader;
					odGrid.SheetDrawRow(0,g,sheetFieldDef.XPos,yPosGrid,false,true); //a single dummy row.
				...
				case "Procedures":
					...
					odGrid.SheetDrawTitle(g,sheetFieldDef.XPos,yPosGrid);
					yPosGrid+=heightGridTitle;
					...
					while(gridRemainingHeight>gridRowHeight) {
						odGrid.SheetDrawRow(0,g,sheetFieldDef.XPos,yPosGrid,(gridRemainingHeight-gridRowHeight <= gridRowHeight),true);//A single dummy row.
		...
		///<summary>Returns true if both SheetFieldDefs have matching FieldNames, FieldValues and Bounds, otherwise false.</summary>
		private bool IsEquivilantSheetDef(SheetFieldDef defOne,SheetFieldDef defTwo){
			return (defOne.FieldName==defTwo.FieldName 
				&& defOne.FieldValue==defTwo.FieldValue 
		...
		private void AddNewSheeFieldDef(SheetFieldDef sheetFieldDef) {
			...
			if(sheetFieldDef.Language.IsNullOrEmpty()){
				...
				sheetFieldDef.Language=_selectedLanguageThreeLetters;
			}
			...
			if(doAddToTranslatedViews){
				AddNewSheetFieldDefsForTranslations(sheetFieldDef,_listUsedTranslations.ToArray());
			}
			sheetEditMobile.ScrollToSheetFieldDefNum=sheetFieldDef.SheetFieldDefNum;
		}

		///<summary>
		///<para>Creates copies of given defDefault for every given value in listLanguages and inserts them into _sheetDefCur.SheetFieldDefs via AddNewSheeFieldDef(...)</para>
		///<para>Either called when user selects a new language to translate to or when the user adds a SheetFieldDef to the 'Default' view.</para>
		///</summary>
		...
		private List<SheetFieldDef> GetPertinentSheetFieldDefs(SheetDef sheetDef=null) {
			...
			return sheetDef.SheetFieldDefs.FindAll(x => x.LayoutMode==_sheetLayoutModeCur && x.Language==_selectedLanguageThreeLetters);
		}
		...
		private void LaunchEditWindow(SheetFieldDef field,bool isEditMobile) {
			...
			SheetFieldDef defCopy=field.Copy();//Used for comparison below.
			...
			if(FormS!=null) { //in case MobileHeader was used instead
				...
				if(FormS.SheetFieldDefCur==null) {
					...
					RemoveEquivilantTranslatedSheetDefs(defCopy);//Remove all SheetFieldDefs that were not change for translated fields.
				}
				else if(HasTranslatedSheetDefS(defCopy,out List<SheetFieldDef> listMatchedDefs)) {//def was not deleted.
					//Update unchanged, translated SheetFieldDefs to reflect new values.
					for(int i=0;i<listMatchedDefs.Count;i++){
						SheetFieldDef def=listMatchedDefs[i];
						string language=def.Language;
						def=FormS.SheetFieldDefCur.Copy();
			...
			listFields.SetSelectedItem<SheetFieldDef>((sheetFieldDef) => sheetFieldDef==field);//If selection is valid this will trigger panelMain.Refresh()
		...
		private void panelMain_MouseMove(object sender,MouseEventArgs e) {
			...
			for(int i=0;i<listFields.SelectedIndices.Count;i++) {
				...
				Point originalLocaiton=OriginalControlPositions[i];
				int newX=originalLocaiton.X+e.X-MouseOriginalPos.X;
				int newY=originalLocaiton.Y+e.Y-MouseOriginalPos.Y;
		...
		private void butDelete_Click(object sender,EventArgs e) {
			...
			if(!_selectedLanguageThreeLetters.IsNullOrEmpty()){//User clicked 'Delete' while viewing a translated view.
				string msg=$"You are about to delete the entire {_selectedLanguageDisplay} language translation for this sheet.\n"
				...
				if(MsgBox.Show(MsgBoxButtons.YesNo,msg)){
					_sheetDefCur.SheetFieldDefs.RemoveAll(x => x.Language==_selectedLanguageThreeLetters);//Remove all translated SheetFieldDefs for current language.
		...
		private void butAlignLeft_Click(object sender,EventArgs e) {
			...
			if(listFields.GetListSelected<SheetFieldDef>().Any(x => x.FieldType==SheetFieldType.Image)) {
				RefreshDoubleBuffer();
			}
			panelMain.Refresh();
		}
