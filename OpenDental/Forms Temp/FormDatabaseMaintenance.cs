...
namespace OpenDental {
	...
	public class FormDatabaseMaintenance:ODForm {
		...
		private Label labelRedundantIndexes;
		private UI.Button butRedundantIndexes;
    ...
    "ttachments.";
		...
		private void butOptimize_Click(object sender,EventArgs e) {
			if(MessageBox.Show("This tool will backup, optimize, and repair all tables."+"\r\n"+"Continue?"
				,"Backup Optimize Repair"
			...
			if(Shared.BackupRepairAndOptimize(true,BackupLocation.OptimizeTool)) {
				result=DateTime.Now.ToString()+"\r\n"+"Repair and Optimization Complete";
			}
			else {
				result=DateTime.Now.ToString()+"\r\n";
				result+="Backup, repair, or optimize has failed.  Your database has not been altered."+"\r\n";
				result+="Please call support for help, a manual backup of your data must be made before trying to fix your database."+"\r\n";
			}
		...
		private void butRemoveNulls_Click(object sender,EventArgs e) {
			...
			MessageBox.Show("Number of null strings replaced with empty strings"+": "+DatabaseMaintenances.MySqlRemoveNullStrings());
		...
		private void butRawEmails_Click(object sender,EventArgs e) {
			...
			ODProgress.ShowAction(() => results=DatabaseMaintenances.CleanUpRawEmails(),
				actionException:ex => {
					results="There was an error cleaning up email bloat:"+"\r\n"+ex.Message;
				},
		...
		private void butEmailAttaches_Click(object sender,EventArgs e) {
			...
			ODProgress.ShowAction(() => results=DatabaseMaintenances.CleanUpAttachmentsRootDirectiory(),
				actionException: ex => {
					results="There was an error cleaning up email attachments:"+"\r\n"+ex.Message;
				},
				eventType: typeof(DatabaseMaintEvent),
				odEventType: EventCategory.DatabaseMaint);
		...
		private void butPayPlanPayments_Click(object sender,EventArgs e) {
			...
			msgBoxCopyPaste.Text="Payments Fixed";
			msgBoxCopyPaste.Show();
		}
		...
		private void butRedundantIndexes_Click(object sender,EventArgs e) {
			...
			FormRedundantIndexes formRI=new FormRedundantIndexes();
			formRI.ShowDialog();			
		}
		...
		private void Run(ODGrid grid,DbmMode modeCur) {
            ...
            (string, bool) tableCheckResult = default;
			ODProgress.ShowAction(() => tableCheckResult=DatabaseMaintenances.MySQLTables(verbose),
				eventType:typeof(DatabaseMaintEvent),
				odEventType:EventCategory.DatabaseMaint);
			...
			_threadRunDBM.AddExceptionHandler(ex => this.InvokeIfRequired(() => {
				...
				if(selectedIndex>=0) {
					UpdateResultTextForRow(grid,selectedIndex,"ERROR: "+ex.Message);
				}
				logText.Append("ERROR: "+ex.Message);
				ToggleUI(false);
				grid.SetSelected(selectedIndices,true);//Reselect all rows that were originally selected.
			}));
		...
		private void butPrint_Click(object sender,EventArgs e) {
			...
			PrinterL.TryPrintOrDebugClassicPreview(pd2_PrintPage,"Database Maintenance log printed",new Margins(40,50,50,60),0);
		}
