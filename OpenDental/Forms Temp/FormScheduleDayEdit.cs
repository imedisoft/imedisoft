...
namespace OpenDental{
	...
	public class FormScheduleDayEdit:ODForm {
		...
		private void comboClinicChanged(){
			...
			this.Text="Edit Day"+" - "+comboClinic.Text;
			if(comboClinic.SelectedClinicNum==0) {
				groupPractice.Text="Practice";
			}
			else {
				groupPractice.Text="Clinic";
			}
			if(_listScheds!=null) {
				//skips on startup
			...
			_listScheds=Schedules.RefreshDayEditForPracticeProvsEmps(_dateSched,_listProvs.Select(x => x.Id).Where(x => x>0).ToList(),
				_listEmps.Select(x => x.Id).Where(x => x>0).ToList(),comboClinic.SelectedClinicNum);
		...
		private void FillProvsAndEmps() {
			...
			//Prov Listbox
			List<long> listPreviouslySelectedProvNums=listProv.GetListSelected<Provider>().Select(x => x.Id).ToList();
			...
			//Emp Listbox
			List<long> listPreviouslySelectedEmpNums=listEmp.GetListSelected<Employee>().Select(x => x.Id).ToList();
			listEmp.Items.Clear();
		...
		private void listProv_SelectedIndexChanged(object sender,EventArgs e) {
			tabPageProv.Text="Providers"+" ("+listProv.SelectedIndices.OfType<int>().Count(x => x>0)+")";
		}

		private void listEmp_SelectedIndexChanged(object sender,EventArgs e) {
			tabPageEmp.Text="Employees"+" ("+listEmp.SelectedIndices.OfType<int>().Count(x => x>0)+")";
		}
		...
		private void FillGrid() {
			...
			_dictProvNumProvider=_listScheds.Select(x => x.ProvNum).Distinct().Select(x => Providers.GetById(x))//returns null if ProvNum==0 or invalid
				.Where(x => x!=null).ToDictionary(x => x.Id);//speed up sort.
			...
			foreach(Schedule schedCur in _listScheds) {
				...
				if(schedCur.EmployeeNum==0) {
					...
					row.Cells.Add(Employees.GetEmp(schedCur.EmployeeNum).FirstName);
				}
				//times
				...
				if(schedCur.SchedType==ScheduleType.Practice) {//note or holiday
					...
					if(PrefC.HasClinicsEnabled) {
						...
						if(schedCur.Status!=SchedStatus.Holiday) {//must be a Note, only add 'Note' if clinics are enabled
							note="Note"+clinicAbbr+": ";
						}
					}
					if(schedCur.Status==SchedStatus.Holiday) {
						note="Holiday"+clinicAbbr+": ";
					}
				}
				note+=schedCur.Note;
				row.Cells.Add(note);
		...
		private bool ValidateLists() {
			...
			if(listProv.SelectedIndices.OfType<int>().Count(x => x>0)==0 && listEmp.SelectedIndices.OfType<int>().Count(x => x>0)==0) {
				listErrorMsgs.Add("Please select at least one provider or one employee first.");
			}
      ...
      FillGrid();
		...
		private void butDelete_Click(object sender,EventArgs e) {
			...
			if(countProviders>0) {
				string message="Delete schedules on this day for"+" "+countProviders+" "+"provider(s)?";
