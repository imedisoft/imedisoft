...
namespace OpenDental {
	public partial class FormProcCodeEditMore:ODForm {
		...
		private void FillAndSortListFees() {
			List<long> listClinicNums=Clinics.GetByCurrentUser().Select(x => x.Id).ToList();
		...
		private void FillGrid() {
			...
			if(!PrefC.HasClinicsEnabled) {
				col=new GridColumn("Schedule",200);
				gridMain.ListGridColumns.Add(col);
				col=new GridColumn("Provider",135);
				gridMain.ListGridColumns.Add(col);
			}
			else {//Using clinics.
				col=new GridColumn("Schedule",130);
				gridMain.ListGridColumns.Add(col);
				col=new GridColumn("Clinic",130);
				gridMain.ListGridColumns.Add(col);
				col=new GridColumn("Provider",75);
				gridMain.ListGridColumns.Add(col);
			}
			col=new GridColumn("Amount",100,HorizontalAlignment.Right);
			gridMain.ListGridColumns.Add(col);
