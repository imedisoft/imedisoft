...
namespace OpenDental
{
	public partial class FormEhrLabOrderEdit2014 : ODForm
	{
		...
		private void FillGrid()
		{
			...
			GridColumn col;
			if (CdsPermissions.GetByUser(Security.CurrentUser.Id).ShowInfoButton)
		...
		private void butServicePicker_Click(object sender, EventArgs e)
		{
			...
			textUsiID.Text = FormL.SelectedLoinc.Code;
			textUsiCodeSystemName.Text = "LN";
			textUsiText.Text = FormL.SelectedLoinc.ShortName;
			textUsiTextOriginal.Text = FormL.SelectedLoinc.LongCommonName;
		}
		...
		private void butOk_Click(object sender, EventArgs e)
		{
			...
			if (Security.CurrentUser.ProviderId != 0 && EhrProviderKeys.GetByProviderName(prov.LastName, prov.FirstName).Count() > 0)
			...
			*/
			...
			for (int i = 0; i < EhrLabCur.ListEhrLabResults.Count; i++)
			{
				...
				{
					FormCdsIntervention.ShowIfRequired(EhrTriggers.TriggerMatch(EhrLabCur.ListEhrLabResults[i], patCur), false);
				}
			}
			DialogResult = DialogResult.OK;
		}
