...
namespace OpenDental
{
	public partial class FormVitalsignEdit2014 : ODForm
	{
		...
		private void FormVitalsignEdit2014_Load(object sender, EventArgs e)
		{
			...
			{
				if (listHeightCodes[i].ShortName == "" || listHeightCodes[i].LongCommonName.Length < 30)
				{//30 is roughly the number of characters that will fit in the combo box
					comboHeightExamCode.Items.Add(listHeightCodes[i].LongCommonName);
				}
				else
				{
					comboHeightExamCode.Items.Add(listHeightCodes[i].ShortName);
				}
				if (i == 0 || VitalsignCur.HeightExamCode == listHeightCodes[i].Code)
				{
			...
			{
				if (listWeightCodes[i].ShortName == "" || listWeightCodes[i].LongCommonName.Length < 30)
				{//30 is roughly the number of characters that will fit in the combo box
					comboWeightExamCode.Items.Add(listWeightCodes[i].LongCommonName);
				}
				else
				{
					comboWeightExamCode.Items.Add(listWeightCodes[i].ShortName);
				}
				if (i == 0 || VitalsignCur.WeightExamCode == listWeightCodes[i].Code)
				{
		...
		private void SetPregCodeAndDescript()
		{
			...
			{//not currently attached to a disease
				...
				{//only try to find existing problem if a valid exam date is entered before checking the box, otherwise we do not know what date to compare to the active dates of the pregnancy dx
					...
					{//loop through all diseases for this patient, shouldn't be very many
						...
						{//loop through preg disease defs in the db, shouldn't be very many
							if (listPatDiseases[i].ProblemDefId != listPregDisDefs[j].Id)
				...
				{//we are going to insert either the default pregnancy problem or a manually selected problem
					...
					pregCode = Preferences.GetString(PreferenceName.PregnancyDefaultCodeValue);//could be 'none' which disables the automatic dx insertion
					string pregCodeSys = Preferences.GetString(PreferenceName.PregnancyDefaultCodeSystem);//if 'none' for code, code system will default to 'SNOMEDCT', display will be ""
		...
		private void textDateTaken_Leave(object sender, EventArgs e)
		{
			...
			{//the currently attached preg disease is valid, will be null if they checked the box on new exam but haven't hit ok to save
				if (examDate.Date < disCur.DateStart || (disCur.DateStop.HasValue && disCur.DateStop < examDate.Date))
...
Do you want to remove the pregnancy diagnosis?"))
		...
		private void butOK_Click(object sender, EventArgs e)
		{
			...
			{
				VitalsignCur.HeightExamCode = listHeightCodes[comboHeightExamCode.SelectedIndex - 1].Code;
			}
			if (comboWeightExamCode.SelectedIndex > 0)
			{
				VitalsignCur.WeightExamCode = listWeightCodes[comboWeightExamCode.SelectedIndex - 1].Code;
			}
			...
			if (CdsPermissions.GetByUser(Security.CurrentUser.Id).ShowCDS && CdsPermissions.GetByUser(Security.CurrentUser.Id).VitalCDS)
			{
				FormCdsIntervention.ShowIfRequired(EhrTriggers.TriggerMatch(VitalsignCur, Patients.GetPat(VitalsignCur.PatNum)), false);
			}
			#endregion
