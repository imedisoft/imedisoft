    ...
    "the insurance EOB.";
		...
		private void FormEtrans835ClaimPay_Load(object sender, System.EventArgs e) {
			...
			if(_insPlan!=null && _insPlan.PlanType.In("","f")) {
				checkIncludeWOPercCoPay.Checked=Preferences.GetBool(PreferenceName.EraIncludeWOPercCoPay);
			}
			else {
		...
		private void FillGridProcedures(){
			...
			for(int i=0;i<ListClaimProcsForClaim.Count;i++){
				...
				if(claimProc.ProcNum==0) {
					...
					row.Cells.Add(ProcedureCodes.GetById(ProcCur.CodeNum).Code);
					row.Cells.Add(Tooth.ToInternat(ProcCur.ToothNum));
					row.Cells.Add(ProcedureCodes.GetById(ProcCur.CodeNum).Description);
				}
		...
		private void butOK_Click(object sender,System.EventArgs e) {
			...
			if(!Preferences.GetBool(PreferenceName.EraAllowTotalPayments) && gridPayments.Rows.Select(x => (x.Tag as ClaimProc)).Any(x => x.ProcNum==0 && x.InsPayAmt>0)){
