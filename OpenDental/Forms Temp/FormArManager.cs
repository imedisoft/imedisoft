...
namespace OpenDental
{
    public partial class FormArManager:ODForm {
		...
		private void FormArManager_Load(object sender,EventArgs e) {
			...
			comboUnsentAccountAge.Items.Add("Any Balance");
			comboUnsentAccountAge.Items.Add("Over 30 Days");
			...
			comboExcludedAccountAge.Items.Add("Any Balance");
			comboExcludedAccountAge.Items.Add("Over 30 Days");
			...
			List<TsiTransType> listDefaultLastTransTypes=Prefs.GetString(PrefName.ArManagerLastTransTypes)
				...
				.Where(x => !x.In(TsiTransType.PF,TsiTransType.PT,TsiTransType.SS,TsiTransType.CN,TsiTransType.Agg)).ToList();
			comboBoxMultiLastTransType.Items.Add("All");
			...
			comboSentAccountAge.Items.Add("Any Balance");
			comboSentAccountAge.Items.Add("Over 30 Days");
		...
		private bool ValidateSendUpdateData(List<long> listClinicNums,out List<long> listClinicsSkipped) {
			...
			foreach(long clinicNum in listClinicNums.Distinct()) {
				...
				if(!_dictClinicProgProps.TryGetValue(clinicNum,out List<ProgramProperty> listProgProps) //if the clinic doesn't have prog props, use HQ's
					&& (clinicNum==0 || !PrefC.HasClinicsEnabled || Security.CurrentUser.ClinicIsRestricted || !_dictClinicProgProps.TryGetValue(0,out listProgProps)))
				{
		...
		private void gridUnsent_MouseMove(object sender,MouseEventArgs e) {
			try {
				...
				_toolTipUnsentErrors.SetToolTip(gridUnsent,"Invalid"+" "+string.Join(" "+"and"+" ",listErrors));
			}
			catch {
				_toolTipUnsentErrors.RemoveAll();
			}
...
//TODO
		...
		private void butSend_Click(object sender,EventArgs e) {
			...
			if(listPatNumsToReselect.Count > 0)	{
				Cursor=Cursors.Default;
				msgTxt="At least one of the selected guarantors is assigned to a clinic that does not have the"+" "+demandType.GetDescription()
					+" "+"service enabled.  Those account(s) will not be sent to TSI and will remain in the unsent grid.";
			...
			if(demandType==TsiDemandType.Accelerator) {
				...
				msgTxt="The accelerator service is recommended for accounts 31-120 days old and one or more of the selected account balances "
					...
					+"Would you like to send the accounts to Accelerator?"+"\r\n\r\n"+"Press Yes to send all accounts to Accelerator."
					+"\r\n\r\n"+"Press No to send only the accounts 120 days old or less to Accelerator and leave the older accounts in the unsent "
					+"grid to send to Profit Recovery or Collection later."+"\r\n\r\n"+"Press Cancel to cancel sending all accounts.";
			}
				...
				listPatNumsWrongService=listPatAging.FindAll(x => !listPatNumsToReselect.Contains(x.PatNum) && x.DateBalBegan.Date<dateAsOf.AddDays(-180).Date)
					.Select(x => x.PatNum).ToList();
				msgTxt="The Profit Recovery service is recommended for accounts 121-180 days old and one or more of the selected account "
					+"balances is over 180 days old.  You may wish to consider placing those accounts directly to the Collection service."+"\r\n"
					+"Would you like to send the accounts to Profit Recovery?"+"\r\n\r\n"+"Press Yes to send all accounts to Profit "
					+"Recovery."+"\r\n\r\n"+"Press No to send only the accounts 180 days old or less to Profit Recovery and leave the older "
			...
			if(listPatNumsBadBday.Count>0) {
				listErrorMsgs.Add("Invalid birthdate or under the age of 18");
			...
			if(listPatNumsBadAddress.Count>0) {
				listErrorMsgs.Add("Bad address");
				listPatNumsToReselect.AddRange(listPatNumsBadAddress);
			}
			if(listErrorMsgs.Count>0) {
				Cursor=Cursors.Default;
				msgTxt="One or more of the selected guarantors has the following error(s) and will not be sent to TSI"+":\r\n\r\n"
					+string.Join("\r\n",listErrorMsgs);
				MessageBox.Show(msgTxt);
			}
			...
			if(listPatNumsNegBal.Count>0) {
				Cursor=Cursors.Default;
				msgTxt=listPatNumsNegBal.Count+" "+"of the selected guarantor(s) have a balance less than or equal to 0.  Are you sure you want "
					+"to send the account(s) to TSI?"+"\r\n\r\n"
					+"Press Yes to send the account(s) with a balance less than or equal to 0 anyway."+"\r\n\r\n"
					+"Press No to skip the account(s) with a balance less than or equal to 0 and send the remaining account(s) to TSI."+"\r\n\r\n"
					+"Press Cancel to cancel sending all accounts.";
				switch(MessageBox.Show(msgTxt,"",MessageBoxButtons.YesNoCancel)) {
					case DialogResult.No:
			...
			if(listFailedPatNums.Count>0) {
				MessageBox.Show(listFailedPatNums.Count+" "+"accounts did not upload successfully.  They have not been marked as sent to "
		...
		private void comboNewStatus_SelectedIndexChanged(object sender,EventArgs e) {
			...
			if(comboNewBillType.SelectedIndex<0 || comboNewBillType.SelectedIndex>_listBillTypesNoColl.Count-1) {
				errorProvider1.SetError(comboNewBillType,"Select billing type for accounts that will no longer be managed by Transworld.");
			}
		}
...
//TODO
		...
		private void butUpdateStatus_Click(object sender,EventArgs e) {
			...
			if(transType==TsiTransType.SS) {
				string msgTxt="The account(s) will be suspended for a maximum of 50 days but only if they are in the Accelerator or Profit Recovery "
					...
					+"and will no longer be available to reinstate."+"\r\n\r\n"+"Do you want to suspend the service for the selected account(s)?";
