	...
	public partial class FormClaimAttachment:Form {
		...
		private void buttonAddImage_Click(object sender,EventArgs e) {
			...
			try {
				...
				FriendlyException.Show("The selected file at: "+selectedFile+" could not be found",ex);
			}
				...
				FriendlyException.Show("The file does not have a valid image format. Please try again or call support."+"\r\n"+ex.Message,ex);
			}
			catch(Exception ex) {
				FriendlyException.Show("An error occurred. Please try again or call support."+"\r\n"+ex.Message,ex);
			}
		}
		...
		private void ButPasteImage_Click(object sender,EventArgs e) {
			try{
				...
				else if(Clipboard.ContainsFileDropList()) { //If the user has a file or list of files on the clipboard
					IDataObject dataObject=Clipboard.GetDataObject();
					if(dataObject.GetDataPresent(DataFormats.FileDrop)) {
						string[] files=(string[])dataObject.GetData(DataFormats.FileDrop);
						if(files.Length>1) {//Throw error if multiple images are in the list
							throw new ODException("Can only paste one image from file at a time");
						}
						try {
							...
							throw new ODException("Invalid file type when copying image from location. Valid file types include BMP, GIF, JPEG, PNG, and TIFF.");
						}
					}
					else {
						throw new ODException("Could not find a valid image file on the clipboard.");
					}
				}		
				else {
					throw new ODException("Could not find a valid image or image file on the clipboard.");
				}
			}
				...
				FriendlyException.Show("The clipboard could not be accessed. This typically occurs when the clipboard is being used by another process.",ee);
			}
			catch(Exception ex) {
				FriendlyException.Show("An error occurred while accessing the clipboard. Please call support if the issue persists.",ex);
			}
		}
