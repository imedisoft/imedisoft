...
///<summary></summary>
	public class FormClaimsSend:ODForm {
		...
		bool headingPrinted;
		int headingPrintH;
		private ComboBoxPlus comboClinic;
		...
		private void InitializeComponent(){
			...
			this.comboClinic = new OpenDental.UI.ComboBoxPlus();
		...
		private void FormClaimsSend_Load(object sender, System.EventArgs e) {
			...
			if(Preferences.GetBool(PreferenceName.ClaimsSendWindowValidatesOnLoad)) {
		...
		public void FillClinicsList(long claimCustomTracking) {
			...
			if(!Security.CurrentUser.ClinicIsRestricted) {
				comboClinic.Items.Add("Unassigned/Default");
		...
		public void LayoutToolBars(){
			...
			ToolBarMain.Buttons.Add(new ODToolBarButton("Preview",0,"Preview the Selected Claim","Preview"));
			ToolBarMain.Buttons.Add(new ODToolBarButton("Blank",1,"Print a Blank Claim Form","Blank"));
			ToolBarMain.Buttons.Add(new ODToolBarButton("Print",2,"Print Selected Claims","Print"));
			ToolBarMain.Buttons.Add(new ODToolBarButton("Labels",6,"Print Single Labels","Labels"));
			/*ToolBarMain.Buttons.Add(new ODToolBarButton(ODToolBarButtonStyle.Separator));
			ODToolBarButton button=new ODToolBarButton("Change Status",-1,"Changes Status of Selected Claims","Status");
			button.Style=ODToolBarButtonStyle.DropDownButton;
			button.DropDownMenu=contextMenuStatus;
			ToolBarMain.Buttons.Add(button);*/
			ToolBarMain.Buttons.Add(new ODToolBarButton(ODToolBarButtonStyle.Separator));
			ODToolBarButton button=new ODToolBarButton("Send E-Claims",4,"Send claims Electronically","Eclaims");
			button.Style=ODToolBarButtonStyle.DropDownButton;
			button.DropDownMenu=contextMenuEclaims;
			ToolBarMain.Buttons.Add(button);
			ToolBarMain.Buttons.Add(new ODToolBarButton("Validate Claims",-1,"Refresh and Validate Selected Claims","Validate"));
			if(CultureInfo.CurrentCulture.Name.EndsWith("CA")) {//Canadian. en-CA or fr-CA
				ToolBarMain.Buttons.Add(new ODToolBarButton("Outstanding",-1,"Get Outstanding Transactions","Outstanding"));
				ToolBarMain.Buttons.Add(new ODToolBarButton("Payment Rec",-1,"Get Payment Reconciliation Transactions","PayRec"));
				//ToolBarMain.Buttons.Add(new ODToolBarButton("Summary Rec",-1,"Get Summary Reconciliation Transactions","SummaryRec"));
			}
			else {
				ToolBarMain.Buttons.Add(new ODToolBarButton("Get Reports",5,"Get Reports from Other Clearinghouses","Reports"));
			}
			ToolBarMain.Buttons.Add(new ODToolBarButton(ODToolBarButtonStyle.Separator));
			ToolBarMain.Buttons.Add(new ODToolBarButton("Refresh",-1,"Refresh the Grid","Refresh"));
			ToolBarMain.Buttons.Add(new ODToolBarButton(ODToolBarButtonStyle.Separator));
			ToolBarMain.Buttons.Add(new ODToolBarButton("Procs Not Billed",-1,"","ProcsNotBilled"));
			ToolBarMain.Buttons.Add(new ODToolBarButton("Close",-1,"","Close"));
			...
			ToolBarHistory.Buttons.Add(new ODToolBarButton("Refresh",-1,"Refresh this list.","Refresh"));
			ToolBarHistory.Buttons.Add(new ODToolBarButton(ODToolBarButtonStyle.Separator));
			ToolBarHistory.Buttons.Add(new ODToolBarButton("Undo",-1,
				"Change the status of claims back to 'Waiting'.","Undo"));
			ToolBarHistory.Buttons.Add(new ODToolBarButton(ODToolBarButtonStyle.Separator));
			ToolBarHistory.Buttons.Add(new ODToolBarButton("Print List",2,
				"Print history list.","PrintList"));
			ToolBarHistory.Buttons.Add(new ODToolBarButton("Outstanding Claims",-1,"","OutstandingClaims"));
			/*#if DEBUG
			ToolBarHistory.Buttons.Add(new ODToolBarButton("Print Item",2,
				"For debugging, this will simply display the first item in the list.","PrintItem"));
			#else
			ToolBarHistory.Buttons.Add(new ODToolBarButton("Print Item",2,
				"Print one item from the list.","PrintItem"));
			#endif*/
		...
		private void toolBarButPreview_Click(){
			...
			if(gridMain.SelectedTags<ClaimSendQueueItem>().Count>1){
				MessageBox.Show("Please select only one claim.");
				return;
			}
		...
		private void toolBarButBlank_Click(){
			...
			FormCP.PrintImmediate("Blank claim printed",PrintSituation.Claim,0);
		}
		...
		private void toolBarButValidate_Click() {
			if(gridMain.SelectedTags<ClaimSendQueueItem>().Count==0) {
				MessageBox.Show("Please select one or more claims first.");
				return;
			}
		...
		private void SendEclaimsToClearinghouse(long hqClearinghouseNum) {
			...
			for(int i=0;i<queueItems.Count;i++) {
				if(queueItems[i].ClaimStatus=="S") {
					...
					SecurityLogs.MakeLogEntry(Permissions.ClaimSend,queueItems[i].PatNum,"Claim sent from Claims Send Window.");
				}
			}
		...
		private void pd2_PrintPage(object sender,System.Drawing.Printing.PrintPageEventArgs e) {
			...
			if(!headingPrinted) {
				...
				text=dateRangePicker.GetDateTimeFrom().ToShortDateString()+" "+"to"+" "+dateRangePicker.GetDateTimeTo().ToShortDateString();
