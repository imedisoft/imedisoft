...
namespace OpenDental {
	public partial class DashIndividualInsurance:UserControl,IDashWidgetField {
		...
		public void RefreshInsurance(Patient pat,List<InsurancePlan> listInsPlans,List<InsSub> listInsSubs,List<PatPlan> listPatPlans,List<Benefit> listBenefits,List<ClaimProcHist> histList){
			textPriMax.Text="";
			textPriDed.Text="";
			textPriDedRem.Text="";
			textPriUsed.Text="";
			...
			double pend=0;
			double used=0;
			InsurancePlan isnPlanCur;//=new InsPlan();
			InsSub subCur;
			if(listPatPlans.Count>0){
				...
				used=InsPlans.GetInsUsedDisplay(histList,DateTime.Today,isnPlanCur.Id,listPatPlans[0].PatPlanNum,-1,listInsPlans,listBenefits,pat.PatNum,listPatPlans[0].InsSubNum);
				textPriPend.Text=pend.ToString("F");
				textPriUsed.Text=used.ToString("F");
				maxInd=Benefits.GetAnnualMaxDisplay(listBenefits,isnPlanCur.Id,listPatPlans[0].PatPlanNum,false);
				...
				//deductible:
				ded=Benefits.GetDeductGeneralDisplay(listBenefits,isnPlanCur.Id,listPatPlans[0].PatPlanNum,BenefitCoverageLevel.Individual);
				dedFam=Benefits.GetDeductGeneralDisplay(listBenefits,isnPlanCur.Id,listPatPlans[0].PatPlanNum,BenefitCoverageLevel.Family);
				if(ded!=-1){
					textPriDed.Text=ded.ToString("F");
					dedRem=InsPlans.GetDedRemainDisplay(histList,DateTime.Today,isnPlanCur.Id,listPatPlans[0].PatPlanNum,-1,listInsPlans,pat.PatNum,ded,dedFam);
			...
			if(listPatPlans.Count>1){
				...
				used=InsPlans.GetInsUsedDisplay(histList,DateTime.Today,isnPlanCur.Id,listPatPlans[1].PatPlanNum,-1,listInsPlans,listBenefits,pat.PatNum,listPatPlans[1].InsSubNum);
				textSecUsed.Text=used.ToString("F");
				maxInd=Benefits.GetAnnualMaxDisplay(listBenefits,isnPlanCur.Id,listPatPlans[1].PatPlanNum,false);
				if(maxInd==-1){//if annual max is blank
					textSecMax.Text="";
					textSecRem.Text="";
				}
				...
				//deductible:
				ded=Benefits.GetDeductGeneralDisplay(listBenefits,isnPlanCur.Id,listPatPlans[1].PatPlanNum,BenefitCoverageLevel.Individual);
				dedFam=Benefits.GetDeductGeneralDisplay(listBenefits,isnPlanCur.Id,listPatPlans[1].PatPlanNum,BenefitCoverageLevel.Family);
				if(ded!=-1){
					textSecDed.Text=ded.ToString("F");
					dedRem=InsPlans.GetDedRemainDisplay(histList,DateTime.Today,isnPlanCur.Id,listPatPlans[1].PatPlanNum,-1,listInsPlans,pat.PatNum,ded,dedFam);
