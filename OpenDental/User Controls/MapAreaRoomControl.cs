	...
	public partial class MapAreaRoomControl:DraggableControl {
		...
		private void PaintLarge(PaintEventArgs e) {
			...
			try {
				...
				//This is to preserve logic in case we want to bring back ChatImage.  See FormMapHQ for setting ChatImage.
			...
			try {
				...
				if(PhoneImage!=null) {
					using(Bitmap bitmap = new Bitmap(PhoneImage,new Size((int)(PhoneImage.Width/1.2),(int)(PhoneImage.Height/1.2)))) {//right-most 1/4 of row 3 is the phone icon
						...
						RectangleF rectImage=new RectangleF(((rcOuter.Width+BorderThickness)-bitmap.Width-smallMargin-2),rcOuter.Y+yPosBottom-smallMargin,bitmap.Width+3,rowHeight);
						//Scale the image.
						if(bitmap.Height<rectImage.Height || bitmap.Width<=rectImage.Width) {
							rectImage.Y+=(rectImage.Height-bitmap.Height)/2;
							rectImage.X+=(rectImage.Width-bitmap.Width)/2;
						...
						e.Graphics.DrawImage(
							PhoneImage,
							_rectPhone,
							new RectangleF(0,0,bitmap.Width+3,bitmap.Height),
							GraphicsUnit.Pixel);
						if(ODBuild.IsDebug()) {
