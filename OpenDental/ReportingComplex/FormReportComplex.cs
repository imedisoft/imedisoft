    ...
    public void RefreshWindow() {
      LayoutToolBar();
		...
		public void LayoutToolBar(){
			...
			ToolBarMain.Buttons.Add(new ODToolBarButton(ODToolBarButtonStyle.Separator));
      ODToolBarButton butWrapText=new ODToolBarButton("Wrap Text",-1,"Wrap Text In Columns","WrapText");
      ...
      ToolBarMain.Buttons.Add(new ODToolBarButton(ODToolBarButtonStyle.Separator));
		...
		private bool ResetODprintout(){
			ReportComplexEvent.Fire(EventCategory.ReportComplex,"Retrieving Printer Settings"+"...");
			...
			_totalRowsPrinted=0;
			_pagesPrinted=0;
			ReportComplexEvent.Fire(EventCategory.ReportComplex,"Calculating Row Heights"+"...");
		...
		private void pd2_PrintPage(object sender, PrintPageEventArgs ev){
			...
			{ 
				...
				ReportComplexEvent.Fire(EventCategory.ReportComplex,"Printing Page"+" "+(_currentPage)+" - "
		...
		private void pagePrinter(PrintPageEventArgs ev) {
			...
			ReportComplexEvent.Fire(EventCategory.ReportComplex,new ProgressBarHelper("Printing Page"+" "+(_currentPage)+"..."
			...
			foreach(ReportObject reportObject in _myReport.ReportObjects){
				...
				using(StringFormat strFormat=ReportObject.GetStringFormatAlignment(reportObject.ContentAlignment)) {
					...
					#region FieldObject
						...
						g.DrawString("Page"+" "+_currentPage,fieldObject.Font,Brushes.Black,layoutRect,strFormat);
					}
		...
		private List<string> GetDisplayString(string rawText,string prevDisplayText,ReportObject reportObject,int i,QueryObject queryObj,bool isExport) {
			...
			if(reportObject.FieldValueType==FieldValueType.Age) {
				...
				displayText=PIn.Date(dt.Rows[i][queryObj.ArrDataFields.IndexOf(reportObject.DataField)].ToString()).ToString(reportObject.StringFormat);
				if(i>0 && reportObject.SuppressIfDuplicate) {
					prevDisplayText=PIn.Date(dt.Rows[i-1][queryObj.ArrDataFields.IndexOf(reportObject.DataField)].ToString()).ToString(reportObject.StringFormat);
				}
			}
