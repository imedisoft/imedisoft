	...
	public partial class FormOrthoChart:ODForm {
		...
		private void FormOrthoChart_Load(object sender,EventArgs e) {
			...
			if(_indexInitialTab!=tabControl.SelectedIndex) {
				...
				Logger.LogAction("FormOrthoChart_Load",() => {
					FillGrid();
				});
			}
			FillGridPat();
		...
		private void menuItemSetup_Click(object sender,EventArgs e) {
			...
			if(form.ShowDialog()==DialogResult.OK) {
				FillTabs();
				FillDisplayFields();
				Logger.LogAction("menuItemSetup_Click",() => FillGrid());
			}
		}

		private void tabControl_TabIndexChanged(object sender,EventArgs e) {
			Logger.LogAction("tabControl_TabIndexChanged",() => FillGrid());
		}
		...
		private void ClearSignature(int orthoChartRow) {
			DateTime orthoDate=GetOrthoDate(orthoChartRow);
			...
			DisplaySignature(orthoChartRow);
			_prevRow=orthoChartRow;
		}
		...
		private void gridMain_CellEnter(object sender,ODGridClickEventArgs e) {
			Logger.LogAction("gridMain_CellEnter",() => {
				SaveAndSetSignatures(e.Row);
			});
		}
			...
			Logger.LogAction("gridMain_CellLeave",() => {
				...
				if(CanEditRow(orthoDate)) {
					if(newText != oldText) {
						...
						if(_showSigBox) {
							Logger.LogAction("gridMain_CellLeave",() => { signatureBoxWrapper.ClearSignature(false); });
						}
		...
		private void DisplaySignature(int gridRow,bool hasRefresh=true) {
			Logger.LogAction("DisplaySignature",() => {
		...
		private bool CanEditRow(DateTime orthoDate) {
			...
			if(sigChart==null || sigChart.UserNum==_curUser.Id) {
				_dictCanEditDay[orthoDate]=true;
				return true;
			}
			bool canEditRow;
		...
		private void butAdd_Click(object sender, EventArgs e)
		{
			...
			{
				Logger.LogAction("butAdd_Click", () => FillGrid());
				return;
			}
