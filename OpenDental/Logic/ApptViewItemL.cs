...
namespace OpenDental
{
	...
	{
		/// <summary>
		/// Gets the 'visible' operatories for todays date with the currently selected appointment view.
		/// This is strictly used when filtering the waiting room for clinics.
		/// Pass in null for the dailySched if this is a weekly view.
		/// </summary>
		public static List<Operatory> GetOpsForApptView(ApptView apptViewCur, bool isWeekly, List<Schedule> dailySched)
		{
            FillForApptView(isWeekly, apptViewCur, 
				out List<Provider> visProvs, 
				out List<Operatory> visOps, 
				out List<ApptViewItem> forCurView, 
				out List<ApptViewItem> apptRows, 
				out int rowsPerIncr, false);
		...
		/// <summary>
		/// Fills visProvs, visOps, forCurView, apptRows, and rowsPerIncr based on the appointment view passed in and whether it is for the week view or not.
		/// This method uses 'out' variables so that the encompassing logic doesn't ALWAYS affect the global static variables used to draw the appointment views.
		/// We don't want the following logic to affect the global static variables in the case where we are trying to get information needed to filter the waiting room.
		/// </summary>
		public static void FillForApptView(bool isWeekly, ApptView apptViewCur, out List<Provider> visProvs, out List<Operatory> visOps, out List<ApptViewItem> forCurView, out List<ApptViewItem> apptRows, out int rowsPerIncr, bool isFillVisProvs = true)
		{
