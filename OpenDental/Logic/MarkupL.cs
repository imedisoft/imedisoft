...
{
    /// <summary>
    /// Used for wiki and HTML email editing.
    /// </summary>
		...
		public static bool ValidateMarkup(ODcodeBox textContent, bool isForSaving, bool showMsgBox = true, bool isEmail = false)
		{
			...
			for (int i = 0; i < tagMatches.Count; i++)
			{
				if (tagMatches[i].ToString().Contains("\n"))
				{
					...
					{
						MessageBox.Show(
							"Error at line: " + textContent.GetLineFromCharIndex(tagMatches[i].Index) + " - " +
							"Tag definitions cannot contain a return line: " + tagMatches[i].Value.Replace("\n", ""));
					}
					return false;
				}
			}
			...
			if (!isEmail)
			{
				...
				if (isForSaving)
				{
					...
					{
						string imgPath = Storage.CombinePaths(wikiImagePath, matches[i].Value.Substring(6).Trim(']'));
						if (!Storage.FileExists(imgPath))
						{
							if (showMsgBox)
							{
								MessageBox.Show(
									"Error at line: " + textContent.GetLineFromCharIndex(matches[i].Index) + " - " + 
									"Not allowed to save because image does not exist: " + imgPath);
							}
							return false;
						}
					}
			...
			if (isEmail)
			{
				...
				if (isForSaving)
				{
					for (int i = 0; i < matches.Count; i++)
					{
						string imgPath = Storage.CombinePaths(emailImagePath, matches[i].Value.Substring(6).Trim(']'));
						if (!Storage.FileExists(imgPath))
						{
							if (showMsgBox)
							{
			...
			foreach (Match match in matches)
			{
				if (!WikiLists.CheckExists(match.Value.Substring(7).Trim(']')))
				{
					...
					{
						MessageBox.Show(
							"Error at line: " + textContent.GetLineFromCharIndex(match.Index) + " - " + 
							"Wiki list does not exist in database: " + match.Value.Substring(7).Trim(']'));
					}
					return false;
				}
			}
				...
				if (lines[i].Trim().StartsWith("*"))
				{
					if (!lines[i].StartsWith("*"))
					{
						if (showMsgBox)
						{
							MessageBox.Show(
								"Error at line: " + (i + 1) + " - " + 
								"Stars used for lists may not have a space before them.");
						}
						return false;
					}
					if (lines[i].Trim().StartsWith("* "))
					{
						if (showMsgBox)
						{
							MessageBox.Show(
								"Error at line:" + " " + (i + 1) + " - " + 
								"Stars used for lists may not have a space after them.");
						}
						return false;
					}
				}
				if (lines[i].Trim().StartsWith("#"))
				{
					if (!lines[i].StartsWith("#"))
					{
						if (showMsgBox)
						{
							MessageBox.Show(
								"Error at line: " + (i + 1) + " - " + 
								"Hashes used for lists may not have a space before them.");
						}
						return false;
					}
					if (lines[i].Trim().StartsWith("# "))
					{
						if (showMsgBox)
						{
							MessageBox.Show(
								"Error at line: " + (i + 1) + " - " + 
								"Hashes used for lists may not have a space after them.");
						}
						return false;
					}
				}
			...
			foreach (Match match in matches)
			{
				...
				{//allow colored text to have quotes.
					if (showMsgBox)
					{
						MessageBox.Show(
							"Error at line: " + textContent.GetLineFromCharIndex(match.Index) + " - " + 
							"Link cannot contain double quotes: " + match.Value);
					}
					return false;
				}
				...
				else
				{
					if (match.Value.Contains("|"))
					{
						if (showMsgBox)
						{
							MessageBox.Show(
								"Error at line: " + textContent.GetLineFromCharIndex(match.Index) + " - " + 
								"Internal link cannot contain a pipe character: " + match.Value);
						}
						return false;
					}
				}
			...
			foreach (Match match in matches)
			{
				...
				if (!lines[0].StartsWith("!"))
				{
					if (showMsgBox)
					{
						MessageBox.Show(
							"Error at line: " + textContent.GetLineFromCharIndex(match.Index) + " - " + 
							"The second line of a table markup section must start with ! to indicate column headers.");
					}
					return false;
				}
				...
				if (lines[0].StartsWith("! "))
				{
					if (showMsgBox)
					{
						MessageBox.Show(
							"Error at line: " + textContent.GetLineFromCharIndex(match.Index) + " - " + 
							"In the table, at line 2, there cannot be a space after the first !");
					}
					return false;
				}
				...
				for (int c = 0; c < cells.Length; c++)
				{
					...
					{//e.g. Width="90"| 
						if (showMsgBox)
						{
							MessageBox.Show(
								"Error at line: " + textContent.GetLineFromCharIndex(match.Index) + " - " +
								"In the table markup, each header must be formatted like this: Width=\"#\"|...");
						}
						return false;
					}
				}
				...
				{//loop through the lines after the header
					if (!lines[i].StartsWith("|"))
					{
						...
						{
							MessageBox.Show(
								"Table rows must start with |.  At line " + (i + 1).ToString() + ", this was found instead:" + lines[i]);
						}
						return false;
					}
				}
