	...
	public class WebFormL {
		...
		public static void LoadImagesToSheetDef(SheetDef sheetDefCur) {
			for(int j=0;j<sheetDefCur.SheetFieldDefs.Count;j++) {
				try {
					//Do not load images for sheetfielddefs which have a non-null language value. Language support for Webforms is not instituted yet.
					if(sheetDefCur.SheetFieldDefs[j].FieldType==SheetFieldType.Image && string.IsNullOrEmpty(sheetDefCur.SheetFieldDefs[j].Language)) {
		...
		public static bool VerifyRequiredFieldsPresent(SheetDef sheetDef) {
			...
			if(!hasFName || !hasLName || !hasBirthdate) {
				MessageBox.Show("The sheet called"+" \""+sheetDef.Description+"\" "
					+"does not contain all three required fields: LName, FName, and Birthdate.");
				return false;
			}
			return true;
		}
		...
		public static bool TryAddOrUpdateSheetDef(Form currentForm,SheetDef sheetDef,bool isNew,List<WebForms_SheetDef> listWebFormSheetDefs = null) {
			//Remove sheetfielddefs where language is non-null since Languages are not supported in webforms at this time.
			SheetDef webSheetDefToUpdate=sheetDef.Copy();
			webSheetDefToUpdate.SheetFieldDefs=webSheetDefToUpdate.SheetFieldDefs.Select(x => x.Copy()).ToList();
			webSheetDefToUpdate.SheetFieldDefs.RemoveAll((x) => !string.IsNullOrEmpty(x.Language));
			try {				
				...
				if(!isNew) {
					...
					foreach(WebForms_SheetDef webSheetDef in listWebFormSheetDefs) {
						WebForms_SheetDefs.UpdateSheetDef(webSheetDef.WebSheetDefID,webSheetDefToUpdate,doCatchExceptions: false);
					}
				}
				else {
					WebForms_SheetDefs.TryUploadSheetDef(webSheetDefToUpdate);
				}
				return true;
			}
				...
				if(((HttpWebResponse)wex.Response).StatusCode==HttpStatusCode.RequestEntityTooLarge) {
					...
					while(isTooLargeRequest && chunkSize>=1024) {//loop until successful request transfer or chunk sizes get smaller than 1 KB
						try {
							if(!isNew) {
								foreach(WebForms_SheetDef webSheetDef in listWebFormSheetDefs) {
									WebForms_SheetDefs.UpdateSheetDefChunked(webSheetDef.WebSheetDefID,webSheetDefToUpdate,chunkSize);
								}
							}
							else {
								WebForms_SheetDefs.TryUploadSheetDefChunked(webSheetDefToUpdate,chunkSize);
							}
						}
		...
		public static string RetrieveAndSaveData(string cultureName) {
			...
			try {
				...
				if(WebUtils.GetDentalOfficeID()==0) {
					strMsg="Either the registration key provided by the dental office is incorrect or the Host Server Address cannot be found.";
					return strMsg;
				}
				...
				do {
					...
					if(listWebFormSheets.Count==0) {
						if(iterations==1 && listSheetsFromCemtTool.Count==0) {
							strMsg="No Patient forms retrieved from server";
						}
						else {
							strMsg="All Patient forms retrieved from server";
						}
						return strMsg;
					}
			...
			ref List<long> listSheetIdsForDeletion) 
		{
			...
			if(listMatchingPats.IsNullOrEmpty() || listMatchingPats.Count > 1) {//0 or > 1
				...
				string logText;
				if(isWebForms) {
					logText="Web form import from:";
				}
				else {
					logText="CEMT patient transfer import from:";
				}
			...
			if(patNum==0) {
				...
				logText+=" "+lName+", "+fName+" "+bDate.ToShortDateString()+"\r\n"
					+"User created new pat:"+" "+pat.LName+", "+pat.FName+" "+pat.Birthdate.ToShortDateString();
				SecurityLogs.MakeLogEntry(Permissions.SheetEdit,patNum,logText);
			}
			else if(pat==null) {
				pat=Patients.GetPat(patNum);
		...
		private static Patient CreatePatient(String LastName,String FirstName,DateTime birthDate,WebForms_Sheet webFormSheet,Sheet sheet, string cultureName) {
			...
			newPat.BillingType=Preferences.GetLong(PreferenceName.PracticeDefaultBillType);
			if(isWebForm) {
