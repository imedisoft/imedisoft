...
namespace OpenDental
{
	public class ProcedureL
	{
		...
		public static bool CheckPermissionsAndGlobalLockDate(Procedure procOld, Procedure procNew, DateTime procDate, double procFeeOverride = double.MinValue)
		{
			...
			{
				dateToUseProcOld = DateTime.Today;//ignore date limitation for EO/EC procedures
			}
			if (procOld.ProcStatus != procNew.ProcStatus && !Security.IsAuthorized(perm, dateToUseProcOld))
			{//block old date
				return false;
			}
			if (procNew.ProcStatus.In(ProcStat.C, ProcStat.EO, ProcStat.EC))
			{
				...
				{
					//ignore date limitation for EO/EC procedures
				...
				if (procOld.ProcStatus != procNew.ProcStatus)
				{
					...
					if (!Security.IsAuthorized(perm, dateToUseProcCur))
					{//block new date, too
						return false;
					}
				}
