	...
	public class AppointmentL {
		...
		public static void ShowKioskManagerIfNeeded(Appointment oldAppt,long newConfirmed) {
			...
			if(MobileAppDevices.IsClinicSignedUpForEClipboard(clinicNum)) {
				...
				OpenDentBusiness.AutoComm.Arrivals.ProcessArrival(oldAppt.PatNum,oldAppt.PatNum.SingleItemToList(),oldAppt.ClinicNum
					,listAppts:oldAppt.SingleItemToList());//Pass oldAppt so it still has the old confirmation status; which has already updated in db.
			}
		}
