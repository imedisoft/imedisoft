...
namespace OpenDental{
	...
	//this test case worked, so now it is just a matter of finishing this off, and Excel export will be done.
		...
		private void butExport_Click(object sender, System.EventArgs e){
			...
			string filePath=ODFileUtils.CombinePaths(Path.GetTempPath(),fileName);
				...
				else saveFileDialog2.InitialDirectory=Preferences.GetString(PreferenceName.ExportPath);
		...
		private void FormQuery_Load(object sender, System.EventArgs e) {
			...
			if(string.IsNullOrWhiteSpace(Preferences.GetString(PreferenceName.ReportingServerDbName)) 
				|| string.IsNullOrWhiteSpace(Preferences.GetString(PreferenceName.ReportingServerCompName))) {
	...
	//					  tableOut.Rows[i][j]=CovCats.GetDesc(PIn.PInt(tableOut.Rows[i][j].ToString()));
		...
		private void FillForm() {
			...
			_reportSimpleGrid.SubTitle.Add(Preferences.GetString(PreferenceName.PracticeTitle));
		...
		private void pagePrinter(PrintPageEventArgs ev) {
			...
			{
				bool isColWrap=Preferences.GetBool(PreferenceName.ReportsWrapColumns);
