	...
	public class FormRpProcSheet : ODForm {
		...
		private void CreateIndividual() {
			...
			try { 
				...
				string text="Error getting report data:"+" "+ex.Message+"\r\n\r\n"+ex.StackTrace;
			...
			report.ReportName="Daily Procedures";
			report.AddTitle("Title","Daily Procedures",fontTitle);
			...
			if(isAnyClinicMedical) {
				...
				query.AddColumn("Code",70,FieldValueType.String,font);
			...
			if(PrefC.HasClinicsEnabled) {
				query.AddColumn("Clinic",100,FieldValueType.String,font);
			}
			query.AddColumn("Fee",80,FieldValueType.Number,font);
		...
		private void CreateGrouped() {
			...
			Font fontTitle=new Font("Tahoma",17,FontStyle.Bold);
			Font fontSubTitle=new Font("Tahoma",10,FontStyle.Bold);
			report.ReportName="Procedures By Procedure Code";
			report.AddTitle("Title","Procedures By Procedure Code",fontTitle);
			report.AddSubTitle("Practice Title",Prefs.GetString(PrefName.PracticeTitle),fontSubTitle);
			report.AddSubTitle("Dates of Report",date1.SelectionStart.ToString("d")+" - "+date2.SelectionStart.ToString("d"),fontSubTitle);
			report.AddSubTitle("Providers",subtitleProvs,fontSubTitle);
			if(PrefC.HasClinicsEnabled) {
			...
			QueryObject query=report.AddQuery(table,"Date"+": "+DateTimeOD.Today.ToString("d"));
			query.AddColumn("Category",150,FieldValueType.String,font);
			query.AddColumn("Code",130,FieldValueType.String,font);
			query.AddColumn("Description",140,FieldValueType.String,font);
			query.AddColumn("Quantity",60,FieldValueType.Integer,font);
			query.GetColumnDetail("Quantity").ContentAlignment=ContentAlignment.MiddleRight;
			query.AddColumn("Average Fee",110,FieldValueType.String,font);
			query.GetColumnDetail("Average Fee").ContentAlignment=ContentAlignment.MiddleRight;
			query.AddColumn("Total Fees",110,FieldValueType.Number,font);
			report.AddPageNum(font);
			if(!report.SubmitQueries()) {
				return;
			}
