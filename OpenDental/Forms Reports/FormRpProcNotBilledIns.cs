...
namespace OpenDental{
	...
	public partial class FormRpProcNotBilledIns : ODForm {
		...
		public void FillGrid() {
			...
			for(int i=0;i<_myReport.ReportObjects.Count;i++) {
				...
				for(int j=0;j<queryObj.ReportTable.Rows.Count;j++) {
					...
					row.Cells.Add(PIn.String(queryObj.ReportTable.Rows[j][1].ToString()));//Stat
		...
		private void RefreshReport() {
			...
			if(PrefC.HasClinicsEnabled) {
				subtitleClinics="Clinics: "+comboClinics.GetStringSelectedClinics();
			}
		...
		private void butNewClaims_Click(object sender,EventArgs e) {
			...
			DateTime? dateRestricted=GroupPermissions.GetDateRestrictedForPermission(Permissions.NewClaimsProcNotBilled,
				UserGroups.GetForUser(Security.CurrentUser.Id, true).Select(x => x.Id).ToList());
			...
			MessageBox.Show("Number of claims created"+": "+claimCreatedCount);
		}
