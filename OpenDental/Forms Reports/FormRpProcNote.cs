...
namespace OpenDental{
	public class FormRpProcNote:ODForm {
		...
		private void butPrint_Click(object sender,EventArgs e) {
			...
			PrinterL.TryPrintOrDebugRpPreview(pd_PrintPage,"Incomplete Procedure Notes report printed",PrintoutOrientation.Landscape);
		}  
		...
		private void pd_PrintPage(object sender,PrintPageEventArgs e) {
			...
			if(!_headingPrinted) {
				...
				if(comboProvs.IsAllSelected){
					...
					text="For Providers:"+" "+string.Join(",",GetListSelectedProvNums().Select(provNum => Providers.GetAbbr(provNum)));
				}
				g.DrawString(text,subHeadingFont,Brushes.Black,center-g.MeasureString(text,subHeadingFont).Width/2,yPos);
				yPos+=(int)g.MeasureString(text,headingFont).Height;
