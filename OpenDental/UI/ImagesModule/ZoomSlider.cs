...
namespace OpenDental.UI{
	public partial class ZoomSlider : Control{
		...
		private int _fit;
		private Font _font=new Font(FontFamily.GenericSansSerif,8.25f);
		private Font _fontPlusMinus=new Font(FontFamily.GenericSansSerif,14f);
		...
		public ZoomSlider(){
			...
			DoubleBuffered=true;
			_brushMainGradient=new LinearGradientBrush(new Point(0,0),new Point(0,25),Color.FromArgb(255,255,255),Color.FromArgb(171,181,209));
			_brushPushedGradient=new LinearGradientBrush(new Point(0,0),new Point(0,25),Color.FromArgb(225,225,225),Color.FromArgb(141,151,179));
			CalcMainRectangles();
		}
		...
		protected override Size DefaultSize {
			get {
				...
				//heavily optimized for about 25 to 26px high
			}
		}
		...
		[Category("OD")]
		[Description("Occurs when user changes the zoom level.  If fires repeatedly while dragging.")]
		...
		protected override void OnPaint(PaintEventArgs pe){
			...
			if(DesignMode){
				...
				g.DrawRectangle(Pens.SlateGray,0,0,Width-1,Height-1);
			...
			g.DrawString("-",_fontPlusMinus,brushText,_rectangleMinus.Left+3,_rectangleMinus.Top-1);
			g.DrawString("+",_fontPlusMinus,brushText,_rectanglePlus.Left,_rectangleMinus.Top+1);
			g.DrawString("Fit",_font,brushText,_rectangleFit.Left+5,_rectangleFit.Top+6);
			g.DrawString("100",_font,brushText,_rectangle100.Left+3,_rectangle100.Top+6);
			//Tick line
			int yTickLine=4;
			g.DrawLine(Pens.SlateGray,_rectangleTickLine.X,yTickLine,_rectangleTickLine.Right,yTickLine);
			g.DrawLine(Pens.SlateGray,_rectangleTickLine.X,0,_rectangleTickLine.X,yTickLine);
			int xFitPixels=(int)(((float)_rectangleTickLine.Right-_rectangleTickLine.Left)//how many pixels in tick line
				/(_maximum-1) *_fit);
			g.DrawLine(Pens.SlateGray,_rectangleTickLine.Left+xFitPixels,0,_rectangleTickLine.Left+xFitPixels,yTickLine);
			g.DrawLine(Pens.SlateGray,_rectangleTickLine.Right,0,_rectangleTickLine.Right,yTickLine);
			//Slider
			...
			if(_isMouseDown){//outline any pressed button
				if(_rectangleMinus.Contains(_pointMouseDown)){
					g.DrawRectangle(Pens.SlateGray,_rectangleMinus);
				}
				if(_rectanglePlus.Contains(_pointMouseDown)){
					g.DrawRectangle(Pens.SlateGray,_rectanglePlus);
				}
				if(_rectangleFit.Contains(_pointMouseDown)){
					g.DrawRectangle(Pens.SlateGray,_rectangleFit);
				}
				if(_rectangle100.Contains(_pointMouseDown)){
					g.DrawRectangle(Pens.SlateGray,_rectangle100);
				}
			}
			else{//outline any hover button
			...
			g.DrawLine(Pens.SlateGray,0,Height-1,Width,Height-1);
		}
		...
		#region Methods
		private void CalcMainRectangles(){
