...
namespace OpenDental.UI
{
    ...
    public partial class ContrApptPanel : UserControl
	{
		...
		private static readonly float _radiusCorn = 2;
		...
		public List<Operatory> ListOpsVisible
		{
			...
			set
			{
				...
				_dictOpNumToColumnNum = _listOpsVisible.ToDictionary(x => x.Id, x => _listOpsVisible.FindIndex(y => y.Id == x.Id));
				_isValidMain = false;
		...
		protected override void OnMouseDown(MouseEventArgs e)
		{
			...
			{//clicked in operatory or weekday header
				...
				for (int i = 0; i < ListOpsVisible.Count; i++)
				{
						...
						|| e.X < _widthTime + _widthProv * _listProvsVisible.Count + _widthOpCol * (i + 1))//compare to the righthand side of op
					{
						...
						Provider prov = Providers.GetById(ListOpsVisible[i].ProvDentist);//We can enhance this later to show Hygienist.
						if (prov == null)
						{
						...
						else
						{
							...
							{
								txt += "\r\n" + "Default: " + prov.FirstName + " " + prov.LastName + ", " + defProvSpecialty.Name;
							}
						}
			...
			_opNumClicked = ListOpsVisible[XPosToOpIdx(e.X)].Id;
		...
		protected override void OnMouseUp(MouseEventArgs e)
		{
			...
			try
			{
				...
				if (timeSpanNewRounded != appt.AptDateTime.TimeOfDay
					|| ListOpsVisible[opIdx].Id != appt.Op
				...
				appt.Op = ListOpsVisible[opIdx].Id;
		...
		public string GetProduction(List<DataRow> listDataRows, List<long> listOpNums, List<long> listProvNums, DateTime start, DateTime end)
		{
			...
			if (Preferences.GetBool(PreferenceName.ApptModuleAdjustmentsInProd) && TableAppointments.Rows.Count > 0)
			{
				netproduction += Adjustments.GetAdjustAmtForAptView(start, end, Clinics.Active.Id, listOpNums, listProvNums);
			}
			...
			int widthMain, float widthNarrowedOnRight, DateTime dateStart, DateTime dateEnd)
		{
			...
			for (int i = 0; i < tableAppointments.Rows.Count; i++)
			{
				...
				apptLayoutInfo.IdxOp = listOperatories.FindIndex(x => x.Id == PIn.Long(dataRow["Op"].ToString()));
		...
		private void DrawBlockouts(Graphics g)
		{
			...
			for (int i = 0; i < schedForType.Length; i++)
			{
				...
				for (int o = 0; o < schedForType[i].Ops.Count; o++)
				{
					...
					if (Preferences.GetBool(PreferenceName.SolidBlockouts))
					{
				...
				{//_rowsPerHr
					...
					switch (_provBar[j, i])
					{
						...
						case 1:
							...
							{
								using (SolidBrush solidBrushProvColor = new SolidBrush(ListProvsVisible[j].Color))
								{
							...
							break;
						case 2:
							using (HatchBrush hatchBrushProvColor = new HatchBrush(HatchStyle.DarkUpwardDiagonal, Color.Black, ListProvsVisible[j].Color))
							{
		...
		private void DrawProvScheds(Graphics g)
		{
			...
			for (int j = 0; j < ListProvsVisible.Count; j++)
			{
				schedulesForProv = Schedules.GetForType(ListSchedules, ScheduleType.Provider, ListProvsVisible[j].Id);
		...
		{
			if ((ApptViewCur == null && Preferences.GetBool(PreferenceName.AppointmentBubblesDisabled))
			...
			{//if the pointer is still on the same appt as last time
				if (DateTime.Now.AddMilliseconds(-280) > _dateTimeBubble //if it's been .28 seconds
					| !Preferences.GetBool(PreferenceName.ApptBubbleDelay)) //or if there is not supposed to be a delay
				{
			...
			for (int i = 0; i < listDisplayFieldsBubble.Count; i++)
			{
				...
				switch (listDisplayFieldsBubble[i].InternalName)
				{
					...
					case "Note":
						...
						int maxNoteLength = PrefC.GetInt(PreferenceName.AppointmentBubblesNoteLength);
					...
					case "Horizontal Line":
						if (showingPatientPicture && y < 100 + imageOffset)
						{ //height of pat picture is 100 and the image offset
							y = 100 + imageOffset;
						}
						y += 3;
			...
			if (!Preferences.GetBool(PreferenceName.ApptBubbleDelay))
			{
				_isBubbleVisible = true;
			}
			else
			...
			{//to the right of prov header, so in ops area
				...
				for (int i = 0; i < ListOpsVisible.Count; i++)
				{
						...
						|| point.X + hScrollBar1.Value < _widthTime + _widthProv * _listProvsVisible.Count + _widthOpCol * (i + 1))//compare to the righthand side of op
					{
						...
						if (_opNumHeaderTipLast == ListOpsVisible[i].Id)
						{
						...
						{//changed op
							txt = ListOpsVisible[i].OpName;
							string opProduction = GetOperatoryProduction(ListOpsVisible[i].Id);
							if (opProduction != "")
							{
								txt += "\r\n" + opProduction;
							}
							_opNumHeaderTipLast = ListOpsVisible[i].Id;
						}
						break;
					}
				}
