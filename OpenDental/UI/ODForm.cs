...
namespace OpenDental {
	///<summary>Most OD forms extend this class. It does help and signal processing.  Jordan is the only one allowed to alter this file.</summary>
	public class ODForm : Form {
		...
		///<summary>Holds the windows edition of the client. HelpForm and CreateHelpMenu() use this. Linux users will be treated as Windows 10 as far as the HelpForm is concerned. Formatting issues on Linux will have to be handled as they are found.</summary>
		...
		///<summary>This is here so that the default and reset show properly.  Also, this still shows in designer in spite of being set to Browsable false.  The value doesn't matter at all because this property is deprecated with new high dpi support.</summary>
		[Browsable(false)]
		...
		///<summary>Takes the form's name and attempts to find a manual page associated to it to display to the user.  Pass in the name of the calling form.</summary>
		...
		///<summary>Creates or recreates the OD Help button. There are some spots in the program that close or modify all forms. If that is the case the help button may have been affected and needs to be refreshed. Example- FormOpendental log off closes all forms.</summary>
		...
		///<summary>Returns an int value representing how much to adjust the x position for the ODHelp button based off of testing with each version. ComputerInfo().OSFullName returns the OS product name (edition). Here is a list of all Window's product names as of 2/21/19
		...
		///<summary>Sets global preference for x location of help button. Percentage away on toolbar. 1=100%=Default. This will be saved to db once FormOpenDental closes at the end of this session.  Saving from this class instance was too chatty so we will hang onto it and save when this session ends.</summary>
	...
	#region FormHelpWrapper
	///<summary>The class that encompasses the two forms used to create the help "button". This class reduces duplicate code. Simple eventing happens at this level. More complex events are tied to methods after instantiation. If this class is modified please reconsider CreateHelpMenu().</summary>
