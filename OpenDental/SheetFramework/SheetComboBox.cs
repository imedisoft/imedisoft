...
namespace OpenDental
{
    public partial class SheetComboBox : Control
	{
        ...
        /// <summary>
		/// A default option for the combo box to display.  Required to be set for screening tooth chart combo boxes.  E.g. "ling", "b".
		/// </summary>
        ...
        /// <summary>
		/// Currently fills the combo box with the default options for combo boxes on screen charts.
		/// </summary>
		...
		public SheetComboBox(string values)
		{
			...
			if (arrayValues.Length > 1)
			{
				...
				ComboOptions = arrayValues[1].Split('|');
			}
			else
			{//Incorrect format.
			 ...
			 //Upon Ok click this will correct the fieldvalue format.
				SelectedOption = "";
				ComboOptions = arrayValues[0].Split('|');//Will be an empty string if no '|' is present.
			}
			foreach (string option in ComboOptions)
			{
