...
namespace OpenDental.Bridges
{
    ...
    public static class Dexis
	{
		...
		public static void SendData(Program program, Patient patient)
		{
			...
			var programProperties = ProgramProperties.GetForProgram(program.ProgramNum);
			var programProperty = ProgramProperties.GetCur(programProperties, "InfoFile path");

			string infoFile = programProperty.PropertyValue;
			if (infoFile.Trim() == "")
			{
			...
			if (patient != null)
			{
				try
				{
					...
					string id = "";
					if (programProperty.PropertyValue == "0")
					{
