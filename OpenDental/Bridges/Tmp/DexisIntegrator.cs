...
{
    /// <summary>
    /// The Dexis Integrator is used when an office uses Terminal Services or Citrix and want to take X-rays on a client workstation with OD
    /// running on a server viewed through an RDP session. 
    /// The Integrator will be installed on the Terminal Server and on the client workstations.
    /// When a user pushes the Dexis Integrator button to launch Dexis the Remote Desktop window is minimized and Dexis is opened on the client workstation with the selected patient.
    /// The patient data is sent via DDE commands, NOT using an infofile.txt file to communicate between server and client.
    /// The Dexis Integrator can also be used by multi-office installations using multiple image db's.
    /// </summary>
    public static class DexisIntegrator
	{
		...
		/// <summary>
		/// Sends data for pat to the Dexis Integrator program using a DDE Execute command.
		/// Shows a message box if pat is null instructing user to first select a pat.
		/// Sends SET command with the following format: SET 17 LN="LName" FN="FName" MI="MiddleI" BD=19760205.
		/// The first parameter of the command is the patient ID defined by the program property as either PatNum or ChartNumber.
		/// The BD portion will only be added if the pat has a valid bday and adding it to the commmand won't increase the length of the command to >255 characters.
		/// </summary>
		public static void SendData(Program progCur, Patient pat)
		{
			...
			try
			{
				...
				if (ProgramProperties.GetPropVal(progCur.Id, "Enter 0 to use PatientNum, or 1 to use ChartNum") == "1")
				{
		...
		/// <summary>
		/// This event will be added to the PatientChangedEvent.Fired handler when the DexisIntegrator button is pressed. 
		/// When this fires we will send the command to Dexis Integrator to change pats if the connection is still active and the program link is still enabled. 
		/// If the program link is no longer enabled we will unsubscribe this event from the PatientChangedEvent.Fired handler and dispose of _client.
		/// If the PatNum tagged to ODEventArgs e is invalid we will also dispose of _client so the user will have to press the button again to launch.
		/// </summary>
