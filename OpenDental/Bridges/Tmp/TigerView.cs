...
namespace OpenDental.Bridges
{
    public static class TigerView
	{
		...
		private static bool WritePrivatePofileString2(string section, Hashtable keyVals, string filePath)
		{
			bool rVal = true;
			StreamReader sr = null;
			StreamWriter sw = null;
			List<string> Before_Section_Collection = new List<string>();
			List<string> Section_Collection = new List<string>();
			List<string> After_Section_Collection = new List<string>();
			...
			catch
			{
				if (File.Exists(backupFileName))
				{
					File.Copy(backupFileName, filePath); // restore backup
				}
		...
		public static void SendData(Program ProgramCur, Patient pat)
		{
			...
			iniFile = PPCur.Value;
			Hashtable htKeyVals = new Hashtable();
			if (pat != null)
			{
		...
		public static void StartFileWatcher()
		{
			...
			List<ProgramProperty> propertiesForProgram = ProgramProperties.GetForProgram(prog.Id);
