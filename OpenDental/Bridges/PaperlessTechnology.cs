...
{
    /// <summary>
    /// PT Dental.  www.gopaperlessnow.com.
    /// This bridge only works on Windows and in English, so some shortcuts were taken.
    /// </summary>
    public static class PaperlessTechnology
	{
		private const string exportAddCsv = "addpatient_OD.csv";
		private const string exportUpdateCsv = "updatepatient_OD.csv";
		private const string importCsv = "patientinfo_PT.csv";
		private const string importMedCsv = "patientmedalerts.csv";
		private const string exportAddExe = "addpatient_OD.exe";
		private const string exportUpdateExe = "updatepatient_OD.exe";
		private const string dir = @"C:\PT\USI";
		...
		/// <Summary>
		/// There might be incoming files that we have to watch for. 
		/// They will get processed and deleted.
		/// There is no user interface for this function.
		/// This method is called when OD first starts up.
		/// </Summary>
		...
		/// <summary>
		/// Sends data for Patient.Cur to an export file and then launches an exe to notify PT.
		/// If patient exists, this simply opens the patient.
		/// If patient does not exist, then this triggers creation of the patient in PT Dental.
		/// If isUpdate is true, then the export file and exe will have different names.
		/// In PT, update is a separate programlink with a separate button.
		/// </summary>
