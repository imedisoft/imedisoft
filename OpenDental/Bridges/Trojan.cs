...
namespace OpenDental.Bridges
{
    public static class Trojan
	{
			...
			if (deletePatientRecords.Count > 0)
			{
				...
				if (pendingDeletionTable.Rows.Count > 0)
				{
                    FormPrintReport fpr = new FormPrintReport
                    {
                        ...
                        ScrollAmount = 10,
                        printGenerator = ShowPendingDeletionReportForPatients
                    };
			...
			if (deleteTrojanRecords.Count > 0)
			{
				...
				if (pendingDeletionTableTrojan.Rows.Count > 0)
				{
                    FormPrintReport fpr = new FormPrintReport
                    {
                        ...
                        ScrollAmount = 10,
                        printGenerator = ShowPendingDeletionReportForTrojan
                    };
                    fpr.UsePageNumbers(new Font(FontFamily.GenericMonospace, 8));
					fpr.MinimumTimesToPrint = 1;
					fpr.Landscape = true;
		...
		/// <summary>
		/// Returns number of subscribers affected.
		/// Can throw an exception if user clicks cancel in a note box.
		/// </summary>
			...
			for (int i = 0; i < lines.Length; i++)
			{
				...
				switch (fields[0])
				{
					...
					case "PLANMAX"://eg $3000 per person per year
						...
						if (CovCats.GetCount(true) > 0)
						{
                            ben = new Benefit
                            {
                                ...
                                CovCatNum = CovCats.GetForEbenCat(EbenefitCategory.General).CovCatNum,
                                MonetaryAmt = PIn.Double(fields[2]),
					...
					case "DEDUCT"://eg There is no deductible
                        ...
                        ben = new Benefit
                        {
                            BenefitType = InsBenefitType.Deductible,
                            CovCatNum = CovCats.GetForEbenCat(EbenefitCategory.General).CovCatNum,
                            TimePeriod = BenefitTimePeriod.CalendarYear,
                            MonetaryAmt = amt,
                            CoverageLevel = BenefitCoverageLevel.Individual
                        ...
                        {
                            BenefitType = InsBenefitType.Deductible,
                            CovCatNum = CovCats.GetForEbenCat(EbenefitCategory.Diagnostic).CovCatNum,
                            TimePeriod = BenefitTimePeriod.CalendarYear,
                            MonetaryAmt = 0,//amt;
                            CoverageLevel = BenefitCoverageLevel.Individual
                        };
                        ...
                        {
                            BenefitType = InsBenefitType.Deductible,
                            CovCatNum = CovCats.GetForEbenCat(EbenefitCategory.RoutinePreventive).CovCatNum,
                            TimePeriod = BenefitTimePeriod.CalendarYear,
                            MonetaryAmt = 0,//amt;
                            CoverageLevel = BenefitCoverageLevel.Individual
                        };
					...
					case "PREV"://eg 100% or 'Incentive begins at 70%' or '80% Endo Major see notes'
                        ...
                        ben = new Benefit
                        {
                            BenefitType = InsBenefitType.CoInsurance,
                            CovCatNum = CovCats.GetForEbenCat(EbenefitCategory.Diagnostic).CovCatNum,
                            Percent = percent,
                            TimePeriod = BenefitTimePeriod.CalendarYear
                        };
					...
					case "BASIC":
                        ...
                        {
                            BenefitType = InsBenefitType.CoInsurance,
                            CovCatNum = CovCats.GetForEbenCat(EbenefitCategory.Restorative).CovCatNum,
                            Percent = percent,
                            TimePeriod = BenefitTimePeriod.CalendarYear
                        };
                        troj.BenefitList.Add(ben.Copy());
                        ...
                        {
                            BenefitType = InsBenefitType.CoInsurance,
                            CovCatNum = CovCats.GetForEbenCat(EbenefitCategory.Endodontics).CovCatNum,
                            Percent = percent,
                            TimePeriod = BenefitTimePeriod.CalendarYear
                        };
                        troj.BenefitList.Add(ben.Copy());
                        ...
                        {
                            BenefitType = InsBenefitType.CoInsurance,
                            CovCatNum = CovCats.GetForEbenCat(EbenefitCategory.Periodontics).CovCatNum,
                            Percent = percent,
                            TimePeriod = BenefitTimePeriod.CalendarYear
                        };
                        troj.BenefitList.Add(ben.Copy());
                        ...
                        {
                            BenefitType = InsBenefitType.CoInsurance,
                            CovCatNum = CovCats.GetForEbenCat(EbenefitCategory.OralSurgery).CovCatNum,
                            Percent = percent,
                            TimePeriod = BenefitTimePeriod.CalendarYear
                        };
                        troj.BenefitList.Add(ben.Copy());
					...
					case "MAJOR":
                        ...
                        {
                            BenefitType = InsBenefitType.CoInsurance,
                            CovCatNum = CovCats.GetForEbenCat(EbenefitCategory.Prosthodontics).CovCatNum,
                            Percent = percent,
                            TimePeriod = BenefitTimePeriod.CalendarYear
                        };
                        troj.BenefitList.Add(ben.Copy());
					...
					case "ORMAX"://eg $3500 lifetime
						...
						if (CovCats.GetCount(true) > 0)
						{
                            ...
                            {
                                BenefitType = InsBenefitType.Limitations,
                                CovCatNum = CovCats.GetForEbenCat(EbenefitCategory.Orthodontics).CovCatNum,
                                MonetaryAmt = PIn.Double(fields[2]),
                                TimePeriod = BenefitTimePeriod.CalendarYear
                            };
                            troj.BenefitList.Add(ben.Copy());
		...
		/// <summary>
		/// Takes a string percentage and returns the integer value.
