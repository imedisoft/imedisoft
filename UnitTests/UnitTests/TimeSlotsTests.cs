...
namespace UnitTests.TimeSlots_Tests {
	...
	public class TimeSlotsTests:TestBase {
		...
		///<summary>(WSNPA Integration Test) Verifies that WSNP Appt Types that are restricted to blockouts are only provided timeslots that contain those blockouts.
		...
		///blockouts.</summary>
		[TestMethod]
		public void TimeSlots_GetAvailableNewPatApptTimeSlots_IntegrationTestForRestrictingNewPatApptsToBlockouts() {
			string suffix=MethodBase.GetCurrentMethod().Name;
			string timePattern="XXXXXXXXXXXX"; //1 hour time pattern
			DateTime tomorrow=DateTime.Today.AddDays(1);
			...
			Def restrictedBlockout=DefT.CreateDefinition(DefCat.BlockoutTypes,suffix+"restricted");
			...
			Schedule restrictedBlockoutSched=ScheduleT.CreateSchedule(tomorrow,
				...
				blockoutType:restrictedBlockout.DefNum,
				listOpNums:new List<long>(){ op.OperatoryNum });
			...
			Schedule openSchedule=ScheduleT.CreateSchedule(tomorrow,
				...
				provNum:prov,
				listOpNums:new List<long>(){ op.OperatoryNum });
			List<TimeSlot> listAvailableTimeSlots=TimeSlots.GetAvailableNewPatApptTimeSlots(DateTime.Today.AddDays(1),DateTime.Today.AddDays(2),0,wsnpaDefA.DefNum);
			//Assert that the only available time slot is from 3 - 4 when searching for restricted blockouts
			Assert.AreEqual(1,listAvailableTimeSlots.Count);
			Assert.IsTrue(listAvailableTimeSlots.All(x => x.DateTimeStart==tomorrow.AddHours(3)
				&& x.DateTimeStop==tomorrow.AddHours(4)
				&& x.OperatoryNum==op.OperatoryNum
				&& x.ProvNum==prov
				&& x.DefNumApptType==wsnpaDefA.DefNum));
			listAvailableTimeSlots=TimeSlots.GetAvailableNewPatApptTimeSlots(DateTime.Today.AddDays(1),DateTime.Today.AddDays(2),0,wsnpaDefB.DefNum);
			//Assert that all day, except from 3 - 4 and 7 - 8, is available when not searching for restricted blockouts
			Assert.AreEqual(21,listAvailableTimeSlots.Count);//Prov scheduled for 23 hours, two hour blocks are not available, thus 21 open time slots expected.
			Assert.IsFalse(listAvailableTimeSlots.Any(x => ((x.DateTimeStart==tomorrow.AddHours(3) && x.DateTimeStop==tomorrow.AddHours(4))
