...
//todo: change name to SortingAllowByColumn, but it will affect many files
		...
		private void DrawRows(Graphics g) {
			...
			for(int i=0;i<ListGridRows.Count;i++) {
				...
				if(-vScroll.Value+1+DpiScale(_heightTitle+_heightHeader)+ListGridRows[i].State.YPos > Height) {
...
//todo: brushes aren't disposed
		...
		private void DrawHeaders(Graphics g) {
			...
			g.FillRectangle(_brushHeaderBackground,1,DpiScale(_heightTitle)+1,Width,DpiScale(_heightHeader));//background
			g.DrawLine(_penGridInnerLine,0,DpiScale(_heightTitle),Width,DpiScale(_heightTitle));//line between title and headers
			...
			for(int i=0;i<ListGridColumns.Count;i++) {
				if(i!=0) {
					...
					g.DrawLine(_penColumnSeparator,-hScroll.Value+ListGridColumns[i].State.XPos,DpiScale(_heightTitle)+3,
						-hScroll.Value+ListGridColumns[i].State.XPos,OriginY()-2);
					g.DrawLine(new Pen(Color.White),-hScroll.Value+ListGridColumns[i].State.XPos+1,DpiScale(_heightTitle)+3,
			...
			for(int i=0;i<ListGridColumns.Count;i++) {
				if(SortedByColumnIdx == i) {
					PointF p=new PointF(-hScroll.Value+ListGridColumns[i].State.XPos+6,DpiScaleF(_heightTitle+_heightHeader/2f));
		...
		protected override void OnClick(EventArgs e) {
			...
			if(HasAddButton){ 
				if(((MouseEventArgs)e).X>=Width-DpiScale(_heightTitle)-5 && ((MouseEventArgs)e).Y<=DpiScale(_heightTitle)){
					OnTitleAddClick();
				}
			}
			if(_hasDropDowns
		...
		protected override void OnMouseDown(MouseEventArgs e) {
			...
			_isMouseDown=true;
			if(e.Y < 1+DpiScale(_heightTitle)) {//mouse down was in the title section
				return;
			}
		...
		private void CreateEditBox() {
			...
			textBoxEdit.Text=ListGridRows[_selectedCell.Y].Cells[_selectedCell.X].Text;
		...
		void editBox_KeyUp(object sender,KeyEventArgs e) {
			...
			g.Dispose();
			if(heightText < DpiScale(EDITABLE_ROW_HEIGHT)) {//if it's less than one line
			  heightText=DpiScale(EDITABLE_ROW_HEIGHT);//set it to one line
			}
			if(heightText>textBoxEdit.Height+1) {//it needs to grow
		...
		void editBox_TextChanged(object sender,EventArgs e) {
			if(textBoxEdit!=null) {
				ListGridRows[_selectedCell.Y].Cells[_selectedCell.X].Text=textBoxEdit.Text;
			}
			OnCellTextChanged();
		}
		...
		private void LayoutScrollsAndFonts() {
			...
			_fontHeader?.Dispose();
			_fontTitle?.Dispose();
			_brushTitleBackground=new LinearGradientBrush(new Point(0,0),new Point(0,DpiScale(18)),_colorTitleTop,_colorTitleBottom);
		...
		private void ComputeRows(Graphics g) {
			...
			for(int i=0;i<ListGridColumns.Count;i++) {
				...
				if(ListGridColumns[i].ImageList!=null) {
					if(DpiScale(ListGridColumns[i].ImageList.ImageSize.Height)> heightImage) {
						heightImage=DpiScale(ListGridColumns[i].ImageList.ImageSize.Height)+1;
					}
				}
			}
			int yPos=0;
			for(int i=0;i<ListGridRows.Count;i++) {
				...
				if(WrapText) {//make row taller after measuring
					...
					if(_hasEditableColumn) {
						ListGridRows[i].State.HeightMain=DpiScale(EDITABLE_ROW_HEIGHT);
					}
					else {
						ListGridRows[i].State.HeightMain=font.Height;
					}
				...
				if(widthNote>0 && ListGridRows[i].Note!="") {
					ListGridRows[i].State.HeightNote=(int)g.MeasureString(ListGridRows[i].Note,fontNormal,widthNote,_stringFormat).Height;//Notes cannot be bold.  Always normal font.
				}
		...
		private void CreateComboBox() {
			...
			GridColumn odGridColumn=ListGridColumns[_selectedCell.X];
		...
		private int OriginY(){
			return DpiScale(_heightTitle+_heightHeader);
		}
