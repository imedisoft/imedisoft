...
{
    /// <summary>
    /// Some cache classes need to have a dictionary for faster lookup capabilities and do not want to use a unique key (PK).
    ...
    /// This class was created in order to preserve old behavior for several of our caches that don't care if the dict ismissing data at times.
    /// </summary>
    public abstract class CacheDictNonPkAbs<T, KEY_TYPE, VALUE_TYPE> : CacheDictAbs<T, KEY_TYPE, VALUE_TYPE> where T : TableBase
	{
		...
		/// <summary>
		/// The list of the actual cached objects.
		/// Will always contain every possible cache item.
		...
		/// The base dict cannot be trusted when multiple Ts that share the same key. 
		/// E.g. multiple procedure codes with the same ProcCode.
		/// </summary>
		...
		/// <summary>
		/// Several methods that manipulate _listAllItems need the "short" version sometimes. 
		/// This helper method simply saves code.
		/// Calling method must already be in a Read or Write lock, if not then this method will throw an exception.
		/// </summary>
		...
		/// Use this method when you want to guarantee a correct answer from the cache and are unsure if the dictionary can be trusted.
		/// </summary>
		...
		/// <summary>
		/// If you need to override this method then you should probably be extending CacheDictAbs instead of CacheDictNonPkAbs.
		...
		/// The base method would throw an exception if there were multiple Ts that shared the same key which is why this class exists.
		/// </summary>
		...
		/// <summary>
		/// The Non PK version of the CacheDictAbs needs to return the list of all cached items instead of the dictionary because of duplicates.
		/// If the dictionary was utilized then clients would not get all available items from the database (only the web service would have them)
		/// </summary>
