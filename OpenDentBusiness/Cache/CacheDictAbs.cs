...
namespace OpenDentBusiness
{
    ...
    public abstract class CacheDictAbs<T, KEY_TYPE, VALUE_TYPE> : CacheAbs<T> where T : TableBase
	{
		...
		/// </summary>
		protected virtual VALUE_TYPE GetShortValue(VALUE_TYPE value) => value;
		...
		/// <summary>
		/// Returns true if _dictAllItems is null; otherwise, false.
		/// </summary>
		public bool DictIsNull() => IsCacheNull();
		...
		/// </summary>
		protected virtual List<KEY_TYPE> GetDictShortKeys(List<T> allItems) 
			=> allItems
				.FindAll(
					x => IsInDictShort(x))
				.Select(
		...
		/// Optionally set isShort true to check short version of cache only. 
		/// If value not found then returns empty list.
		/// </summary>
		...
		/// This has been sealed at this level on purpose so that no concreted implementers can override its behavior.
		/// </summary>
		...
		/// <summary>
		/// No longer sealed so that CacheDictNonPkAbs can override.  Once we get rid of the Non PK version this can be resealed.
		/// </summary>
