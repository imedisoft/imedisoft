...
namespace OpenDentBusiness.Crud{
	public class CreditCardCrud {
		...
		public static CreditCard SelectOne(long creditCardNum) {
			...
			List<CreditCard> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(CreditCard creditCard,bool useExistingPK) {
			...
			var paramProcedures = new MySqlParameter("paramProcedures", POut.StringParam(creditCard.Procedures));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramProcedures);
			}
			else {
				creditCard.CreditCardNum=Database.ExecuteInsert(command,paramProcedures);
			}
