...
namespace OpenDentBusiness.Crud{
	public class EmailMessageUidCrud {
		...
		public static EmailMessageUid SelectOne(long emailMessageUidNum) {
			...
			List<EmailMessageUid> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(EmailMessageUid emailMessageUid,bool useExistingPK) {
			...
			var paramMsgId = new MySqlParameter("paramMsgId", POut.StringParam(emailMessageUid.MsgId));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramMsgId);
			}
			else {
				emailMessageUid.EmailMessageUidNum=Database.ExecuteInsert(command,paramMsgId);
			}
