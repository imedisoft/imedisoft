...
namespace OpenDentBusiness.Crud{
	public class LoincCrud {
		...
		public static Loinc SelectOne(long loincNum) {
			...
			List<Loinc> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(Loinc loinc,bool useExistingPK) {
			...
			var paramExternalCopyrightNotice = new MySqlParameter("paramExternalCopyrightNotice", POut.StringParam(loinc.ExternalCopyrightNotice));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramExternalCopyrightNotice);
			}
			else {
				loinc.LoincNum=Database.ExecuteInsert(command,paramExternalCopyrightNotice);
			}
