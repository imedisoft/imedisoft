...
namespace OpenDentBusiness.Crud{
	public class InterventionCrud {
		...
		public static Intervention SelectOne(long interventionNum) {
			...
			List<Intervention> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(Intervention intervention,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(intervention.Note));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote);
			}
			else {
				intervention.InterventionNum=Database.ExecuteInsert(command,paramNote);
			}
