...
namespace OpenDentBusiness.Crud{
	public class ElectIDCrud {
		...
		public static ElectID SelectOne(long electIDNum) {
			...
			List<ElectID> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(ElectID electID,bool useExistingPK) {
			...
			var paramComments = new MySqlParameter("paramComments", POut.StringParam(electID.Comments));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramComments);
			}
			else {
				electID.ElectIDNum=Database.ExecuteInsert(command,paramComments);
			}
