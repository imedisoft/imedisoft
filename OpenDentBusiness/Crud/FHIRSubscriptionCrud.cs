...
namespace OpenDentBusiness.Crud{
	public class FHIRSubscriptionCrud {
		...
		public static FHIRSubscription SelectOne(long fHIRSubscriptionNum) {
			...
			List<FHIRSubscription> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(FHIRSubscription fHIRSubscription,bool useExistingPK) {
			...
			var paramErrorNote = new MySqlParameter("paramErrorNote", POut.StringParam(fHIRSubscription.ErrorNote));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramErrorNote);
			}
			else {
				fHIRSubscription.FHIRSubscriptionNum=Database.ExecuteInsert(command,paramErrorNote);
			}
