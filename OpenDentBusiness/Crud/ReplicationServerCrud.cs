...
namespace OpenDentBusiness.Crud{
	public class ReplicationServerCrud {
		...
		public static ReplicationServer SelectOne(long replicationServerNum) {
			...
			List<ReplicationServer> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(ReplicationServer replicationServer,bool useExistingPK) {
			...
			var paramDescript = new MySqlParameter("paramDescript", POut.StringParam(replicationServer.Descript));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramDescript);
			}
			else {
				replicationServer.ReplicationServerNum=Database.ExecuteInsert(command,paramDescript);
			}
