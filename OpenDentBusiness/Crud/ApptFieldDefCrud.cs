...
namespace OpenDentBusiness.Crud{
	public class ApptFieldDefCrud {
		...
		public static ApptFieldDef SelectOne(long apptFieldDefNum) {
			...
			List<ApptFieldDef> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(ApptFieldDef apptFieldDef,bool useExistingPK) {
			...
			var paramPickList = new MySqlParameter("paramPickList", POut.StringParam(apptFieldDef.PickList));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramPickList);
			}
			else {
				apptFieldDef.ApptFieldDefNum=Database.ExecuteInsert(command,paramPickList);
			}
