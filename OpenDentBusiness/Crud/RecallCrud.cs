...
namespace OpenDentBusiness.Crud{
	public class RecallCrud {
		...
		public static Recall SelectOne(long recallNum) {
			...
			List<Recall> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(Recall recall,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(recall.Note));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote);
			}
			else {
				recall.RecallNum=Database.ExecuteInsert(command,paramNote);
			}
