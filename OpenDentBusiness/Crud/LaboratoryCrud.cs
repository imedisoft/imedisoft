...
namespace OpenDentBusiness.Crud{
	public class LaboratoryCrud {
		...
		public static Laboratory SelectOne(long laboratoryNum) {
			...
			List<Laboratory> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(Laboratory laboratory,bool useExistingPK) {
			...
			var paramNotes = new MySqlParameter("paramNotes", POut.StringParam(laboratory.Notes));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNotes);
			}
			else {
				laboratory.LaboratoryNum=Database.ExecuteInsert(command,paramNotes);
			}
