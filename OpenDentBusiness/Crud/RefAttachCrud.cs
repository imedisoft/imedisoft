...
namespace OpenDentBusiness.Crud{
	public class RefAttachCrud {
		...
		public static RefAttach SelectOne(long refAttachNum) {
			...
			List<RefAttach> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(RefAttach refAttach,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(refAttach.Note));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote);
			}
			else {
				refAttach.RefAttachNum=Database.ExecuteInsert(command,paramNote);
			}
