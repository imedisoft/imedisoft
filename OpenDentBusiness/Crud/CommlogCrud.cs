...
namespace OpenDentBusiness.Crud{
	public class CommlogCrud {
		...
		public static Commlog SelectOne(long commlogNum) {
			...
			List<Commlog> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(Commlog commlog,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringNote(commlog.Note));
			if(commlog.Signature==null) {
				commlog.Signature="";
			}
			var paramSignature = new MySqlParameter("paramSignature", POut.StringParam(commlog.Signature));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote,paramSignature);
			}
			else {
				commlog.CommlogNum=Database.ExecuteInsert(command,paramNote,paramSignature);
			}
