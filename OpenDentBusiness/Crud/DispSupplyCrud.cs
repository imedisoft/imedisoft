...
namespace OpenDentBusiness.Crud{
	public class DispSupplyCrud {
		...
		public static DispSupply SelectOne(long dispSupplyNum) {
			...
			List<DispSupply> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(DispSupply dispSupply,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringNote(dispSupply.Note));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote);
			}
			else {
				dispSupply.DispSupplyNum=Database.ExecuteInsert(command,paramNote);
			}
