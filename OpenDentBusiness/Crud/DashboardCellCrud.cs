...
namespace OpenDentBusiness.Crud{
	public class DashboardCellCrud {
		...
		public static DashboardCell SelectOne(long dashboardCellNum) {
			...
			List<DashboardCell> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(DashboardCell dashboardCell,bool useExistingPK) {
			...
			var paramCellSettings = new MySqlParameter("paramCellSettings", POut.StringParam(dashboardCell.CellSettings));
			if(dashboardCell.LastQueryData==null) {
				dashboardCell.LastQueryData="";
			}
			var paramLastQueryData = new MySqlParameter("paramLastQueryData", POut.StringParam(dashboardCell.LastQueryData));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramCellSettings,paramLastQueryData);
			}
			else {
				dashboardCell.DashboardCellNum=Database.ExecuteInsert(command,paramCellSettings,paramLastQueryData);
			}
