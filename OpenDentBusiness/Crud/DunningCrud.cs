...
namespace OpenDentBusiness.Crud{
	public class DunningCrud {
		...
		public static Dunning SelectOne(long dunningNum) {
			...
			List<Dunning> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(Dunning dunning,bool useExistingPK) {
			...
			var paramDunMessage = new MySqlParameter("paramDunMessage", POut.StringParam(dunning.DunMessage));
			if(dunning.MessageBold==null) {
				dunning.MessageBold="";
			}
			var paramMessageBold = new MySqlParameter("paramMessageBold", POut.StringParam(dunning.MessageBold));
			if(dunning.EmailBody==null) {
				dunning.EmailBody="";
			}
			var paramEmailBody = new MySqlParameter("paramEmailBody", POut.StringParam(dunning.EmailBody));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramDunMessage,paramMessageBold,paramEmailBody);
			}
			else {
				dunning.DunningNum=Database.ExecuteInsert(command,paramDunMessage,paramMessageBold,paramEmailBody);
			}
