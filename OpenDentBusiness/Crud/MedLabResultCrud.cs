...
namespace OpenDentBusiness.Crud{
	public class MedLabResultCrud {
		...
		public static MedLabResult SelectOne(long medLabResultNum) {
			...
			List<MedLabResult> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(MedLabResult medLabResult,bool useExistingPK) {
			...
			var paramObsValue = new MySqlParameter("paramObsValue", POut.StringParam(medLabResult.ObsValue));
			if(medLabResult.Note==null) {
				medLabResult.Note="";
			}
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(medLabResult.Note));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramObsValue,paramNote);
			}
			else {
				medLabResult.MedLabResultNum=Database.ExecuteInsert(command,paramObsValue,paramNote);
			}
