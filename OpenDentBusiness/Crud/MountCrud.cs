...
namespace OpenDentBusiness.Crud{
	public class MountCrud {
		...
		public static Mount SelectOne(long mountNum) {
			...
			List<Mount> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(Mount mount,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(mount.Note));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote);
			}
			else {
				mount.MountNum=Database.ExecuteInsert(command,paramNote);
			}
