...
namespace OpenDentBusiness.Crud{
	public class LetterCrud {
		...
		public static Letter SelectOne(long letterNum) {
			...
			List<Letter> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(Letter letter,bool useExistingPK) {
			...
			var paramBodyText = new MySqlParameter("paramBodyText", POut.StringParam(letter.BodyText));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramBodyText);
			}
			else {
				letter.LetterNum=Database.ExecuteInsert(command,paramBodyText);
			}
