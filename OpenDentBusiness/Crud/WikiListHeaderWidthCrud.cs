...
namespace OpenDentBusiness.Crud{
	public class WikiListHeaderWidthCrud {
		...
		public static WikiListHeaderWidth SelectOne(long wikiListHeaderWidthNum) {
			...
			List<WikiListHeaderWidth> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(WikiListHeaderWidth wikiListHeaderWidth,bool useExistingPK) {
			...
			var paramPickList = new MySqlParameter("paramPickList", POut.StringParam(wikiListHeaderWidth.PickList));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramPickList);
			}
			else {
				wikiListHeaderWidth.WikiListHeaderWidthNum=Database.ExecuteInsert(command,paramPickList);
			}
