...
namespace OpenDentBusiness.Crud{
	public class SignalodCrud {
		...
		public static Signalod SelectOne(long signalNum) {
			...
			List<Signalod> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(Signalod signalod,bool useExistingPK) {
			...
			var paramMsgValue = new MySqlParameter("paramMsgValue", POut.StringParam(signalod.MsgValue));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramMsgValue);
			}
			else {
				signalod.SignalNum=Database.ExecuteInsert(command,paramMsgValue);
			}
