...
namespace OpenDentBusiness.Crud{
	public class PayorTypeCrud {
		...
		public static PayorType SelectOne(long payorTypeNum) {
			...
			List<PayorType> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(PayorType payorType,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(payorType.Note));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote);
			}
			else {
				payorType.PayorTypeNum=Database.ExecuteInsert(command,paramNote);
			}
