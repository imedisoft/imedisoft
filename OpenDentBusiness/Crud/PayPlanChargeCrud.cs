...
namespace OpenDentBusiness.Crud{
	public class PayPlanChargeCrud {
		...
		public static PayPlanCharge SelectOne(long payPlanChargeNum) {
			...
			List<PayPlanCharge> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(PayPlanCharge payPlanCharge,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(payPlanCharge.Note));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote);
			}
			else {
				payPlanCharge.PayPlanChargeNum=Database.ExecuteInsert(command,paramNote);
			}
