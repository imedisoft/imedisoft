...
namespace OpenDentBusiness.Crud{
	public class DiseaseCrud {
		...
		public static Disease SelectOne(long diseaseNum) {
			...
			List<Disease> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(Disease disease,bool useExistingPK) {
			...
			var paramPatNote = new MySqlParameter("paramPatNote", POut.StringParam(disease.PatNote));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramPatNote);
			}
			else {
				disease.DiseaseNum=Database.ExecuteInsert(command,paramPatNote);
			}
