...
namespace OpenDentBusiness.Crud{
	public class SmsFromMobileCrud {
		...
		public static SmsFromMobile SelectOne(long smsFromMobileNum) {
			...
			List<SmsFromMobile> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(SmsFromMobile smsFromMobile,bool useExistingPK) {
			...
			var paramMsgText = new MySqlParameter("paramMsgText", POut.StringNote(smsFromMobile.MsgText));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramMsgText);
			}
			else {
				smsFromMobile.SmsFromMobileNum=Database.ExecuteInsert(command,paramMsgText);
			}
