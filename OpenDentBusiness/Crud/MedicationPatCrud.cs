...
namespace OpenDentBusiness.Crud{
	public class MedicationPatCrud {
		...
		public static MedicationPat SelectOne(long medicationPatNum) {
			...
			List<MedicationPat> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(MedicationPat medicationPat,bool useExistingPK) {
			...
			var paramPatNote = new MySqlParameter("paramPatNote", POut.StringParam(medicationPat.PatNote));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramPatNote);
			}
			else {
				medicationPat.MedicationPatNum=Database.ExecuteInsert(command,paramPatNote);
			}
