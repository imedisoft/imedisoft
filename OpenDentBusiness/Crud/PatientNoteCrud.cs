...
namespace OpenDentBusiness.Crud{
	public class PatientNoteCrud {
		...
		public static PatientNote SelectOne(long patNum) {
			...
			List<PatientNote> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(PatientNote patientNote,bool useExistingPK) {
			...
			var paramFamFinancial = new MySqlParameter("paramFamFinancial", POut.StringNote(patientNote.FamFinancial));
			if(patientNote.ApptPhone==null) {
				patientNote.ApptPhone="";
			}
			var paramApptPhone = new MySqlParameter("paramApptPhone", POut.StringParam(patientNote.ApptPhone));
			if(patientNote.Medical==null) {
				patientNote.Medical="";
			}
			var paramMedical = new MySqlParameter("paramMedical", POut.StringNote(patientNote.Medical));
			if(patientNote.Service==null) {
				patientNote.Service="";
			}
			var paramService = new MySqlParameter("paramService", POut.StringNote(patientNote.Service));
			if(patientNote.MedicalComp==null) {
				patientNote.MedicalComp="";
			}
			var paramMedicalComp = new MySqlParameter("paramMedicalComp", POut.StringNote(patientNote.MedicalComp));
			if(patientNote.Treatment==null) {
				patientNote.Treatment="";
			}
			var paramTreatment = new MySqlParameter("paramTreatment", POut.StringNote(patientNote.Treatment));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramFamFinancial,paramApptPhone,paramMedical,paramService,paramMedicalComp,paramTreatment);
			}
			else {
				patientNote.PatNum=Database.ExecuteInsert(command,paramFamFinancial,paramApptPhone,paramMedical,paramService,paramMedicalComp,paramTreatment);
			}
