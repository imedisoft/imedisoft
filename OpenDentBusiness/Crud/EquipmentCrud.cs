...
namespace OpenDentBusiness.Crud{
	public class EquipmentCrud {
		...
		public static Equipment SelectOne(long equipmentNum) {
			...
			List<Equipment> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(Equipment equipment,bool useExistingPK) {
			...
			var paramDescription = new MySqlParameter("paramDescription", POut.StringParam(equipment.Description));
			if(equipment.Location==null) {
				equipment.Location="";
			}
			var paramLocation = new MySqlParameter("paramLocation", POut.StringParam(equipment.Location));
			if(equipment.DispenseNote==null) {
				equipment.DispenseNote="";
			}
			var paramDispenseNote = new MySqlParameter("paramDispenseNote", POut.StringNote(equipment.DispenseNote));
			if(equipment.Status==null) {
				equipment.Status="";
			}
			var paramStatus = new MySqlParameter("paramStatus", POut.StringParam(equipment.Status));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramDescription,paramLocation,paramDispenseNote,paramStatus);
			}
			else {
				equipment.EquipmentNum=Database.ExecuteInsert(command,paramDescription,paramLocation,paramDispenseNote,paramStatus);
			}
