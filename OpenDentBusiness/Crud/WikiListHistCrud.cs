...
namespace OpenDentBusiness.Crud{
	public class WikiListHistCrud {
		...
		public static WikiListHist SelectOne(long wikiListHistNum) {
			...
			List<WikiListHist> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
        ...
        public static long Insert(WikiListHist wikiListHist,bool useExistingPK) {
			...
			var paramListHeaders = new MySqlParameter("paramListHeaders", POut.StringParam(wikiListHist.ListHeaders));
			if(wikiListHist.ListContent==null) {
				wikiListHist.ListContent="";
			}
			var paramListContent = new MySqlParameter("paramListContent", POut.StringParam(wikiListHist.ListContent));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramListHeaders,paramListContent);
			}
			else {
				wikiListHist.WikiListHistNum=Database.ExecuteInsert(command,paramListHeaders,paramListContent);
			}
