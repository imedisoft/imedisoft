...
namespace OpenDentBusiness.Crud{
	public class ToothInitialCrud {
		...
		public static ToothInitial SelectOne(long toothInitialNum) {
			...
			List<ToothInitial> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(ToothInitial toothInitial,bool useExistingPK) {
			...
			var paramDrawingSegment = new MySqlParameter("paramDrawingSegment", POut.StringParam(toothInitial.DrawingSegment));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramDrawingSegment);
			}
			else {
				toothInitial.ToothInitialNum=Database.ExecuteInsert(command,paramDrawingSegment);
			}
