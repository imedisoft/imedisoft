...
namespace OpenDentBusiness.Crud{
	public class WebSchedRecallCrud {
		...
		public static WebSchedRecall SelectOne(long webSchedRecallNum) {
			...
			List<WebSchedRecall> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(WebSchedRecall webSchedRecall,bool useExistingPK) {
			...
			var paramMsgTextToMobileTemplate=new MySqlParameter("paramMsgTextToMobileTemplate",POut.StringParam(webSchedRecall.MsgTextToMobileTemplate));
			if(webSchedRecall.MsgTextToMobile==null) {
				webSchedRecall.MsgTextToMobile="";
			}
			var paramMsgTextToMobile=new MySqlParameter("paramMsgTextToMobile",POut.StringParam(webSchedRecall.MsgTextToMobile));
			if(webSchedRecall.EmailSubjTemplate==null) {
				webSchedRecall.EmailSubjTemplate="";
			}
			var paramEmailSubjTemplate=new MySqlParameter("paramEmailSubjTemplate",POut.StringParam(webSchedRecall.EmailSubjTemplate));
			if(webSchedRecall.EmailSubj==null) {
				webSchedRecall.EmailSubj="";
			}
			var paramEmailSubj=new MySqlParameter("paramEmailSubj",POut.StringParam(webSchedRecall.EmailSubj));
			if(webSchedRecall.EmailTextTemplate==null) {
				webSchedRecall.EmailTextTemplate="";
			}
			var paramEmailTextTemplate=new MySqlParameter("paramEmailTextTemplate",POut.StringParam(webSchedRecall.EmailTextTemplate));
			if(webSchedRecall.EmailText==null) {
				webSchedRecall.EmailText="";
			}
			var paramEmailText=new MySqlParameter("paramEmailText",POut.StringParam(webSchedRecall.EmailText));
			if(webSchedRecall.GuidMessageToMobile==null) {
				webSchedRecall.GuidMessageToMobile="";
			}
			var paramGuidMessageToMobile=new MySqlParameter("paramGuidMessageToMobile",POut.StringParam(webSchedRecall.GuidMessageToMobile));
			if(webSchedRecall.ResponseDescript==null) {
				webSchedRecall.ResponseDescript="";
			}
			var paramResponseDescript=new MySqlParameter("paramResponseDescript",POut.StringParam(webSchedRecall.ResponseDescript));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramMsgTextToMobileTemplate,paramMsgTextToMobile,paramEmailSubjTemplate,paramEmailSubj,paramEmailTextTemplate,paramEmailText,paramGuidMessageToMobile,paramResponseDescript);
			}
			else {
				webSchedRecall.WebSchedRecallNum=Database.ExecuteInsert(command,paramMsgTextToMobileTemplate,paramMsgTextToMobile,paramEmailSubjTemplate,paramEmailSubj,paramEmailTextTemplate,paramEmailText,paramGuidMessageToMobile,paramResponseDescript);
			}
