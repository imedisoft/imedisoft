...
namespace OpenDentBusiness.Crud{
	public class VaccinePatCrud {
		...
		public static VaccinePat SelectOne(long vaccinePatNum) {
			...
			List<VaccinePat> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(VaccinePat vaccinePat,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(vaccinePat.Note));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote);
			}
			else {
				vaccinePat.VaccinePatNum=Database.ExecuteInsert(command,paramNote);
			}
