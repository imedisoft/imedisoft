...
namespace OpenDentBusiness.Crud{
	public class AutoNoteControlCrud {
		...
		public static AutoNoteControl SelectOne(long autoNoteControlNum) {
			...
			List<AutoNoteControl> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(AutoNoteControl autoNoteControl,bool useExistingPK) {
			...
			var paramControlOptions = new MySqlParameter("paramControlOptions", POut.StringParam(autoNoteControl.ControlOptions));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramControlOptions);
			}
			else {
				autoNoteControl.AutoNoteControlNum=Database.ExecuteInsert(command,paramControlOptions);
			}
