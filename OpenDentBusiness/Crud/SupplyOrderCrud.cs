...
namespace OpenDentBusiness.Crud{
	public class SupplyOrderCrud {
		...
		public static SupplyOrder SelectOne(long supplyOrderNum) {
			...
			List<SupplyOrder> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(SupplyOrder supplyOrder,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(supplyOrder.Note));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote);
			}
			else {
				supplyOrder.SupplyOrderNum=Database.ExecuteInsert(command,paramNote);
			}
