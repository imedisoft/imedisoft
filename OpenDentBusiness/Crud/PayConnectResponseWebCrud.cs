...
namespace OpenDentBusiness.Crud{
	public class PayConnectResponseWebCrud {
		...
		public static PayConnectResponseWeb SelectOne(long payConnectResponseWebNum) {
			...
			List<PayConnectResponseWeb> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(PayConnectResponseWeb payConnectResponseWeb,bool useExistingPK) {
			...
			var paramLastResponseStr = new MySqlParameter("paramLastResponseStr", POut.StringParam(payConnectResponseWeb.LastResponseStr));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramLastResponseStr);
			}
			else {
				payConnectResponseWeb.PayConnectResponseWebNum=Database.ExecuteInsert(command,paramLastResponseStr);
			}
