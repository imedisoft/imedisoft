...
namespace OpenDentBusiness.Crud{
	public class SiteCrud {
		...
		public static Site SelectOne(long siteNum) {
			...
			List<Site> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(Site site,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(site.Note));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote);
			}
			else {
				site.SiteNum=Database.ExecuteInsert(command,paramNote);
			}
			return site.SiteNum;
		}
