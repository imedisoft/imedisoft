...
namespace OpenDentBusiness.Crud{
	public class DashboardARCrud {
		...
		public static DashboardAR SelectOne(long dashboardARNum) {
			...
			List<DashboardAR> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static List<DashboardAR> SelectMany(string command) {
			...
			var list=TableToList(Database.ExecuteDataTable(command));
			return list;
		}
		...
		public static long Insert(DashboardAR dashboardAR,bool useExistingPK) {
			...
			if(useExistingPK || PrefC.RandomKeys) {
				...
				dashboardAR.DashboardARNum=Database.ExecuteInsert(command);
			}
