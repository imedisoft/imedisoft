...
namespace OpenDentBusiness.Crud{
	public class SecurityLogCrud {
		...
		public static SecurityLog SelectOne(long securityLogNum) {
			...
			List<SecurityLog> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(SecurityLog securityLog,bool useExistingPK) {
			...
			var paramLogText = new MySqlParameter("paramLogText", POut.StringParam(securityLog.LogText));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramLogText);
			}
			else {
				securityLog.SecurityLogNum=Database.ExecuteInsert(command,paramLogText);
			}
