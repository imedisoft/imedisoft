...
namespace OpenDentBusiness.Crud{
	public class TreatPlanCrud {
		...
		public static TreatPlan SelectOne(long treatPlanNum) {
			...
			List<TreatPlan> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(TreatPlan treatPlan,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(treatPlan.Note));
			if(treatPlan.Signature==null) {
				treatPlan.Signature="";
			}
			var paramSignature = new MySqlParameter("paramSignature", POut.StringParam(treatPlan.Signature));
			if(treatPlan.SignaturePractice==null) {
				treatPlan.SignaturePractice="";
			}
			var paramSignaturePractice = new MySqlParameter("paramSignaturePractice", POut.StringParam(treatPlan.SignaturePractice));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote,paramSignature,paramSignaturePractice);
			}
			else {
				treatPlan.TreatPlanNum=Database.ExecuteInsert(command,paramNote,paramSignature,paramSignaturePractice);
			}
