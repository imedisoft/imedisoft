...
namespace OpenDentBusiness.Crud{
	public class PaymentCrud {
		...
		public static Payment SelectOne(long payNum) {
			...
			List<Payment> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(Payment payment,bool useExistingPK) {
			...
			var paramPayNote = new MySqlParameter("paramPayNote", POut.StringNote(payment.PayNote));
			if(payment.Receipt==null) {
				payment.Receipt="";
			}
			var paramReceipt = new MySqlParameter("paramReceipt", POut.StringParam(payment.Receipt));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramPayNote,paramReceipt);
			}
			else {
				payment.PayNum=Database.ExecuteInsert(command,paramPayNote,paramReceipt);
			}
