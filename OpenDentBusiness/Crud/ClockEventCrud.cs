...
namespace OpenDentBusiness.Crud{
	public class ClockEventCrud {
		...
		public static ClockEvent SelectOne(long clockEventNum) {
			...
			List<ClockEvent> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(ClockEvent clockEvent,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(clockEvent.Note));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote);
			}
			else {
				clockEvent.ClockEventNum=Database.ExecuteInsert(command,paramNote);
			}
