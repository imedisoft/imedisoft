...
namespace OpenDentBusiness.Crud{
	public class HL7DefMessageCrud {
		...
		public static HL7DefMessage SelectOne(long hL7DefMessageNum) {
			...
			List<HL7DefMessage> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(HL7DefMessage hL7DefMessage,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(hL7DefMessage.Note));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote);
			}
			else {
				hL7DefMessage.HL7DefMessageNum=Database.ExecuteInsert(command,paramNote);
			}
