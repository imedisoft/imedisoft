...
namespace OpenDentBusiness.Crud{
	public class QuickPasteCatCrud {
		...
		public static QuickPasteCat SelectOne(long quickPasteCatNum) {
			...
			List<QuickPasteCat> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(QuickPasteCat quickPasteCat,bool useExistingPK) {
			...
			var paramDefaultForTypes = new MySqlParameter("paramDefaultForTypes", POut.StringParam(quickPasteCat.DefaultForTypes));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramDefaultForTypes);
			}
			else {
				quickPasteCat.QuickPasteCatNum=Database.ExecuteInsert(command,paramDefaultForTypes);
			}
