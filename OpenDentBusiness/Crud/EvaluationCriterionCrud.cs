...
namespace OpenDentBusiness.Crud{
	public class EvaluationCriterionCrud {
		...
		public static EvaluationCriterion SelectOne(long evaluationCriterionNum) {
			...
			List<EvaluationCriterion> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(EvaluationCriterion evaluationCriterion,bool useExistingPK) {
			...
			var paramNotes = new MySqlParameter("paramNotes", POut.StringParam(evaluationCriterion.Notes));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNotes);
			}
			else {
				evaluationCriterion.EvaluationCriterionNum=Database.ExecuteInsert(command,paramNotes);
			}
