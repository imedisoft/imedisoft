...
namespace OpenDentBusiness.Crud{
	public class TaskListCrud {
		...
		public static TaskList SelectOne(long taskListNum) {
			...
			List<TaskList> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static List<TaskList> TableToList(DataTable table) {
			...
			foreach(DataRow row in table.Rows) {
				...
				taskList.DateTimeEntry       = PIn.Date (row["DateTimeEntry"].ToString());
		...
		public static long Insert(TaskList taskList,bool useExistingPK) {
			...
			if(useExistingPK || PrefC.RandomKeys) {
				...
				taskList.TaskListNum=Database.ExecuteInsert(command);
			}
