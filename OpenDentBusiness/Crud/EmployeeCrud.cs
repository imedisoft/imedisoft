...
namespace OpenDentBusiness.Crud{
	public class EmployeeCrud {
		...
		public static Employee SelectOne(long employeeNum) {
			...
			List<Employee> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(Employee employee,bool useExistingPK) {
			...
			command+="LName,FName,MiddleI,IsHidden,ClockStatus,PhoneExt,PayrollID,WirelessPhone,EmailWork,EmailPersonal,IsFurloughed,IsWorkingHome) VALUES(";
			if(useExistingPK || PrefC.RandomKeys) {
			...
			if(useExistingPK || PrefC.RandomKeys) {
				...
				employee.EmployeeNum=Database.ExecuteInsert(command);
			}
