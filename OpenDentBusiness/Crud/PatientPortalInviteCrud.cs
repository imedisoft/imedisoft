...
namespace OpenDentBusiness.Crud{
	public class PatientPortalInviteCrud {
		...
		public static PatientPortalInvite SelectOne(long patientPortalInviteNum) {
			...
			List<PatientPortalInvite> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(PatientPortalInvite patientPortalInvite,bool useExistingPK) {
			...
			var paramTemplateEmail = new MySqlParameter("paramTemplateEmail", POut.StringParam(patientPortalInvite.TemplateEmail));
			if(patientPortalInvite.Note==null) {
				patientPortalInvite.Note="";
			}
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(patientPortalInvite.Note));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramTemplateEmail,paramNote);
			}
			else {
				patientPortalInvite.PatientPortalInviteNum=Database.ExecuteInsert(command,paramTemplateEmail,paramNote);
			}
