...
namespace OpenDentBusiness.Crud{
	public class EobAttachCrud {
		...
		public static EobAttach SelectOne(long eobAttachNum) {
			...
			List<EobAttach> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(EobAttach eobAttach,bool useExistingPK) {
			...
			var paramRawBase64 = new MySqlParameter("paramRawBase64", POut.StringParam(eobAttach.RawBase64));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramRawBase64);
			}
			else {
				eobAttach.EobAttachNum=Database.ExecuteInsert(command,paramRawBase64);
			}
