...
namespace OpenDentBusiness.Crud{
	public class ProcNoteCrud {
		...
		public static ProcNote SelectOne(long procNoteNum) {
			...
			List<ProcNote> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(ProcNote procNote,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringNote(procNote.Note));
			if(procNote.Signature==null) {
				procNote.Signature="";
			}
			var paramSignature = new MySqlParameter("paramSignature", POut.StringParam(procNote.Signature));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote,paramSignature);
			}
			else {
				procNote.ProcNoteNum=Database.ExecuteInsert(command,paramNote,paramSignature);
			}
