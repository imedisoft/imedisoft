...
namespace OpenDentBusiness.Crud{
	public class PayPlanCrud {
		...
		public static PayPlan SelectOne(long payPlanNum) {
			...
			List<PayPlan> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(PayPlan payPlan,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(payPlan.Note));
			if(payPlan.Signature==null) {
				payPlan.Signature="";
			}
			var paramSignature = new MySqlParameter("paramSignature", POut.StringParam(payPlan.Signature));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote,paramSignature);
			}
			else {
				payPlan.PayPlanNum=Database.ExecuteInsert(command,paramNote,paramSignature);
			}
			...
			if(!useExistingPK && PrefC.RandomKeys) {
				...
				while(index < listPayPlans.Count) {
					...
					if(sbCommands==null) {
						...
						sbCommands.Append("PatNum,Guarantor,PayPlanDate,APR,Note,PlanNum,CompletedAmt,InsSubNum,PaySchedule,NumberOfPayments,PayAmt,DownPayment,IsClosed,Signature,SigIsTopaz,PlanCategory,IsDynamic,ChargeFrequency,DatePayPlanStart,IsLocked,DateInterestStart) VALUES ");
						countRows=0;
					}
					else {
						hasComma=true;
