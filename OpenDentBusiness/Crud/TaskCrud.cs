...
namespace OpenDentBusiness.Crud{
	public class TaskCrud {
		...
		public static Task SelectOne(long taskNum) {
			...
			List<Task> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(Task task,bool useExistingPK) {
			...
			var paramDescript = new MySqlParameter("paramDescript", POut.StringParam(task.Descript));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramDescript);
			}
			else {
				task.TaskNum=Database.ExecuteInsert(command,paramDescript);
			}
			return task.TaskNum;
		}
