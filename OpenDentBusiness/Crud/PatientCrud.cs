...
namespace OpenDentBusiness.Crud
{
    public class PatientCrud {
		...
		public static Patient SelectOne(long patNum) {
			...
			List<Patient> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static List<Patient> TableToList(DataTable table) {
			...
			foreach(DataRow row in table.Rows) {
				...
				patient.PatStatus                = (PatientStatus)PIn.Int(row["PatStatus"].ToString());
		...
		public static long Insert(Patient patient,bool useExistingPK) {
			...
			var paramAddrNote = new MySqlParameter("paramAddrNote", POut.StringNote(patient.AddrNote));
			if(patient.FamFinUrgNote==null) {
				patient.FamFinUrgNote="";
			}
			var paramFamFinUrgNote = new MySqlParameter("paramFamFinUrgNote", POut.StringNote(patient.FamFinUrgNote));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramAddrNote,paramFamFinUrgNote);
			}
			else {
				patient.PatNum=Database.ExecuteInsert(command,paramAddrNote,paramFamFinUrgNote);
			}
