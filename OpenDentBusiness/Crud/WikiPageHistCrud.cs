...
namespace OpenDentBusiness.Crud{
	public class WikiPageHistCrud {
		...
		public static WikiPageHist SelectOne(long wikiPageNum) {
			...
			List<WikiPageHist> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(WikiPageHist wikiPageHist,bool useExistingPK) {
			...
			var paramPageContent = new MySqlParameter("paramPageContent", POut.StringParam(wikiPageHist.PageContent));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramPageContent);
			}
			else {
				wikiPageHist.WikiPageNum=Database.ExecuteInsert(command,paramPageContent);
			}
