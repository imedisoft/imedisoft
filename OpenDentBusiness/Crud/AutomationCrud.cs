...
namespace OpenDentBusiness.Crud{
	public class AutomationCrud {
		...
		public static Automation SelectOne(long automationNum) {
			...
			List<Automation> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(Automation automation,bool useExistingPK) {
			...
			var paramDescription=new MySqlParameter("paramDescription",POut.StringParam(automation.Description));
			if(automation.ProcCodes==null) {
				automation.ProcCodes="";
			}
			var paramProcCodes=new MySqlParameter("paramProcCodes",POut.StringParam(automation.ProcCodes));
			if(automation.MessageContent==null) {
				automation.MessageContent="";
			}
			var paramMessageContent=new MySqlParameter("paramMessageContent",POut.StringParam(automation.MessageContent));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramDescription,paramProcCodes,paramMessageContent);
			}
			else {
				automation.AutomationNum=Database.ExecuteInsert(command,paramDescription,paramProcCodes,paramMessageContent);
			}
