...
namespace OpenDentBusiness.Crud{
	public class AutoNoteCrud {
		...
		public static AutoNote SelectOne(long autoNoteNum) {
			...
			List<AutoNote> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(AutoNote autoNote,bool useExistingPK) {
			...
			var paramMainText = new MySqlParameter("paramMainText", POut.StringParam(autoNote.MainText));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramMainText);
			}
			else {
				autoNote.AutoNoteNum=Database.ExecuteInsert(command,paramMainText);
			}
