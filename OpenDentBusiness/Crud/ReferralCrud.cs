...
namespace OpenDentBusiness.Crud{
	public class ReferralCrud {
		...
		public static Referral SelectOne(long referralNum) {
			...
			List<Referral> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(Referral referral,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(referral.Note));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote);
			}
			else {
				referral.ReferralNum=Database.ExecuteInsert(command,paramNote);
			}
