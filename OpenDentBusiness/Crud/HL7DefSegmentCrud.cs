...
namespace OpenDentBusiness.Crud{
	public class HL7DefSegmentCrud {
		...
		public static HL7DefSegment SelectOne(long hL7DefSegmentNum) {
			...
			List<HL7DefSegment> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(HL7DefSegment hL7DefSegment,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(hL7DefSegment.Note));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote);
			}
			else {
				hL7DefSegment.HL7DefSegmentNum=Database.ExecuteInsert(command,paramNote);
			}
