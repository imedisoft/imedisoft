...
namespace OpenDentBusiness.Crud{
	public class TsiTransLogCrud {
		...
		public static TsiTransLog SelectOne(long tsiTransLogNum) {
			...
			List<TsiTransLog> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(TsiTransLog tsiTransLog,bool useExistingPK) {
			...
			var paramTransJson = new MySqlParameter("paramTransJson", POut.StringParam(tsiTransLog.TransJson));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramTransJson);
			}
			else {
				tsiTransLog.TsiTransLogNum=Database.ExecuteInsert(command,paramTransJson);
			}
