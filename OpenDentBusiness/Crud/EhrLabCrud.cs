...
namespace OpenDentBusiness.Crud
{
    public class EhrLabCrud {
		...
		public static EhrLab SelectOne(long ehrLabNum) {
			...
			List<EhrLab> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(EhrLab ehrLab,bool useExistingPK) {
			...
			var paramOriginalPIDSegment = new MySqlParameter("paramOriginalPIDSegment", POut.StringParam(ehrLab.OriginalPIDSegment));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramOriginalPIDSegment);
			}
			else {
				ehrLab.EhrLabNum=Database.ExecuteInsert(command,paramOriginalPIDSegment);
			}
