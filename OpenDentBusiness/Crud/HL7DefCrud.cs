...
namespace OpenDentBusiness.Crud{
	public class HL7DefCrud {
		...
		public static HL7Def SelectOne(long hL7DefNum) {
			...
			List<HL7Def> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(HL7Def hL7Def,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(hL7Def.Note));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote);
			}
			else {
				hL7Def.HL7DefNum=Database.ExecuteInsert(command,paramNote);
			}
