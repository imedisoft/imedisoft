...
namespace OpenDentBusiness.Crud{
	public class MedicationCrud {
		...
		public static Medication SelectOne(long medicationNum) {
			...
			List<Medication> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(Medication medication,bool useExistingPK) {
			...
			var paramNotes = new MySqlParameter("paramNotes", POut.StringParam(medication.Notes));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNotes);
			}
			else {
				medication.MedicationNum=Database.ExecuteInsert(command,paramNotes);
			}
