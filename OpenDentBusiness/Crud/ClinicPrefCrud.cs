...
namespace OpenDentBusiness.Crud{
	public class ClinicPrefCrud {
		...
		public static ClinicPref SelectOne(long clinicPrefNum) {
			...
			List<ClinicPref> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(ClinicPref clinicPref,bool useExistingPK) {
			...
			var paramValueString = new MySqlParameter("paramValueString", POut.StringParam(clinicPref.ValueString));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramValueString);
			}
			else {
				clinicPref.ClinicPrefNum=Database.ExecuteInsert(command,paramValueString);
			}
