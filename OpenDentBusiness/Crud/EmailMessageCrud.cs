...
namespace OpenDentBusiness.Crud{
	public class EmailMessageCrud {
		...
		public static EmailMessage SelectOne(long emailMessageNum) {
			...
			List<EmailMessage> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(EmailMessage emailMessage,bool useExistingPK) {
			...
			var paramToAddress = new MySqlParameter("paramToAddress", POut.StringParam(emailMessage.ToAddress));
			if(emailMessage.FromAddress==null) {
				emailMessage.FromAddress="";
			}
			var paramFromAddress = new MySqlParameter("paramFromAddress", POut.StringParam(emailMessage.FromAddress));
			if(emailMessage.Subject==null) {
				emailMessage.Subject="";
			}
			var paramSubject = new MySqlParameter("paramSubject", POut.StringParam(emailMessage.Subject));
			if(emailMessage.BodyText==null) {
				emailMessage.BodyText="";
			}
			var paramBodyText = new MySqlParameter("paramBodyText", POut.StringParam(emailMessage.BodyText));
			if(emailMessage.RawEmailIn==null) {
				emailMessage.RawEmailIn="";
			}
			var paramRawEmailIn = new MySqlParameter("paramRawEmailIn", POut.StringParam(emailMessage.RawEmailIn));
			if(emailMessage.CcAddress==null) {
				emailMessage.CcAddress="";
			}
			var paramCcAddress = new MySqlParameter("paramCcAddress", POut.StringParam(emailMessage.CcAddress));
			if(emailMessage.BccAddress==null) {
				emailMessage.BccAddress="";
			}
			var paramBccAddress = new MySqlParameter("paramBccAddress", POut.StringParam(emailMessage.BccAddress));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramToAddress,paramFromAddress,paramSubject,paramBodyText,paramRawEmailIn,paramCcAddress,paramBccAddress);
			}
			else {
				emailMessage.EmailMessageNum=Database.ExecuteInsert(command,paramToAddress,paramFromAddress,paramSubject,paramBodyText,paramRawEmailIn,paramCcAddress,paramBccAddress);
			}
