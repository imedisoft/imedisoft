...
namespace OpenDentBusiness.Crud{
	public class StatementCrud {
		...
		public static Statement SelectOne(long statementNum) {
			...
			List<Statement> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(Statement statement,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(statement.Note));
			if(statement.NoteBold==null) {
				statement.NoteBold="";
			}
			var paramNoteBold = new MySqlParameter("paramNoteBold", POut.StringParam(statement.NoteBold));
			if(statement.EmailBody==null) {
				statement.EmailBody="";
			}
			var paramEmailBody = new MySqlParameter("paramEmailBody", POut.StringParam(statement.EmailBody));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote,paramNoteBold,paramEmailBody);
			}
			else {
				statement.StatementNum=Database.ExecuteInsert(command,paramNote,paramNoteBold,paramEmailBody);
			}
