...
namespace OpenDentBusiness.Crud{
	public class ProgramCrud {
		...
		public static Program SelectOne(long programNum) {
			...
			List<Program> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(Program program,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(program.Note));
			if(program.ButtonImage==null) {
				program.ButtonImage="";
			}
			var paramButtonImage = new MySqlParameter("paramButtonImage", POut.StringParam(program.ButtonImage));
			if(program.FileTemplate==null) {
				program.FileTemplate="";
			}
			var paramFileTemplate = new MySqlParameter("paramFileTemplate", POut.StringParam(program.FileTemplate));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote,paramButtonImage,paramFileTemplate);
			}
			else {
				program.ProgramNum=Database.ExecuteInsert(command,paramNote,paramButtonImage,paramFileTemplate);
			}
