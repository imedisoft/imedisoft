...
namespace OpenDentBusiness.Crud{
	public class EtransCrud {
		...
		public static Etrans SelectOne(long etransNum) {
			...
			List<Etrans> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(Etrans etrans,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(etrans.Note));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote);
			}
			else {
				etrans.EtransNum=Database.ExecuteInsert(command,paramNote);
			}
