...
namespace OpenDentBusiness.Crud{
	public class InsEditPatLogCrud {
		...
		public static InsEditPatLog SelectOne(long insEditPatLogNum) {
			...
			List<InsEditPatLog> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static List<InsEditPatLog> TableToList(DataTable table) {
			...
			foreach(DataRow row in table.Rows) {
				...
				insEditPatLog.DateTStamp      = PIn.Date (row["DateTStamp"].ToString());
		...
		public static long Insert(InsEditPatLog insEditPatLog,bool useExistingPK) {
			...
			if(useExistingPK || PrefC.RandomKeys) {
				...
				insEditPatLog.InsEditPatLogNum=Database.ExecuteInsert(command);
			}
