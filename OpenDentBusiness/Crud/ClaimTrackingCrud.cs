...
namespace OpenDentBusiness.Crud{
	public class ClaimTrackingCrud {
		...
		public static ClaimTracking SelectOne(long claimTrackingNum) {
			...
			List<ClaimTracking> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(ClaimTracking claimTracking,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(claimTracking.Note));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote);
			}
			else {
				claimTracking.ClaimTrackingNum=Database.ExecuteInsert(command,paramNote);
			}
