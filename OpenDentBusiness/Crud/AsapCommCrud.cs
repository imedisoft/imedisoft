...
namespace OpenDentBusiness.Crud{
	public class AsapCommCrud {
		...
		public static AsapComm SelectOne(long asapCommNum) {
			...
			List<AsapComm> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static List<AsapComm> TableToList(DataTable table) {
			...
			foreach(DataRow row in table.Rows) {
				...
				asapComm.DateTimeEntry       = PIn.Date (row["DateTimeEntry"].ToString());
				asapComm.DateTimeExpire      = PIn.Date (row["DateTimeExpire"].ToString());
				asapComm.DateTimeSmsScheduled= PIn.Date (row["DateTimeSmsScheduled"].ToString());
				...
				asapComm.DateTimeSmsSent     = PIn.Date (row["DateTimeSmsSent"].ToString());
				asapComm.DateTimeEmailSent   = PIn.Date (row["DateTimeEmailSent"].ToString());
				...
				asapComm.DateTimeOrig        = PIn.Date (row["DateTimeOrig"].ToString());
		...
		public static long Insert(AsapComm asapComm,bool useExistingPK) {
			...
			var paramTemplateText = new MySqlParameter("paramTemplateText", POut.StringParam(asapComm.TemplateText));
			if(asapComm.TemplateEmail==null) {
				asapComm.TemplateEmail="";
			}
			var paramTemplateEmail = new MySqlParameter("paramTemplateEmail", POut.StringParam(asapComm.TemplateEmail));
			if(asapComm.Note==null) {
				asapComm.Note="";
			}
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(asapComm.Note));
			if(asapComm.GuidMessageToMobile==null) {
				asapComm.GuidMessageToMobile="";
			}
			var paramGuidMessageToMobile = new MySqlParameter("paramGuidMessageToMobile", POut.StringParam(asapComm.GuidMessageToMobile));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramTemplateText,paramTemplateEmail,paramNote,paramGuidMessageToMobile);
			}
			else {
				asapComm.AsapCommNum=Database.ExecuteInsert(command,paramTemplateText,paramTemplateEmail,paramNote,paramGuidMessageToMobile);
			}
