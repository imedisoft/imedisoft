...
namespace OpenDentBusiness.Crud{
	public class EhrNotPerformedCrud {
		...
		public static EhrNotPerformed SelectOne(long ehrNotPerformedNum) {
			...
			List<EhrNotPerformed> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(EhrNotPerformed ehrNotPerformed,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(ehrNotPerformed.Note));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote);
			}
			else {
				ehrNotPerformed.EhrNotPerformedNum=Database.ExecuteInsert(command,paramNote);
			}
