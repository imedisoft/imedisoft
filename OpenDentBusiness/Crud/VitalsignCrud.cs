...
namespace OpenDentBusiness.Crud{
	public class VitalsignCrud {
		...
		public static Vitalsign SelectOne(long vitalsignNum) {
			...
			List<Vitalsign> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(Vitalsign vitalsign,bool useExistingPK) {
			...
			var paramDocumentation = new MySqlParameter("paramDocumentation", POut.StringParam(vitalsign.Documentation));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramDocumentation);
			}
			else {
				vitalsign.VitalsignNum=Database.ExecuteInsert(command,paramDocumentation);
			}
