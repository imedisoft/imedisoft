...
namespace OpenDentBusiness.Crud{
	public class MedLabCrud {
		...
		public static MedLab SelectOne(long medLabNum) {
			...
			List<MedLab> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(MedLab medLab,bool useExistingPK) {
			...
			var paramNotePat = new MySqlParameter("paramNotePat", POut.StringParam(medLab.NotePat));
			if(medLab.NoteLab==null) {
				medLab.NoteLab="";
			}
			var paramNoteLab = new MySqlParameter("paramNoteLab", POut.StringParam(medLab.NoteLab));
			if(medLab.OriginalPIDSegment==null) {
				medLab.OriginalPIDSegment="";
			}
			var paramOriginalPIDSegment = new MySqlParameter("paramOriginalPIDSegment", POut.StringParam(medLab.OriginalPIDSegment));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNotePat,paramNoteLab,paramOriginalPIDSegment);
			}
			else {
				medLab.MedLabNum=Database.ExecuteInsert(command,paramNotePat,paramNoteLab,paramOriginalPIDSegment);
			}
