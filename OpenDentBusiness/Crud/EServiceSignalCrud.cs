...
namespace OpenDentBusiness.Crud{
	public class EServiceSignalCrud {
		...
		public static EServiceSignal SelectOne(long eServiceSignalNum) {
			...
			List<EServiceSignal> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(EServiceSignal eServiceSignal,bool useExistingPK) {
			...
			var paramDescription = new MySqlParameter("paramDescription", POut.StringParam(eServiceSignal.Description));
			if(eServiceSignal.Tag==null) {
				eServiceSignal.Tag="";
			}
			var paramTag = new MySqlParameter("paramTag", POut.StringParam(eServiceSignal.Tag));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramDescription,paramTag);
			}
			else {
				eServiceSignal.EServiceSignalNum=Database.ExecuteInsert(command,paramDescription,paramTag);
			}
