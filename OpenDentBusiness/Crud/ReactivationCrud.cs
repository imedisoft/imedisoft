...
namespace OpenDentBusiness.Crud{
	public class ReactivationCrud {
		...
		public static Reactivation SelectOne(long reactivationNum) {
			...
			List<Reactivation> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(Reactivation reactivation,bool useExistingPK) {
			...
			var paramReactivationNote = new MySqlParameter("paramReactivationNote", POut.StringParam(reactivation.ReactivationNote));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramReactivationNote);
			}
			else {
				reactivation.ReactivationNum=Database.ExecuteInsert(command,paramReactivationNote);
			}
