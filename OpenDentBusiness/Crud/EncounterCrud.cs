...
namespace OpenDentBusiness.Crud{
	public class EncounterCrud {
		...
		public static Encounter SelectOne(long encounterNum) {
			...
			List<Encounter> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(Encounter encounter,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(encounter.Note));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote);
			}
			else {
				encounter.EncounterNum=Database.ExecuteInsert(command,paramNote);
			}
