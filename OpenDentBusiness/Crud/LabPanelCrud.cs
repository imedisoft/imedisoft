...
namespace OpenDentBusiness.Crud{
	public class LabPanelCrud {
		...
		public static LabPanel SelectOne(long labPanelNum) {
			...
			List<LabPanel> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(LabPanel labPanel,bool useExistingPK) {
			...
			var paramRawMessage = new MySqlParameter("paramRawMessage", POut.StringParam(labPanel.RawMessage));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramRawMessage);
			}
			else {
				labPanel.LabPanelNum=Database.ExecuteInsert(command,paramRawMessage);
			}
