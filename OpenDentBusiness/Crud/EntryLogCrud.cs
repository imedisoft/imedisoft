...
namespace OpenDentBusiness.Crud{
	public class EntryLogCrud {
		...
		public static EntryLog SelectOne(long entryLogNum) {
			...
			List<EntryLog> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static List<EntryLog> TableToList(DataTable table) {
			...
			foreach(DataRow row in table.Rows) {
				...
				entryLog.LogSource    = (SecurityLogSource)PIn.Int(row["LogSource"].ToString());
		...
		public static long Insert(EntryLog entryLog,bool useExistingPK) {
			...
			if(useExistingPK || PrefC.RandomKeys) {
				...
				entryLog.EntryLogNum=Database.ExecuteInsert(command);
			}
