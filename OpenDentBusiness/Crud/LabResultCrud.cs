...
namespace OpenDentBusiness.Crud{
	public class LabResultCrud {
		...
		public static LabResult SelectOne(long labResultNum) {
			...
			List<LabResult> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static List<LabResult> TableToList(DataTable table) {
			...
			foreach(DataRow row in table.Rows) {
				...
				labResult.DateTimeTest= PIn.Date (row["DateTimeTest"].ToString());
				labResult.TestName    = PIn.String(row["TestName"].ToString());
				labResult.DateTStamp  = PIn.Date (row["DateTStamp"].ToString());
		...
		public static long Insert(LabResult labResult,bool useExistingPK) {
			...
			if(useExistingPK || PrefC.RandomKeys) {
				...
				labResult.LabResultNum=Database.ExecuteInsert(command);
			}
