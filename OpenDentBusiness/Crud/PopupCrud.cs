...
namespace OpenDentBusiness.Crud{
	public class PopupCrud {
		...
		public static Popup SelectOne(long popupNum) {
			...
			List<Popup> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(Popup popup,bool useExistingPK) {
			...
			var paramDescription = new MySqlParameter("paramDescription", POut.StringParam(popup.Description));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramDescription);
			}
			else {
				popup.PopupNum=Database.ExecuteInsert(command,paramDescription);
			}
