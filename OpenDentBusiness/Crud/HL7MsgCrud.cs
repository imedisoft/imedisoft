...
namespace OpenDentBusiness.Crud{
	public class HL7MsgCrud {
		...
		public static HL7Msg SelectOne(long hL7MsgNum) {
			...
			List<HL7Msg> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static List<HL7Msg> TableToList(DataTable table) {
			...
			foreach(DataRow row in table.Rows) {
				...
				hL7Msg.DateTStamp= PIn.Date (row["DateTStamp"].ToString());
		...
		public static long Insert(HL7Msg hL7Msg,bool useExistingPK) {
			...
			var paramMsgText = new MySqlParameter("paramMsgText", POut.StringParam(hL7Msg.MsgText));
			if(hL7Msg.Note==null) {
				hL7Msg.Note="";
			}
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(hL7Msg.Note));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramMsgText,paramNote);
			}
			else {
				hL7Msg.HL7MsgNum=Database.ExecuteInsert(command,paramMsgText,paramNote);
			}
