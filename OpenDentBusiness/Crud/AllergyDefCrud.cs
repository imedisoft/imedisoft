...
namespace OpenDentBusiness.Crud{
	public class AllergyDefCrud {
		...
		public static AllergyDef SelectOne(long allergyDefNum) {
			...
			List<AllergyDef> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static List<AllergyDef> TableToList(DataTable table) {
			...
			foreach(DataRow row in table.Rows) {
				...
				allergyDef.DateTStamp   = PIn.Date (row["DateTStamp"].ToString());
		...
		public static long Insert(AllergyDef allergyDef,bool useExistingPK) {
			...
			if(useExistingPK || PrefC.RandomKeys) {
				...
				allergyDef.AllergyDefNum=Database.ExecuteInsert(command);
			}
