...
namespace OpenDentBusiness.Crud{
	public class RepeatChargeCrud {
		...
		public static RepeatCharge SelectOne(long repeatChargeNum) {
			...
			List<RepeatCharge> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(RepeatCharge repeatCharge,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(repeatCharge.Note));
			if(repeatCharge.Npi==null) {
				repeatCharge.Npi="";
			}
			var paramNpi = new MySqlParameter("paramNpi", POut.StringParam(repeatCharge.Npi));
			if(repeatCharge.ErxAccountId==null) {
				repeatCharge.ErxAccountId="";
			}
			var paramErxAccountId = new MySqlParameter("paramErxAccountId", POut.StringParam(repeatCharge.ErxAccountId));
			if(repeatCharge.ProviderName==null) {
				repeatCharge.ProviderName="";
			}
			var paramProviderName = new MySqlParameter("paramProviderName", POut.StringParam(repeatCharge.ProviderName));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote,paramNpi,paramErxAccountId,paramProviderName);
			}
			else {
				repeatCharge.RepeatChargeNum=Database.ExecuteInsert(command,paramNote,paramNpi,paramErxAccountId,paramProviderName);
			}
