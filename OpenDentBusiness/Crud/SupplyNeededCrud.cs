...
namespace OpenDentBusiness.Crud{
	public class SupplyNeededCrud {
		...
		public static SupplyNeeded SelectOne(long supplyNeededNum) {
			...
			List<SupplyNeeded> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(SupplyNeeded supplyNeeded,bool useExistingPK) {
			...
			var paramDescription = new MySqlParameter("paramDescription", POut.StringParam(supplyNeeded.Description));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramDescription);
			}
			else {
				supplyNeeded.SupplyNeededNum=Database.ExecuteInsert(command,paramDescription);
			}
