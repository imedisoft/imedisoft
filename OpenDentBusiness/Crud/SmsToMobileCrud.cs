...
namespace OpenDentBusiness.Crud{
	public class SmsToMobileCrud {
		...
		public static SmsToMobile SelectOne(long smsToMobileNum) {
			...
			List<SmsToMobile> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(SmsToMobile smsToMobile,bool useExistingPK) {
			...
			var paramMsgText = new MySqlParameter("paramMsgText", POut.StringNote(smsToMobile.MsgText));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramMsgText);
			}
			else {
				smsToMobile.SmsToMobileNum=Database.ExecuteInsert(command,paramMsgText);
			}
