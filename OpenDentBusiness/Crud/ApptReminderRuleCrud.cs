...
namespace OpenDentBusiness.Crud{
	public class ApptReminderRuleCrud {
		...
		public static ApptReminderRule SelectOne(long apptReminderRuleNum) {
			...
			List<ApptReminderRule> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(ApptReminderRule apptReminderRule,bool useExistingPK) {
			...
			var paramTemplateSMS = new MySqlParameter("paramTemplateSMS", POut.StringParam(apptReminderRule.TemplateSMS));
			if(apptReminderRule.TemplateEmailSubject==null) {
				apptReminderRule.TemplateEmailSubject="";
			}
			var paramTemplateEmailSubject = new MySqlParameter("paramTemplateEmailSubject", POut.StringParam(apptReminderRule.TemplateEmailSubject));
			if(apptReminderRule.TemplateEmail==null) {
				apptReminderRule.TemplateEmail="";
			}
			var paramTemplateEmail = new MySqlParameter("paramTemplateEmail", POut.StringParam(apptReminderRule.TemplateEmail));
			if(apptReminderRule.TemplateSMSAggShared==null) {
				apptReminderRule.TemplateSMSAggShared="";
			}
			var paramTemplateSMSAggShared = new MySqlParameter("paramTemplateSMSAggShared", POut.StringParam(apptReminderRule.TemplateSMSAggShared));
			if(apptReminderRule.TemplateSMSAggPerAppt==null) {
				apptReminderRule.TemplateSMSAggPerAppt="";
			}
			var paramTemplateSMSAggPerAppt = new MySqlParameter("paramTemplateSMSAggPerAppt", POut.StringParam(apptReminderRule.TemplateSMSAggPerAppt));
			if(apptReminderRule.TemplateEmailSubjAggShared==null) {
				apptReminderRule.TemplateEmailSubjAggShared="";
			}
			var paramTemplateEmailSubjAggShared = new MySqlParameter("paramTemplateEmailSubjAggShared", POut.StringParam(apptReminderRule.TemplateEmailSubjAggShared));
			if(apptReminderRule.TemplateEmailAggShared==null) {
				apptReminderRule.TemplateEmailAggShared="";
			}
			var paramTemplateEmailAggShared = new MySqlParameter("paramTemplateEmailAggShared", POut.StringParam(apptReminderRule.TemplateEmailAggShared));
			if(apptReminderRule.TemplateEmailAggPerAppt==null) {
				apptReminderRule.TemplateEmailAggPerAppt="";
			}
			var paramTemplateEmailAggPerAppt = new MySqlParameter("paramTemplateEmailAggPerAppt", POut.StringParam(apptReminderRule.TemplateEmailAggPerAppt));
			if(apptReminderRule.TemplateAutoReply==null) {
				apptReminderRule.TemplateAutoReply="";
			}
			var paramTemplateAutoReply = new MySqlParameter("paramTemplateAutoReply", POut.StringParam(apptReminderRule.TemplateAutoReply));
			if(apptReminderRule.TemplateAutoReplyAgg==null) {
				apptReminderRule.TemplateAutoReplyAgg="";
			}
			var paramTemplateAutoReplyAgg = new MySqlParameter("paramTemplateAutoReplyAgg", POut.StringParam(apptReminderRule.TemplateAutoReplyAgg));
			if(apptReminderRule.TemplateComeInMessage==null) {
				apptReminderRule.TemplateComeInMessage="";
			}
			var paramTemplateComeInMessage = new MySqlParameter("paramTemplateComeInMessage", POut.StringParam(apptReminderRule.TemplateComeInMessage));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramTemplateSMS,paramTemplateEmailSubject,paramTemplateEmail,paramTemplateSMSAggShared,paramTemplateSMSAggPerAppt,paramTemplateEmailSubjAggShared,paramTemplateEmailAggShared,paramTemplateEmailAggPerAppt,paramTemplateAutoReply,paramTemplateAutoReplyAgg,paramTemplateComeInMessage);
			}
			else {
				apptReminderRule.ApptReminderRuleNum=Database.ExecuteInsert(command,paramTemplateSMS,paramTemplateEmailSubject,paramTemplateEmail,paramTemplateSMSAggShared,paramTemplateSMSAggPerAppt,paramTemplateEmailSubjAggShared,paramTemplateEmailAggShared,paramTemplateEmailAggPerAppt,paramTemplateAutoReply,paramTemplateAutoReplyAgg,paramTemplateComeInMessage);
			}
