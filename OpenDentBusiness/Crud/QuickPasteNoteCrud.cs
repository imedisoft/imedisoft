...
namespace OpenDentBusiness.Crud{
	public class QuickPasteNoteCrud {
		...
		public static QuickPasteNote SelectOne(long quickPasteNoteNum) {
			...
			List<QuickPasteNote> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(QuickPasteNote quickPasteNote,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(quickPasteNote.Note));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote);
			}
			else {
				quickPasteNote.QuickPasteNoteNum=Database.ExecuteInsert(command,paramNote);
			}
