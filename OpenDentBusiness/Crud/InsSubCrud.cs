...
namespace OpenDentBusiness.Crud{
	public class InsSubCrud {
		...
		public static InsSub SelectOne(long insSubNum) {
			...
			List<InsSub> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(InsSub insSub,bool useExistingPK) {
			...
			var paramBenefitNotes = new MySqlParameter("paramBenefitNotes", POut.StringParam(insSub.BenefitNotes));
			if(insSub.SubscNote==null) {
				insSub.SubscNote="";
			}
			var paramSubscNote = new MySqlParameter("paramSubscNote", POut.StringParam(insSub.SubscNote));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramBenefitNotes,paramSubscNote);
			}
			else {
				insSub.InsSubNum=Database.ExecuteInsert(command,paramBenefitNotes,paramSubscNote);
			}
