...
namespace OpenDentBusiness.Crud{
	public class SheetCrud {
		...
		public static Sheet SelectOne(long sheetNum) {
			...
			List<Sheet> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(Sheet sheet,bool useExistingPK) {
			...
			var paramInternalNote = new MySqlParameter("paramInternalNote", POut.StringParam(sheet.InternalNote));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramInternalNote);
			}
			else {
				sheet.SheetNum=Database.ExecuteInsert(command,paramInternalNote);
			}
