...
namespace OpenDentBusiness.Crud{
	public class FeeCrud {
		...
		public static Fee SelectOne(long feeNum) {
			...
			List<Fee> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static List<Fee> TableToList(DataTable table) {
			...
			foreach(DataRow row in table.Rows) {
				...
				fee.SecDateTEdit   = PIn.Date (row["SecDateTEdit"].ToString());
				retVal.Add(fee);
			}
			return retVal;
		}
		...
		public static long Insert(Fee fee,bool useExistingPK) {
			...
			if(useExistingPK || PrefC.RandomKeys) {
				...
				fee.FeeNum=Database.ExecuteInsert(command);
			}
			return fee.FeeNum;
		}
