...
namespace OpenDentBusiness.Crud{
	public class InsVerifyHistCrud {
		...
		public static InsVerifyHist SelectOne(long insVerifyHistNum) {
			...
			List<InsVerifyHist> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(InsVerifyHist insVerifyHist,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringNote(insVerifyHist.Note));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote);
			}
			else {
				insVerifyHist.InsVerifyHistNum=Database.ExecuteInsert(command,paramNote);
			}
