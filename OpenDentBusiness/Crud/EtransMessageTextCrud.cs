...
namespace OpenDentBusiness.Crud{
	public class EtransMessageTextCrud {
		...
		public static EtransMessageText SelectOne(long etransMessageTextNum) {
			...
			List<EtransMessageText> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(EtransMessageText etransMessageText,bool useExistingPK) {
			...
			var paramMessageText = new MySqlParameter("paramMessageText", POut.StringParam(etransMessageText.MessageText));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramMessageText);
			}
			else {
				etransMessageText.EtransMessageTextNum=Database.ExecuteInsert(command,paramMessageText);
			}
