...
namespace OpenDentBusiness.Crud{
	public class DbmLogCrud {
		...
		public static DbmLog SelectOne(long dbmLogNum) {
			...
			List<DbmLog> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(DbmLog dbmLog,bool useExistingPK) {
			...
			var paramLogText = new MySqlParameter("paramLogText", POut.StringParam(dbmLog.LogText));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramLogText);
			}
			else {
				dbmLog.DbmLogNum=Database.ExecuteInsert(command,paramLogText);
			}
