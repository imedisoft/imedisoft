...
namespace OpenDentBusiness.Crud{
	public class RecurringChargeCrud {
		...
		public static RecurringCharge SelectOne(long recurringChargeNum) {
			...
			List<RecurringCharge> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(RecurringCharge recurringCharge,bool useExistingPK) {
			...
			var paramErrorMsg = new MySqlParameter("paramErrorMsg", POut.StringParam(recurringCharge.ErrorMsg));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramErrorMsg);
			}
			else {
				recurringCharge.RecurringChargeNum=Database.ExecuteInsert(command,paramErrorMsg);
			}
