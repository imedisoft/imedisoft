...
namespace OpenDentBusiness.Crud{
	public class InsVerifyCrud {
		...
		public static InsVerify SelectOne(long insVerifyNum) {
			...
			List<InsVerify> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static List<InsVerify> TableToList(DataTable table) {
			...
			foreach(DataRow row in table.Rows) {
				...
				insVerify.DateTimeEntry                = PIn.Date (row["DateTimeEntry"].ToString());
				insVerify.HoursAvailableForVerification= PIn.Double(row["HoursAvailableForVerification"].ToString());
				insVerify.SecDateTEdit                 = PIn.Date (row["SecDateTEdit"].ToString());
		...
		public static long Insert(InsVerify insVerify,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringNote(insVerify.Note));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote);
			}
			else {
				insVerify.InsVerifyNum=Database.ExecuteInsert(command,paramNote);
			}
