...
namespace OpenDentBusiness.Crud{
	public class SigElementDefCrud {
		...
		public static SigElementDef SelectOne(long sigElementDefNum) {
			...
			List<SigElementDef> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(SigElementDef sigElementDef,bool useExistingPK) {
			...
			var paramSound = new MySqlParameter("paramSound", POut.StringParam(sigElementDef.Sound));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramSound);
			}
			else {
				sigElementDef.SigElementDefNum=Database.ExecuteInsert(command,paramSound);
			}
