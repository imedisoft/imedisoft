...
namespace OpenDentBusiness.Crud{
	public class QuestionDefCrud {
		...
		public static QuestionDef SelectOne(long questionDefNum) {
			...
			List<QuestionDef> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(QuestionDef questionDef,bool useExistingPK) {
			...
			var paramDescription = new MySqlParameter("paramDescription", POut.StringParam(questionDef.Description));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramDescription);
			}
			else {
				questionDef.QuestionDefNum=Database.ExecuteInsert(command,paramDescription);
			}
