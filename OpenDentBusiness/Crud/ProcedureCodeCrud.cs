...
namespace OpenDentBusiness.Crud{
	public class ProcedureCodeCrud {
		...
		public static ProcedureCode SelectOne(long codeNum) {
			...
			List<ProcedureCode> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(ProcedureCode procedureCode,bool useExistingPK) {
			...
			var paramDefaultNote = new MySqlParameter("paramDefaultNote", POut.StringParam(procedureCode.DefaultNote));
			if(procedureCode.DefaultClaimNote==null) {
				procedureCode.DefaultClaimNote="";
			}
			var paramDefaultClaimNote = new MySqlParameter("paramDefaultClaimNote", POut.StringParam(procedureCode.DefaultClaimNote));
			if(procedureCode.DefaultTPNote==null) {
				procedureCode.DefaultTPNote="";
			}
			var paramDefaultTPNote = new MySqlParameter("paramDefaultTPNote", POut.StringParam(procedureCode.DefaultTPNote));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramDefaultNote,paramDefaultClaimNote,paramDefaultTPNote);
			}
			else {
				procedureCode.CodeNum=Database.ExecuteInsert(command,paramDefaultNote,paramDefaultClaimNote,paramDefaultTPNote);
			}
