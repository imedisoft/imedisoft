...
namespace OpenDentBusiness.Crud{
	public class UserOdPrefCrud {
		...
		public static UserOdPref SelectOne(long userOdPrefNum) {
			...
			List<UserOdPref> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(UserOdPref userOdPref,bool useExistingPK) {
			...
			var paramValueString = new MySqlParameter("paramValueString", POut.StringParam(userOdPref.ValueString));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramValueString);
			}
			else {
				userOdPref.UserOdPrefNum=Database.ExecuteInsert(command,paramValueString);
			}
