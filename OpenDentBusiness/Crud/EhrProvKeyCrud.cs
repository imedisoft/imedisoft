...
namespace OpenDentBusiness.Crud{
	public class EhrProvKeyCrud {
		...
		public static EhrProvKey SelectOne(long ehrProvKeyNum) {
			...
			List<EhrProvKey> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(EhrProvKey ehrProvKey,bool useExistingPK) {
			...
			var paramNotes = new MySqlParameter("paramNotes", POut.StringParam(ehrProvKey.Notes));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNotes);
			}
			else {
				ehrProvKey.EhrProvKeyNum=Database.ExecuteInsert(command,paramNotes);
			}
