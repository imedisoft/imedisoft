...
namespace OpenDentBusiness.Crud{
	public class PatFieldCrud {
		...
		public static PatField SelectOne(long patFieldNum) {
			...
			List<PatField> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(PatField patField,bool useExistingPK) {
			...
			var paramFieldValue = new MySqlParameter("paramFieldValue", POut.StringNote(patField.FieldValue));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramFieldValue);
			}
			else {
				patField.PatFieldNum=Database.ExecuteInsert(command,paramFieldValue);
			}
