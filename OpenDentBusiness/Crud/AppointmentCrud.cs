...
namespace OpenDentBusiness.Crud{
	public class AppointmentCrud {
		...
		public static Appointment SelectOne(long aptNum) {
			...
			List<Appointment> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static List<Appointment> TableToList(DataTable table) {
			...
			foreach(DataRow row in table.Rows) {
				...
				appointment.AptDateTime          = PIn.Date (row["AptDateTime"].ToString());
				...
				appointment.DateTStamp           = PIn.Date (row["DateTStamp"].ToString());
				appointment.DateTimeArrived      = PIn.Date (row["DateTimeArrived"].ToString());
				appointment.DateTimeSeated       = PIn.Date (row["DateTimeSeated"].ToString());
				appointment.DateTimeDismissed    = PIn.Date (row["DateTimeDismissed"].ToString());
				appointment.InsPlan1             = PIn.Long  (row["InsPlan1"].ToString());
				appointment.InsPlan2             = PIn.Long  (row["InsPlan2"].ToString());
				appointment.DateTimeAskedToArrive= PIn.Date (row["DateTimeAskedToArrive"].ToString());
				...
				appointment.SecDateTEntry        = PIn.Date (row["SecDateTEntry"].ToString());
		...
		public static long Insert(Appointment appointment,bool useExistingPK) {
			...
			var paramNote=new MySqlParameter("paramNote", POut.StringNote(appointment.Note));
			if(appointment.ProcsColored==null) {
				appointment.ProcsColored="";
			}
			var paramProcsColored=new MySqlParameter("paramProcsColored",POut.StringParam(appointment.ProcsColored));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote,paramProcsColored);
			}
			else {
				appointment.AptNum=Database.ExecuteInsert(command,paramNote,paramProcsColored);
			}
