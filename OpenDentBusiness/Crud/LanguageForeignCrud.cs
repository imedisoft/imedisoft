...
namespace OpenDentBusiness.Crud{
	public class LanguageForeignCrud {
		...
		public static LanguageForeign SelectOne(long languageForeignNum) {
			...
			List<LanguageForeign> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(LanguageForeign languageForeign,bool useExistingPK) {
			...
			var paramClassType = new MySqlParameter("paramClassType", POut.StringParam(languageForeign.ClassType));
			if(languageForeign.English==null) {
				languageForeign.English="";
			}
			var paramEnglish = new MySqlParameter("paramEnglish", POut.StringParam(languageForeign.English));
			if(languageForeign.Translation==null) {
				languageForeign.Translation="";
			}
			var paramTranslation = new MySqlParameter("paramTranslation", POut.StringParam(languageForeign.Translation));
			if(languageForeign.Comments==null) {
				languageForeign.Comments="";
			}
			var paramComments = new MySqlParameter("paramComments", POut.StringParam(languageForeign.Comments));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramClassType,paramEnglish,paramTranslation,paramComments);
			}
			else {
				languageForeign.LanguageForeignNum=Database.ExecuteInsert(command,paramClassType,paramEnglish,paramTranslation,paramComments);
			}
