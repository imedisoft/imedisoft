...
namespace OpenDentBusiness.Crud{
	public class EhrSummaryCcdCrud {
		...
		public static EhrSummaryCcd SelectOne(long ehrSummaryCcdNum) {
			...
			List<EhrSummaryCcd> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(EhrSummaryCcd ehrSummaryCcd,bool useExistingPK) {
			...
			var paramContentSummary = new MySqlParameter("paramContentSummary", POut.StringParam(ehrSummaryCcd.ContentSummary));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramContentSummary);
			}
			else {
				ehrSummaryCcd.EhrSummaryCcdNum=Database.ExecuteInsert(command,paramContentSummary);
			}
