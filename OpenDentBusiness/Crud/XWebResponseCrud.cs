...
namespace OpenDentBusiness.Crud{
	public class XWebResponseCrud {
		...
		public static XWebResponse SelectOne(long xWebResponseNum) {
			...
			List<XWebResponse> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(XWebResponse xWebResponse,bool useExistingPK) {
			...
			var paramHpfUrl = new MySqlParameter("paramHpfUrl", POut.StringParam(xWebResponse.HpfUrl));
			if(xWebResponse.DebugError==null) {
				xWebResponse.DebugError="";
			}
			var paramDebugError = new MySqlParameter("paramDebugError", POut.StringParam(xWebResponse.DebugError));
			if(xWebResponse.PayNote==null) {
				xWebResponse.PayNote="";
			}
			var paramPayNote = new MySqlParameter("paramPayNote", POut.StringParam(xWebResponse.PayNote));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramHpfUrl,paramDebugError,paramPayNote);
			}
			else {
				xWebResponse.XWebResponseNum=Database.ExecuteInsert(command,paramHpfUrl,paramDebugError,paramPayNote);
			}
