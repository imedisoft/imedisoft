...
namespace OpenDentBusiness.Crud{
	public class CentralConnectionCrud {
		...
		public static CentralConnection SelectOne(long centralConnectionNum) {
			...
			List<CentralConnection> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(CentralConnection centralConnection,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(centralConnection.Note));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote);
			}
			else {
				centralConnection.CentralConnectionNum=Database.ExecuteInsert(command,paramNote);
			}
