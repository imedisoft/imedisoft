...
namespace OpenDentBusiness.Crud{
	public class AdjustmentCrud {
		...
		public static Adjustment SelectOne(long adjNum) {
			...
			List<Adjustment> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(Adjustment adjustment,bool useExistingPK) {
			...
			var paramAdjNote=new MySqlParameter("paramAdjNote",adjustment.AdjNote);
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramAdjNote);
			}
			else {
				adjustment.AdjNum=Database.ExecuteInsert(command,paramAdjNote);
			}
		...
		public static long InsertNoCache(Adjustment adjustment,bool useExistingPK) {
			...
			var paramAdjNote = new MySqlParameter("paramAdjNote", POut.StringNote(adjustment.AdjNote));
			if(useExistingPK) {
				Database.ExecuteNonQuery(command,paramAdjNote);
			}
			else {
