...
namespace OpenDentBusiness.Crud{
	public class ProcCodeNoteCrud {
		...
		public static ProcCodeNote SelectOne(long procCodeNoteNum) {
			...
			List<ProcCodeNote> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(ProcCodeNote procCodeNote,bool useExistingPK) {
			...
			var paramNote = new MySqlParameter("paramNote", POut.StringParam(procCodeNote.Note));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramNote);
			}
			else {
				procCodeNote.ProcCodeNoteNum=Database.ExecuteInsert(command,paramNote);
			}
