...
namespace OpenDentBusiness.Crud{
	public class WikiPageCrud {
		...
		public static WikiPage SelectOne(long wikiPageNum) {
			...
			List<WikiPage> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(WikiPage wikiPage,bool useExistingPK) {
			...
			var paramPageContent = new MySqlParameter("paramPageContent", POut.StringParam(wikiPage.PageContent));
			if(wikiPage.PageContentPlainText==null) {
				wikiPage.PageContentPlainText="";
			}
			var paramPageContentPlainText = new MySqlParameter("paramPageContentPlainText", POut.StringParam(wikiPage.PageContentPlainText));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramPageContent,paramPageContentPlainText);
			}
			else {
				wikiPage.WikiPageNum=Database.ExecuteInsert(command,paramPageContent,paramPageContentPlainText);
			}
