...
namespace OpenDentBusiness.Crud{
	public class EhrAmendmentCrud {
		...
		public static EhrAmendment SelectOne(long ehrAmendmentNum) {
			...
			List<EhrAmendment> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(EhrAmendment ehrAmendment,bool useExistingPK) {
			...
			var paramDescription = new MySqlParameter("paramDescription", POut.StringParam(ehrAmendment.Description));
			if(ehrAmendment.SourceName==null) {
				ehrAmendment.SourceName="";
			}
			var paramSourceName = new MySqlParameter("paramSourceName", POut.StringParam(ehrAmendment.SourceName));
			if(ehrAmendment.RawBase64==null) {
				ehrAmendment.RawBase64="";
			}
			var paramRawBase64 = new MySqlParameter("paramRawBase64", POut.StringParam(ehrAmendment.RawBase64));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramDescription,paramSourceName,paramRawBase64);
			}
			else {
				ehrAmendment.EhrAmendmentNum=Database.ExecuteInsert(command,paramDescription,paramSourceName,paramRawBase64);
			}
