...
namespace OpenDentBusiness.Crud{
	public class ProcButtonCrud {
		...
		public static ProcButton SelectOne(long procButtonNum) {
			...
			List<ProcButton> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(ProcButton procButton,bool useExistingPK) {
			...
			var paramButtonImage = new MySqlParameter("paramButtonImage", POut.StringParam(procButton.ButtonImage));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramButtonImage);
			}
			else {
				procButton.ProcButtonNum=Database.ExecuteInsert(command,paramButtonImage);
			}
