...
namespace OpenDentBusiness.Crud{
	public class DocumentMiscCrud {
		...
		public static DocumentMisc SelectOne(long docMiscNum) {
			...
			List<DocumentMisc> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(DocumentMisc documentMisc,bool useExistingPK) {
			...
			var paramRawBase64 = new MySqlParameter("paramRawBase64", POut.StringParam(documentMisc.RawBase64));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramRawBase64);
			}
			else {
				documentMisc.DocMiscNum=Database.ExecuteInsert(command,paramRawBase64);
			}
