...
namespace OpenDentBusiness.Crud{
	public class HL7DefFieldCrud {
		...
		public static HL7DefField SelectOne(long hL7DefFieldNum) {
			...
			List<HL7DefField> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(HL7DefField hL7DefField,bool useExistingPK) {
			...
			var paramFixedText = new MySqlParameter("paramFixedText", POut.StringParam(hL7DefField.FixedText));
			if(useExistingPK || PrefC.RandomKeys) {
				Database.ExecuteNonQuery(command,paramFixedText);
			}
			else {
				hL7DefField.HL7DefFieldNum=Database.ExecuteInsert(command,paramFixedText);
			}
