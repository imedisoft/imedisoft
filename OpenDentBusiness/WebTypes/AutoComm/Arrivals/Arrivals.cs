...
namespace OpenDentBusiness.AutoComm
{
    public class Arrivals
	{
		...
		/// <summary>
		/// If necessary, gets ApptReminderSents and corresponding ApptReminderRules for given appointments from database.
		/// </summary>
		public static Arrivals LoadArrivals(List<long> clinicIds, List<long> appointmentIds)
		{
			...
			if (!appointmentIds.IsNullOrEmpty() && listSignedUpClinics.Any() && Prefs.GetBool(PrefName.ApptArrivalAutoEnabled))
			{
				...
				if (arrivals.ListApptReminderRules.Any())
				{
					...
					arrivals.ListArrivalsSent = ApptReminderSents.GetForApt(appointmentIds.ToArray());
				}
			}
		...
		/// <summary>
		/// Gets Arrival ApptReminderRules, including rules for clinics that are using default rules.
		/// </summary>
		private static List<ApptReminderRule> GetApptReminderRules(List<long> clinicIds)
		{
			...
			return listRules.Where(x => x.IsEnabled && x.AreArrivalResponsesSetup && x.ClinicNum.In(clinicIds)) // Filtered to enabled/setup/signedup
					.ToList();
		}

		/// <summary>
		/// Processes an inboud SMS and determines if it is an "I have arrived" message and office is setup to send an automatic Arrival Response.
		/// </summary>
		...
		/// <summary>
		/// Determines if office is setup to send an automatic Arrival Response for the given patient.
		/// </summary>
		public static void ProcessArrival(long patNumForResponse, List<long> listPatNums, long clinicNum, string mobilePhoneNumber = null, List<Appointment> listAppts = null)
		{
		...
		/// <summary>
		/// Determines if the patient corresponding to the SmsFromMobile can be sent an Arrival Response and marked as Arrived.
		/// </summary>
		private bool TryProcessArrival(long patNum, long clinicNum, string mobilePhoneNumber, List<Appointment> appointments)
		{
			bool retVal = false;
			List<Appointment> appointmentsToday = appointments.Where(x => x.ClinicNum == clinicNum && x.AptDateTime.Date == DateTime.Today).OrderBy(x => x.AptDateTime).ToList();
			...
			if (!listApptResponses.IsNullOrEmpty())
			{
				...
				retVal = TrySendArrivalResponseSms(patNum, mobilePhoneNumber, clinicNum, message);
			}
		...
		{
			// Here is where we will also look at eClipboard prefs to see if we should include the eClipboard token. 
			// We need to aggregate all tokens into one response message.
			return listApptResponses.First().Response;
		}

		/// <summary>
		/// Sets appointments.Confirmed to the ArrivedTimeTrigger.
		/// </summary>
		...
		/// <summary>
		/// Attempts to send the Arrival Response sms. 
		/// Logs on failure.
		/// </summary>
		private bool TrySendArrivalResponseSms(long patNum, string wirelessPhone, long clinicNum, string message)
		{
			bool retVal = false;
			try
			{
		...
		/// <summary>
		/// Determines if the given aptNum corresponds to an ApptReminderRule that has a corresponding ComeInMessageTemplate.
		...
		/// <summary>
		/// Gets an ApptReminderRule with a ComeInMessageTemplate that corresponds to the given aptNum. 
		/// Does not make database calls.
		/// </summary>
		...
		/// <summary>
		/// Returns true if an appropriate 'Come In' message template was found for the given appointment.
		/// </summary>
		...
		/// <summary>
		/// Returns a list of Appointment/Arrival Responses if an appropriate Arrival Response message template was found for at least one of the given appointments.
		/// </summary>
