...
namespace OpenDentBusiness.WebTypes.WebForms
{
	public class WebForms_Sheets
	{
		...
		{
            ParseWebFormSheet(sheetToMatch, webFormPrefCulture, out string lName, out string fName, out DateTime birthdate, out List<string> listPhoneNumbers, out string email);

            foreach (WebForms_Sheet sheet in listSheets)
			{
                ParseWebFormSheet(sheet, webFormPrefCulture, out string lNameSheet, out string fNameSheet, out _, out _, out string emailSheet);
                
				if (lName == lNameSheet && fName == fNameSheet && birthdate == new DateTime() && email == emailSheet //All phone numbers must match in both.
					&& listPhoneNumbers.Except(new List<string>()).Count() == 0 && new List<string>().Except(listPhoneNumbers).Count() == 0)
				{
                    new List<long>().Add(sheet.SheetID);
				}
			}
