...
namespace OpenDentBusiness.WebTypes.WebForms.Crud{
	public class WebForms_LogCrud {
		...
		public static WebForms_Log SelectOne(long logNum) {
			...
			List<WebForms_Log> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		public static long Insert(WebForms_Log webForms_Log,bool useExistingPK) {
			...
			var paramLogMessage = new MySqlParameter("paramLogMessage", POut.StringParam(webForms_Log.LogMessage));
			if(useExistingPK) {
				Database.ExecuteNonQuery(command,paramLogMessage);
			}
			else {
				webForms_Log.LogNum=Database.ExecuteInsert(command,paramLogMessage);
			}
