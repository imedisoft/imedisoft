...
namespace OpenDentBusiness
{
	public class AccountModules
	{
		...
		public static async System.Threading.Tasks.Task<LoadData> GetLoadDataAsync(long patNum, DateTime fromDate, DateTime toDate, bool intermingled, bool showProcBreakdown, bool showPayNotes, bool showAdjNotes, bool doMakeSecLog, bool doGetOrtho)
        {
			...
			Logger.LogAction("Patients.GetFamily", () => fam = Patients.GetFamily(patNum));
			if (intermingled)
			{
				patNum = fam.ListPats[0].PatNum;//guarantor
			}
			Logger.LogAction("Family.GetPatient", () => pat = fam.GetPatient(patNum));
			...
			{
				Logger.LogAction("InsSubs.RefreshForFam", () => retVal.ListInsSubs = InsSubs.RefreshForFam(fam));
				Logger.LogAction("InsPlans.RefreshSubList", () => retVal.ListInsPlans = InsPlans.RefreshForSubList(retVal.ListInsSubs));
			});//InsSubs.RefreshForFam
			Action refreshOrGetFirstOrthoProcDate = new Action(() =>
			{
				Logger.LogAction("PatientNotes.Refresh", () => retVal.PatNote = PatientNotes.Refresh(pat.PatNum, pat.Guarantor));
				if (doGetOrtho)
				{
					Logger.LogAction("Procedures.GetFirstOrthoProcDate", () => retVal.FirstOrthoProcDate = Procedures.GetFirstOrthoProcDate(retVal.PatNote));
				}
			});//PatientNotes.Refresh / Procedures.GetFirstOrthoProcDate
			Action refreshBenefits = new Action(() =>
			{
				Logger.LogAction("Benefits.Refresh", () => retVal.ListBenefits = Benefits.Refresh(retVal.ListPatPlans, retVal.ListInsSubs));
			});//Benefits.Refresh
			Action getAccount = new Action(() =>
			{
				Logger.LogAction("Get Account", () => dataSetAccount = GetAccount(patNum, fromDate, toDate, intermingled, singlePatient, 0
			...
			{
				Logger.LogAction("PatientLinks.GetLinks", () =>
			...
			{
				Logger.LogAction("PaySplits.GetPrepayForFam", () => retVal.ListPrePayments = PaySplits.GetPrepayForFam(fam, false, isAccountModule: true));
			});//PaySplits.GetPrepayForFam
			Action refreshRepeatCharges = new Action(() =>
			{
				Logger.LogAction("RepeatCharges.Refresh", () => retVal.ArrRepeatCharges = RepeatCharges.Refresh(pat.PatNum));
			});//RepeatCharges.Refresh
			Action getProgNotesCommLog = new Action(() =>
			{
				Logger.LogAction("GetCommLog", () => retVal.DataSetMain.Tables.Add(GetCommLog(pat, fam)));
			});//GetProgNotes / GetCommLog
			Action refreshPatFields = new Action(() =>
			{
				Logger.LogAction("PatFields.Refresh", () => retVal.ArrPatFields = PatFields.Refresh(pat.PatNum));
			});//PatFields.Refresh
			...
			{
				Logger.LogAction("Claims.Refresh", () => retVal.ListClaims = Claims.Refresh(pat.PatNum));
			});//Claims.Refresh
			Action claimProcsGetHistList = new Action(() =>
			{
				Logger.LogAction("ClaimProcs.GetHistList", () => retVal.HistList = ClaimProcs.GetHistList(pat.PatNum, retVal.ListBenefits, retVal.ListPatPlans
				  , retVal.ListInsPlans, DateTime.Today, retVal.ListInsSubs));
			});//ClaimProcs.GetHistList
			Action getMisc = new Action(() =>
			{
				Logger.LogAction("GetMisc",  () =>
			...
			Action getFamily = new Action(() =>
			{
				...
				Logger.LogAction("Patients.GetFamily", () => retVal.Fam = Patients.GetFamily(patNum));//have to get family after dataset due to aging calc.
			});//Patients.GetFamily
			...
			{
				Logger.LogAction("FieldDefLinks.GetForLocation", () => retVal.ListFieldDefLinksAcct = FieldDefLinks.GetForLocation(FieldLocations.Account));
		...
		private static DataTable GetPayPlanAmortTable(long payPlanNum)
		{
			...
			DataTable rawCharge = dcon.ExecuteDataTable(command);
			DateTime dateT;
			decimal principal;
			decimal interest;
			decimal total;
			...
			for (int i = 0; i < rawPay.Rows.Count; i++)
			{
				...
				{//attached to claim but no check (claimpayment) created
					row["description"] = "No Insurance Check Created";
				}
				else
				{
		...
		public static DataTable GetCommLog(Patient pat, Family fam)
		{
			...
			for (int i = 0; i < rawComm.Rows.Count; i++)
			{
				...
				{//anything except none
					row["mode"] = ((CommItemMode)PIn.Long(rowCur["Mode_"].ToString())).ToString();
				}
			...
			for (int i = 0; i < rawEmail.Rows.Count; i++)
			{
				...
				row["FormPatNum"] = "0";
				row["mode"] = CommItemMode.Email.ToString();
				txt = "";
			...
			, bool doShowHiddenPaySplits = false)
		{
			...
			foreach (DataRow row in listRows)
			{
				if (row["PatNum"].ToString() != patNumPrev)
				{
					...
					if (row["description"].ToString() == "Balance Forward")
					{
			...
			, bool isComputeAging = true, bool doIncludePatLName = false, List<long> listPayPlanNumsExclude = null, bool doShowHiddenPaySplits = false)
		{
			...
			{//don't run if IsInvoice or if LimitedStatement with no procs
				rawClaimPay = dcon.ExecuteDataTable(command);
			}
			DateTime procdate;
			decimal writeoff;
			List<Definition> listDefs = Definitions.GetDefsForCategory(DefinitionCategory.AccountColors);
			...
			{//0 rows if isInvoice or is LimitedStatement with no procs
			 ...
			 //There was a bug with databases from conversions where there were claims with no procedures but with a claimproc (pay as total).
				...
				row["description"] = "Insurance Payment for Claim" + " " + procdate.ToShortDateString();
				if (rawClaimPayRow["PayPlanNum"].ToString() != "0")
				{
					row["description"] += "\r\n(" + "Payments Tracked in Payment Plan" + ")";
				}
				if (rawClaimPayRow["PayPlanNum"].ToString() != "0" || writeoff != 0)
				{
					row["description"] += "\r\n" + "Payment" + ": " + amt.ToString("c");
				}
				if (writeoff != 0)
				{
			...
			for (int i = 0; i < rawProc.Rows.Count; i++)
			{
				...
				if (patPay > 0)
				{
					extraDetail += "Pat Paid: " + patPay.ToString("c");
				}
				if (adjAmt != 0)
				{
					if (extraDetail != "")
					...
					extraDetail += "Adj: " + adjAmt.ToString("c");
				}
				...
				if (insPayEst > 0)
				{
					...
					extraDetail += "Ins Est: " + insPayEst.ToString("c");
				}
				...
				{//if there is a cap writeoff, showing a patient portion would calculate wrong.
					...
					extraDetail += "Pat Port: " + patPort.ToString("c");
				}
			...
			{
				rawAdj = dcon.ExecuteDataTable(command);
			}
			...
			{//Should not show in limited statements
			 ...
			 //The only differences between the selections below and those from above are that we use 'payment.PatNum' and '0 AS splitAmt_'.
				command += "UNION ALL "
					...
					+ "0 AS splitAmt_,MAX(payment.PayNote) PayNote,MAX(paysplit.UnearnedType) UnearnedType,MAX(paysplit.SecDateTEdit) SecDateTEdit,'' as AdjNums_, 1 isOutOfFamily_ "
					+ "FROM payment "
			...
			{
				rawPay = dcon.ExecuteDataTable(command);
			}
			//Payment query 3
			...
			for (int i = 0; i < rawPay.Rows.Count; i++)
			{
				...
				if (!rowPatNum.In(fam.ListPats.Select(x => x.PatNum).Concat(listSuperFamPatNums)))
				{
					continue;
				}
				...
				if (isOutOfFamily && payNum.In(listPaySplits.Select(x => x.PayNum)))
				{
				...
				if (rawPay.Rows[i]["PayPlanNum"].ToString() != "0" && !isOutOfFamily)
				{
					...
					{
						strDescript = " " + "(Attached to payment plan)"; //in v2, add the rows and show that they are attached to a payplan.
					}
				}
				...
				if (payamt != amt)
				{
					...
					{//That is not in the list of paysplits to this family.
						row["description"] += " " + "(split to another family)";
					}
				}
			...
			{
				rawClaim = dcon.ExecuteDataTable(command);
			...
			if (!isInvoice && stmt.StatementType != StmtType.LimitedStatement)
			{
				rawProcNumsClaim = dcon.ExecuteDataTable(command);
			}
			...
			{//rawClaim will have 0 rows if isInvoice or StatementType is LimitedStatement
				...
				row["description"] += "Claim" + " " + amt.ToString("c") + " "
				...
				if (claimStatus == "R")
				{
					...
					{
						row["description"] += "(no date)";//although I don't think UI allows this
					}
					else
					{
				...
				{
					row["description"] += "\r\n" + "Lab Fees" + " " + claimLabFeeTotalAmt.ToString("c");
				}
					...
					&& rawClaim.Rows[i]["ClaimType"].ToString() != "Cap")
				{
					...
					{//show additional info on resubmits
						row["description"] += "\r\n" + "Remaining Est. Payment Pending:" + " " + (insest - amtpaid).ToString("c");
					}
					else
					{
						row["description"] += "\r\n" + "Estimated Payment Pending:" + " " + insest.ToString("c");
					}
				}
					...
					&& rawClaim.Rows[i]["ClaimType"].ToString() != "Cap")
				{
					...
					row["description"] += "\r\n" + "Est. Patient Portion:" + " " + patport.ToString("c");
				}
			...
			if (!isInvoice && stmt.StatementType != StmtType.LimitedStatement)
			{
				rawState = dcon.ExecuteDataTable(command);
			}
			StatementMode _mode;
			...
			{//rawState will have 0 rows if isInvoice or StatementType is LimitedStatement
				...
				{//Must be invoice
					row["description"] += "Invoice" + " #" + rowCur["StatementNum"].ToString();
				}
				_mode = (StatementMode)PIn.Long(rowCur["Mode_"].ToString());
				row["description"] += "-" + _mode.ToString();
			...
			if (!isInvoice && stmt.StatementType != StmtType.LimitedStatement)
			{
				rawPayPlan = dcon.ExecuteDataTable(command);
			}
			if (payPlanVersionCur == PayPlanVersions.DoNotAge)
			{
				...
				for (int i = 0; i < rawPayPlan.Rows.Count; i++)
				{
					...
					{
						row["description"] = "Expected payments from "
			...
			{ //this information is only required for v2
				...
				if (stmt.StatementType != StmtType.LimitedStatement)
				{
					rawPayPlan2 = dcon.ExecuteDataTable(command);
				}
				//0 rows if isInvoice or statement type is LimitedStatement.  In spite of this, the payment plans breakdown will still show at the top of invoices.
				for (int i = 0; i < rawPayPlan2.Rows.Count; i++)
				{
					...
					row["ProcCode"] = "PayPln:" + " " + chargetype;
					row["ProcNum"] = "0";
					row["ProcNumLab"] = "";
			...
			if (payPlanVersionCur == PayPlanVersions.AgeCreditsOnly)
			{
				...
				if (!isInvoice && stmt.StatementType != StmtType.LimitedStatement)
				{
					rawPayPlan3 = dcon.ExecuteDataTable(command);
				}
				//0 rows if isInvoice or statement type is LimitedStatement.  In spite of this, the payment plans breakdown will still show at the top of invoices.
				for (int i = 0; i < rawPayPlan3.Rows.Count; i++)
				{
					...
					row["ProcCode"] = "PayPln:" + " " + PIn.Enum<PayPlanChargeType>(rawPayPlan3.Rows[i]["ChargeType"].ToString());
					row["ProcNum"] = "0";
					row["ProcNumLab"] = "";
			...
			if (payPlanVersionCur == PayPlanVersions.AgeCreditsAndDebits || payPlanVersionCur == PayPlanVersions.AgeCreditsOnly)
			{
				...
				if (!isInvoice && stmt.StatementType != StmtType.LimitedStatement)
				{
					payplanLinks = dcon.ExecuteDataTable(command);
				}
			...
			if (statementNum == 0)
			{
				...
				DataTable rawInstall = Database.ExecuteDataTable(command);
		...
		private static void SetBalForwardRow(DataRow row, decimal amt, long patNum)
		{
			...
			row["description"] = "Balance Forward";
			row["patient"] = "";
			row["PatNum"] = patNum;
			row["PayNum"] = 0;
			row["PayPlanNum"] = 0;
			...
			bool isForDynamic = false)
		{
			...
			{//loop through the payment plans (usually zero or one)
				...
				row["dateTimeSort"] = PIn.Date(rawPayPlan.Rows[i]["SecDateTEntry"].ToString());//SecDateTEntry will be used for sorting if RandomKeys is enabled
				string description = 
					descriptionPayPlanType + "\r\n" + 
					descriptionPrincipal + " " + princ.ToString("c") + "\r\n";
				if (interest != 0)
				{
					description += descriptionInterest + " " + interest.ToString("c") + "\r\n";
				}
				description += "Amount Remaining:" + " " + bal.ToString("c");
			...
			, StmtType statementType, DataSet ds)
		{
			...
			double unearnedAmt = Database.ExecuteDouble(command);
			row = table.NewRow();
			...
			List<PaySplit> listSplitsCur = null)
		{
			...
			if (calcType == CreditCalcType.IncludeAll)
			{
				...
				{
					if (creditTotal.IsLessThanOrEqualToZero())
					{
						break;
					}
					if (entryCur.AmountEnd.IsLessThanOrEqualToZero())
					{
						continue;
					}
			...
			InsSub SubCur, Relat relatOther, List<Fee> listFees = null)
		{
			...
			{
				claimError = claimError.AppendLine("Not allowed to create a claim with no procedures.");
			...
			{
				claimError = claimError.AppendLine("Claim has more than 4 unique diagnosis codes.  Create multiple claims instead.");
				return new ODTuple<bool, Claim, string>(false, new Claim(), claimError);
			}
			if (Procedures.GetUniqueDiagnosticCodes(listProcs, true).Count > 12)
			{
				claimError = claimError.AppendLine("Claim has more than 12 unique diagnosis codes.  Create multiple claims instead.");
				return new ODTuple<bool, Claim, string>(false, new Claim(), claimError);
			}
			...
			for (int i = 0; i < listProcs.Count; i++)
			{
				...
				{
					claimError = claimError.AppendLine("Not allowed to send a procedure to the same insurance company twice.");
					return new ODTuple<bool, Claim, string>(false, new Claim(), claimError);
				}
			}
			proc = listProcs[0];
