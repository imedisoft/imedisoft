...
namespace OpenDentBusiness {
	public class PayPlanEdit {
		...
		public static void RecalculateScheduleCharges(PayPlanTerms terms,PayPlanRecalculationData recalcData) {
			...
			while(recalcData.PrincipalDecrementingAmt.IsGreaterThanZero() && recalcData.ListChargesToAdd.Count()<payPlanChargesCeiling) {
		...
		private static void HandleIncreasedInterest(PayPlanRecalculationData recalcData,PayPlanTerms terms) {
			if(recalcData.OverPaidAmt.IsLessThanZero()) {
			...
			,bool isNextPeriodOnly,List<PaySplit> listPaySplits,List<PayPlanCharge> listChargesInDB,List<PayPlanCharge> listExpectedCharges)
		{
			...
			if(periodDate.Date>=terms.DateInterestStart.Date) {
				double periodRate=CalcPeriodRate(terms.APR,terms.Frequency);
				double sumChargesAll=listChargesInDB.Sum(x => x.Principal+x.Interest)+listExpectedCharges.Sum(x => x.Principal+x.Interest);
				...
				if(isNextPeriodOnly || listPaySplits.Sum(x => x.SplitAmt).IsGreaterThan(sumChargesAll)) {
					principalAmountUsedForCalculation=(decimal)CalculatePrincipalRemaining(terms.PrincipalAmount,terms.DownPayment,listPaySplits,listChargesInDB
					,listExpectedCharges);
				}
