...
namespace OpenDentBusiness.Mobile.Crud{
	internal class DocumentmCrud {
		...
		internal static Documentm SelectOne(long customerNum,long docNum){
			...
			List<Documentm> list=TableToList(Database.ExecuteDataTable(command));
			if(list.Count==0) {
				return null;
			}
			return list[0];
		...
		internal static long Insert(Documentm documentm,bool useExistingPK){
			...
			var paramRawBase64 = new MySqlParameter("paramRawBase64", documentm.RawBase64);
			Database.ExecuteNonQuery(command,paramRawBase64);//There is no autoincrement in the mobile server.
