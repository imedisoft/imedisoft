...
namespace OpenDentBusiness
{
	public class RpProdInc
	{
		...
		{
			return Definitions.GetDefsNoCache(DefinitionCategory.PaySplitUnearnedType).FindAll(x => !string.IsNullOrEmpty(x.Value))
			...
			, bool hasAllProvs, bool hasAllClinics)
		{
			...
			command = "SELECT procedurelog.PatNum, procedurelog.ProvNum, procedurelog.ClinicNum, procedurelog.DateComplete TranDate, "
				...
				+ whereProv
				+ whereClin;
			tableInsWOEst = Database.ExecuteDataTable(command);
			...
			tableAllocatedPatInc = Database.ExecuteDataTable(command);
			...
			command += "AND paysplit.DateEntry BETWEEN " + POut.Date(dateFrom) + " AND " + POut.Date(dateTo) + " ";
			tableUnallocatedPatInc = Database.ExecuteDataTable(command);
			...
			tableInsIncome = Database.ExecuteDataTable(command);
			...
			tableInsIncomeNotFinalized = Database.ExecuteDataTable(command);
		...
		public static DataTable GetNetProductionDetailDataSet(DateTime dateFrom, DateTime dateTo, List<Provider> listProvs, List<Clinic> listClinics, bool hasAllProvs, bool hasAllClinics, bool useSnapshotForToday)
		{
			...
			DataTable retVal = Database.ExecuteDataTable(command);
			return retVal;
		}
