...
namespace OpenDentBusiness
{
	...
	{
		/// <summary>
		/// If not using clinics then supply an empty list of clinicNums.
		/// listClinicNums must have at least one item if using clinics.
		/// </summary>
		public static DataTable GetInsTable(DateTime dateFrom, DateTime dateTo, List<long> listProvNums, List<long> listClinicNums, List<long> listInsuranceTypes, List<long> listClaimPayGroups, bool hasAllProvs, bool hasAllClinics, bool hasInsuranceTypes, bool isGroupedByPatient, bool hasAllClaimPayGroups, bool doShowProvSeparate)
		{
			string whereProv = "";
			if (!hasAllProvs)
			{
		...
		/// <summary>
		/// If not using clinics, or for all clinics with clinics enabled, supply an empty list of clinicNums.
		/// If the user is restricted, for all clinics supply only those clinics for which the user has permission to access, 
		/// otherwise it will be run for all clinics.
		/// </summary>
