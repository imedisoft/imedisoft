...
namespace OpenDentBusiness {
	public class RpDPPOvercharged {
		...
		public static DataTable GetDPPOvercharged(DateTime dateStart,DateTime dateEnd,List<long> listClinicNums,List<long> listProvNums,long patNum) {
			...
			DataTable result=Database.ExecuteDataTable(query);
			...
			List<DataRow> listRows=result.AsEnumerable().Where(x => x.Field<long>("PayPlanNum").In(listPayPlanNumsInTable)
				&& x.Field<double>("amtOvercharged").IsGreaterThanZero()
				&& (listClinicNums.IsNullOrEmpty() || x.Field<long>("ClinicNum").In(listClinicNums)) 
				&& (listProvNums.IsNullOrEmpty() || x.Field<long>("ProvNum").In(listProvNums)))
