    ...
    public class EhrQuarterlyKeys
	{
		...
		public static DateTime FindStartDate(List<EhrQuarterlyKey> validKeys)
		{
			...
			for (int i = validKeys.Count - 1; i > -1; i--)
			{
				...
				int expectedPrevQuarter = validKeys[i].Quarter - 1;
				if (validKeys[i].Quarter == 1)
				{
					expectedPrevQuarter = 4;
				}
				int prevQuarter = validKeys[i - 1].Quarter;
				int expectedYear = validKeys[i].Year;
				if (validKeys[i].Quarter == 1)
				{
					expectedYear = validKeys[i].Year - 1;
				}
				int prevQuarter_Year = validKeys[i - 1].Year;
		...
		{
			//No need to check RemotingRole; no call to db.
			return GetFirstDayOfQuarter(new DateTime(ehrKey.Year + 2000, ehrKey.Quarter * 3, 1));
		}
		...
		{
			//No need to check RemotingRole; no call to db.
			return GetLastDayOfQuarter(new DateTime(ehrKey.Year + 2000, ehrKey.Quarter * 3, 1));
		}
		...
		public static string ValidateDateRange(List<EhrQuarterlyKey> validKeys, DateTime startDate, DateTime endDate)
		{
			...
			for (int j = 0; j < numberOfQuarters; j++)
			{
				...
				{
					if (MonthToQuarter(startOfFirstQuarter.Month) == validKeys[i].Quarter && startOfFirstQuarter.Year - 2000 == validKeys[i].Year)
					{
						isValid = true;
						break;
					}
		...
		public static void Save(EhrQuarterlyKey ehrQuarterlyKey)
        {
			if (ehrQuarterlyKey.Id == 0)
            {
				ehrQuarterlyKey.Id = Crud.EhrQuarterlyKeyCrud.Insert(ehrQuarterlyKey);
			}
            else
            {
		...
		public static void Delete(EhrQuarterlyKey ehrQuarterlyKey)
		{
			Database.ExecuteNonQuery("DELETE FROM ehrquarterlykey WHERE EhrQuarterlyKeyNum = " + ehrQuarterlyKey.Id);
		}
	}
}