...
namespace OpenDentBusiness
{
	...
	{
		/// <summary>
		/// Normally, includeDiscontinued is false.
		/// User needs to check a box to include discontinued.
		/// </summary>
		...
		/// <summary>
		/// Gets all active medications for the patient.
		/// Exactly like Refresh() except this does not return medications when DateStop has today's date.
		/// Currently only called from FormReconcileMedication.
		/// </summary>
		public static List<MedicationPat> GetMedPatsForReconcile(long patNum)
		{
			return Crud.MedicationPatCrud.SelectMany(
				"SELECT * FROM medicationpat WHERE PatNum = " + patNum + " " +
				"AND (DateStop < " + POut.Date(new DateTime(1880, 1, 1)) + " " +
				"OR DateStop > CURDATE())");
		}

		public static MedicationPat GetOne(long medicationPatNum)
		{
			return Crud.MedicationPatCrud.SelectOne("SELECT * FROM medicationpat WHERE MedicationPatNum = " + medicationPatNum);
		}
		...
		/// <summary>
		/// For CPOE. Used for both manual rx and eRx through NewCrop. 
		/// Creates or updates a medical order using the given prescription information.
		/// Since rxCui is not part of the prescription, it must be passed in as a separate parameter.
		/// If isProvOrder is true, then the medical order provNum will be set to the prescription provNum. 
		/// If isProvOrder is false, then the medical order provNum will be set to 0.
		...
		public static long InsertOrUpdateMedOrderForRx(RxPat rxPat, long rxCui, bool isProvOrder, bool hasRx = true)
		{
			...
			if (!string.IsNullOrWhiteSpace(rxPat.ErxGuid))
			{
				...
				medOrderOld = GetMedicationOrderByErxIdAndPat(rxPat.ErxGuid, rxPat.PatNum);
			}

            MedicationPat medOrder;
            if (medOrderOld == null)
			{
			...
			if (hasRx)
			{
				...
				int numDays = PrefC.GetInt(PreferenceName.MedDefaultStopDays);
				if (numDays != 0)
				{
			...
			else
			{
                ...
                Update(medOrder, false); // Don't ErxGuid here, it was already purposefully set above. 
		...
		{
			Database.ExecuteNonQuery("DELETE from medicationpat WHERE medicationpatNum = " + Cur.MedicationPatNum);
		}
		...
		{
			return Crud.MedicationPatCrud.SelectMany("SELECT * FROM medicationpat WHERE PatNum=" + patNum + " AND MedicationNum=" + medicationNum);
		}
		...
		{
			Database.ExecuteNonQuery("UPDATE medicationpat SET DateTStamp = CURRENT_TIMESTAMP WHERE PatNum =" + patNum);
		}
		...
		{
			string command = "UPDATE medicationpat SET DateTStamp = CURRENT_TIMESTAMP WHERE PatNum = " + patNum;
			if (onlyActive)
			{
		...
		/// <summary>
		/// Used to synch medication.RxCui with medicationpat.RxCui.
		/// Updates all medicationpat.RxCui to the given value for those medication pats linked to the given medication num.
		/// </summary>
		public static void UpdateRxCuiForMedication(long medicationNum, long rxCui)
		{
			Database.ExecuteNonQuery("UPDATE medicationpat SET RxCui=" + POut.Long(rxCui) + " WHERE MedicationNum=" + medicationNum);
		}
		...
		/// <summary>
		/// Gets list of RxCui code strings for medications with RxCui in the supplied list ordered for patients in the last year.
		/// "Ordered" is based on there being a DateStart.
		/// Result list is grouped by RxCui.
		/// </summary>
		...
		public static List<MedicationPat> GetForRxCuis(List<string> listRxCuis)
		{
			...
			return Crud.MedicationPatCrud.SelectMany("SELECT * FROM medicationpat WHERE RxCui IN(" + string.Join(",", listRxCuis) + ")");
		}

		/// <summary>
		/// These can currently only exist if the MedicationPats were created when importing from NewCrop.
		/// </summary>
		public static List<MedicationPat> GetAllMissingMedications()
		{
			return Crud.MedicationPatCrud.SelectMany("SELECT * FROM medicationpat WHERE MedicationNum=0");
		}
		...
		public static void UpdateMedicationNumForMany(long medicationNum, List<long> listMedicationPatNums)
		{
			...
			string command = 
				"UPDATE medicationpat SET MedicationNum=" + medicationNum + " " +
				"WHERE MedicationPatNum IN (" + string.Join(",", listMedicationPatNums) + ")";
