		...
		public static long MoveSubscribers(long insPlanNumFrom,long insPlanNumTo) {
			...
			for(int i=0;i<listInsSubsFrom.Count;i++){
				...
				for(int j=0;j<tablePatsForInsSub.Rows.Count;j++) {
					...
					if(Preferences.Set(PreferenceName.ClaimProcsAllowedToBackdate,false)) {
					...
					if(prefChanged) {
						Preferences.Set(PreferenceName.ClaimProcsAllowedToBackdate,true);//set back to original value if changed.
					}
				}
			}
