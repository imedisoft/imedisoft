...
namespace OpenDentBusiness
{
	public class Claims
	{
		...
		/// <summary>
		/// Gets claimpaysplits attached to a claimpayment with the associated patient, insplan, and carrier.
		/// If showUnattached it also shows all claimpaysplits that have not been attached to a claimpayment. 
		/// Pass (0,true) to just get all unattached (outstanding) claimpaysplits.
		/// </summary>
		...
		/// <summary>
		/// Zeros securitylog FKey column for rows that are using the matching claimNum as FKey and are related to Claim.
		...
		/// <summary>
		/// Returns the defalt/calculated claim ID based on the ClaimIdPrefix preference.
		/// </summary>
		...
		/// Inserts and updates a new split claim. 
		/// Also updates the given claimOriginal to reflect new values.
		/// </summary>
		public static Claim InsertSplitClaim(Claim claimOriginal, List<ClaimProc> listClaimProcsToSplit, Patient pat = null)
		{
            ...
            Insert(newClaim);//We must insert here so that we have the primary key in the loop below.
		...
		/// <summary>
		/// Returns a dictionary such that they key is patnum and the value is the patients GetNameLF(...).
		/// </summary>
	...
	/// It holds data for the X12 (4010 only) generation process. 
	/// It replaces an older multi-dimensional array, so the names are funny, but helpful to prevent bugs. 
	/// Not an actual database table.
	/// </summary>
	[Serializable]
	...
	/// <summary>
	/// Holds a list of claims to show in the Claim Pay Edit window. 
	/// Not an actual database table.
	/// </summary>
	...
	/// <summary>
	/// Different types of filters for the Claims Not Sent report.
	/// </summary>
