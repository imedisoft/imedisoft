...
namespace OpenDentBusiness
{
	public class ClaimPayments
	{
		...
		{
			return Crud.ClaimPaymentCrud.SelectOne("SELECT * FROM claimpayment WHERE ClaimPaymentNum = " + claimPaymentNum);
		}
		...
		public static void Delete(ClaimPayment cp)
		{
			...
			{
				throw new ApplicationException("Not allowed to delete this payment because EOBs are attached.");
			}
		...
		/// <summary>
		/// Returns the number of payments from the passed in claimpaymentnums that are attached to a deposit other than IgnoreDepositNum.
		/// </summary>
		public static int GetCountAttachedToDeposit(List<long> listClaimPaymentNums, long ignoreDepositNum)
		{
			...
			string command = "SELECT COUNT(*) FROM claimpayment WHERE ClaimPaymentNum IN(" + string.Join(",", listClaimPaymentNums) + ") AND DepositNum!=0";
			if (ignoreDepositNum != 0)
			{
				command += " AND DepositNum!=" + POut.Long(ignoreDepositNum);
			}

			return (int)Database.ExecuteLong(command);
		}
