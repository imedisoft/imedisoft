...
namespace OpenDentBusiness
{
    ...
    public class UserWebs
	{
		...
		public static bool UpdateNewPatientPortalCredentials(Tuple<UserWeb, string, string> args)
		{
			if (args == null)
			{
				return false;
			...
			var (userWeb, passwordPlainText, passwordHash) = args;
			...
			if (userWeb != null && !string.IsNullOrEmpty(passwordPlainText))
			{
				...
				userWeb.PasswordHash = passwordHash;
		...
		///will match.</param>
		public static Tuple<UserWeb, string, string> GetNewPatientPortalCredentials(Patient pat, string passwordOverride = "")
		{
			//No need to check RemotingRole; no call to db.
			if (string.IsNullOrEmpty(Preferences.GetString(PreferenceName.PatientPortalURL)))
			{
			...
			string passwordHash = userWeb.PasswordHash;
			if (isNewPasswordRequired)
			{
				...
				passwordHash = Password.Hash(passwordPlainText);
			}
			return new Tuple<UserWeb, string, string>(userWeb, passwordPlainText, passwordHash);
		}
