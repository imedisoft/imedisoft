...
namespace OpenDentBusiness
{
    public static class ImageHelper
	{
		...
		///<summary>Returns a bitmap based on bitmapReference, with typically just windowing (brightness/contrast) applied.  Happens within a different thread because it's not blazing fast.  It can also do the crop, rotate, and flip, but that's only when it's not live in the Images module.</summary>
		public static Bitmap ApplyDocumentSettingsToImage2(Document document, Bitmap bitmapReference, ImageSettingFlags imageSettingFlags)
		{
			...
			if (document == null)
			{//Implies that the image should be returned "unaltered".
			...
			//todo: treat 16bit grayscale separately.  
			//Format16bppGrayScale is not supported by C# 
			//Format48bppRgb will be used, and the image passed in might actually already be 48bpp just so that this step will work
