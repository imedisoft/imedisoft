...
namespace OpenDentBusiness
{
	...
	public static class Podium
	{
		...
		private static List<Appointment> GetAppointmentsToSendReview(ReviewInvitationTrigger trigger, long programNum, bool isNewPatient)
		{
			...
			if (trigger == ReviewInvitationTrigger.AppointmentCompleted)
			{
				...
				AND EXISTS (
					...
					AND histappointment.HistDateTStamp<={DbHelper.Now()} - INTERVAL {minsWaitComplete} MINUTE
				)";
			}
			else
				...
				AND (
					(
						...
						AND EXISTS (
							...
							WHERE histappointment.AptNum=appointment.AptNum
							AND histappointment.AptStatus!={POut.Int((int)ApptStatus.Complete)}
							AND histappointment.HistDateTStamp<={DbHelper.Now()} - INTERVAL {(isArriveTrigger ? minsWaitComplete : "90")} MINUTE
						)
					)
					OR appointment.{(isArriveTrigger ? "DateTimeArrived" : "DateTimeDismissed")} BETWEEN {DbHelper.Curdate()}
						AND {DbHelper.Now()} - INTERVAL {minsWaitArriveOrDismiss} MINUTE
				)";
			}
		...
		public static bool SendData(Patient pat, long clinicNum)
		{
			...
			if (pat.TxtMsgOk == YN.No || (pat.TxtMsgOk == YN.Unknown && Preferences.GetBool(PreferenceName.TextMsgOkStatusTreatAsNo)))
			{//Don't text
			 //Try to use email
