...
namespace OpenDentBusiness
{
	...
	public class Db
	{
		...
		{
			var table = GetTable("SHOW VARIABLES LIKE 'auto_increment_increment'");
		...
		internal static List<long> GetListLong(string command, bool hasExceptions = true)
		{
			...
			var Table = GetTableWithRemotingRoleCheck(command);
			for (int i = 0; i < Table.Rows.Count; i++)
			{
				result.Add(PIn.Long(Table.Rows[i][0].ToString(), hasExceptions));
			}
		...
		internal static List<string> GetListString(string command)
		{
			...
			for (int i = 0; i < Table.Rows.Count; i++)
			{
				result.Add(PIn.String(Table.Rows[i][0].ToString()));
			}
		...
		internal static long NonQ(string command, bool getInsertID, params OdSqlParameter[] parameters)
			=> NonQ(command, getInsertID, "", "", parameters);
		...
		internal static long NonQ(string command, params OdSqlParameter[] parameters) 
			=> NonQ(command, false, parameters);

		/// <summary>
		/// We need to get away from this due to poor support from databases.
		/// For now, each command will be sent entirely separately.
		/// This never returns number of rows affected.
		/// </summary>
		...
		/// <summary>
		/// This is used for historical commands in ConvertDatabase.
		/// Seems to also be used in DBmaint when counting rows affected.
		/// </summary>
		internal static int NonQ32(string command) 
			=> NonQ32(command, false);

		/// <summary>
		/// We use this for queries that return a single value that is an int.
		/// If there are no results, it will return 0.
		/// </summary>
		internal static int GetInt(string command) 
			=> SIn.Int((GetLong(command).ToString()), false);

		/// <summary>
		/// We use this for queries that return a single value that is a long.
		/// If there are no results, it will return 0.
		/// </summary>
