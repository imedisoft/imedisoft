...
namespace OpenDentBusiness{
	public class SheetUtil {
		...
		public static void SetControlSizeAndAnchors(SheetFieldDef sheetFieldDef,Control controlTarget,Control controlParent,int fillHeightAdj=0,bool doAnchorToBottom=true) {
			...
			switch(sheetFieldDef.GrowthBehavior) {
				case GrowthBehaviorEnum.FillRight:
					controlTarget.Width=(controlParent.Width-sheetFieldDef.XPos)-1;
					controlTarget.Height=sheetFieldDef.Height;
					controlTarget.Anchor=(AnchorStyles.Top|AnchorStyles.Left|AnchorStyles.Right);
					break;
				case GrowthBehaviorEnum.FillDown:
					controlTarget.Width=sheetFieldDef.Width;
					controlTarget.Height=Math.Max(1,(controlParent.Height-sheetFieldDef.YPos-fillHeightAdj)-1);
					if(doAnchorToBottom) {
						controlTarget.Anchor=(AnchorStyles.Top|AnchorStyles.Left|AnchorStyles.Bottom);
					}
					else {
						controlTarget.Anchor=(AnchorStyles.Top|AnchorStyles.Left);
					}
					break;
				case GrowthBehaviorEnum.FillRightDown:
					controlTarget.Width=(controlParent.Width-sheetFieldDef.XPos)-1;
					controlTarget.Height=Math.Max(1,(controlParent.Height-sheetFieldDef.YPos-fillHeightAdj)-1);
					controlTarget.Anchor=(AnchorStyles.Top|AnchorStyles.Left|AnchorStyles.Right|AnchorStyles.Bottom);
					break;
				case GrowthBehaviorEnum.FillDownFitColumns://Only grids have access to this option.
					int width=0;
					if(controlTarget is ODGrid) {
						width=(controlTarget as ODGrid).GetIdealWidth();
					}
					else {//Must contain a ODGrid directly then
						width=(controlTarget.Controls.OfType<Control>().First(x => x is ODGrid) as ODGrid).GetIdealWidth();
					}
					controlTarget.Width=Math.Max(width,524);
					controlTarget.Height=Math.Max(1,(controlParent.Height-sheetFieldDef.YPos-fillHeightAdj)-1);
					controlTarget.Anchor=(AnchorStyles.Top|AnchorStyles.Left|AnchorStyles.Bottom);
					break;
				default:
			...
			if(sheetFieldDef.FieldName!="ChartModuleTabs") {//ChartModuleTabs can be collapsed in the Chart Module, do not limit size.
				controlTarget.MinimumSize=new Size(sheetFieldDef.Width,sheetFieldDef.Height);
			}
		}
