...
namespace OpenDentBusiness
{
	...
	public class Patient : TableBase, IComparable
	{
		...
		/// Age is always calculated as needed from birthdate.
		/// </summary>
		public DateTime Birthdate;

		/// <summary>
		/// In the US, this is 9 digits, no dashes. 
		/// For all other countries, any punctuation or format is allowed.
		/// </summary>
		public string SSN;
		...
		/// <summary>
		/// 2 Char in USA. 
		/// Used to store province for Canadian users.
		/// </summary>
		public string State;

		/// <summary>
		/// Postal code. 
		/// For Canadian claims, it must be ANANAN. 
		/// No validation gets done except there.
		/// </summary>
		public string Zip;
		...
		/// <summary>
		/// Single char. 
		/// Shows at upper right corner of appointments. 
		/// Suggested use is A,B,or C to designate creditworthiness, but it can actually be used for any purpose.
		/// </summary>
		...
		/// Fee schedule for this patient. 
		/// Usually not used. 
		/// If missing, the practice default fee schedule is used. I
		/// f patient has insurance, then the fee schedule for the insplan is used.
		/// </summary>
		...
		/// <summary>
		/// Max 15 char. 
		/// Used for reference to previous programs.
		/// </summary>
		...
		/// <summary>
		/// The date that the patient first visited the office. 
		/// Automated.
		/// </summary>
		...
		/// This is only used when displaying appointments.  
		/// It will later be expanded. 
		/// User can't edit.
		/// </summary>
		...
		/// Typically eng (English), fra (French), spa (Spanish), or similar.  
		...
		/// <summary>
		/// Used in hospitals. 
		/// It can be before the first visit date. 
		/// It typically gets set automatically by the hospital system.
		/// </summary>
		...
		/// FK to site.SiteNum. 
		/// Can be zero. 
		/// Replaces the old GradeSchool field with a proper foreign key.
		/// </summary>
		[ForeignKey(typeof(Site), nameof(Site.SiteNum))]
		public long SiteNum;

		/// <summary>
		/// Automatically updated by MySQL every time a row is added or changed.
		/// Could be changed due to user editing, custom queries or program updates. 
		/// Not user editable.
		/// </summary>
		...
		/// 1=FT student, 2=disabled, 3=disabled student, 4=code not applicable. 
		/// Warning. 4 is a 0 if using CDAnet version 02. 
		...
		/// <summary>
		/// FK to patient.PatNum. 
		/// If this is the same as PatNum, then this is a SuperHead. 
		/// If zero, then not part of a superfamily.  
		/// Synched for entire family. 
		/// If family is part of a superfamily, then the guarantor for this family will show in the superfamily list in the Family module for anyone else who is in the superfamily. 
		...
		/// <summary>
		/// EHR smoking status as a SNOMED code. 
		/// Will always be the most recent smoking status for the patient.
		/// </summary>
		public string SmokingSnoMed;

		/// <summary>
		/// Country name. 
		/// Only used by HQ to add country names to statements.
		/// </summary>
		public string Country;

		/// <summary>
		/// Needed for EHR syndromic surveillance messaging. 
		/// Used in HL7 PID-29. 
		/// Also for feature request #3040.
		/// Date and time because we need precision to the minute in syndromic surveillence messging.
		/// </summary>
		...
		/// <summary>
		/// Timestamp automatically generated and user not allowed to change. 
		...
		/// </summary>
		[ForeignKey(typeof(DiscountPlan), nameof(DiscountPlan.DiscountPlanNum))]
		public long DiscountPlanNum;

		/// <summary>
		/// Signed Truth in Lending, relates to client permission to be charged interest on a payment plan.
		/// </summary>
		public bool HasSignedTil;

		/// <summary>
		/// Indicates if the patient has opted in, out, or not yet to using Short Codes for Appointment Texts.
		/// </summary>
		public YN ShortCodeOptIn;

		/// <summary>
		/// Not in the database table. 
		/// The specialty is stored in the deflink table which is not cached.
		/// This field will be lazy loaded (specialty feature might not be enabled) when the Specialty property is used.
		/// Having this field around will save the main window title from running a query every second to get the patient specialty.
		/// </summary>
		[Ignore]
		...
		/// <summary>
		/// Gets the age of the patient in years.
		/// </summary>
		public int Age => Patients.DateToAge(Birthdate);

		/// <summary>
		/// The specialty is stored in the deflink table which is not cached. This will lazy load the patient specialty.
		/// Having this property around will save the main window title from running a query every second to get the patient specialty.
		/// </summary>
		...
		/// Be cautious. In the end, this function will probably not need to change anyway, since it will only be used for comparing patients when the PatNums are not known.
		/// </summary>
