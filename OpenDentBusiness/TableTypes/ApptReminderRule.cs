...
namespace OpenDentBusiness {
	...
	public class ApptReminderRule : TableBase {
		...
		[XmlIgnore]
		public bool AreArrivalResponsesSetup => TypeCur==ApptReminderType.Arrival && IsAutoReplyEnabled
			&& (!string.IsNullOrWhiteSpace(TemplateAutoReply) || !string.IsNullOrWhiteSpace(TemplateComeInMessage));
	...
	public class ShortCodeAttribute:Attribute {
		...
		private static bool IsPrefEnabled(PrefName prefName,long clinicNum,int[] enabledValues,Type valueType) {
			...
			for(int i = 0;i<enumValues.Length;i++) {
				...
				if(prefValue==((int)enumValue).ToString() || prefValue==enumValue.ToString()) {
					prefAsInt=(int)enumValue;
					break;
				}
			}
			if(prefAsInt is null) {
				return false;
			}
			return prefAsInt?.In(enabledValues)??false;
		}
