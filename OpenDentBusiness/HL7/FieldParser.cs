	...
	public class FieldParser
	{
		...
		public static long ProvProcessEcw(FieldHL7 field)
		{
			...
			if (prov == null)
			{
                ...
                prov = new Provider
                {
                    Abbr = eID,//They can manually change this later.
                    EcwID = eID,
                    FeeScheduleId = FeeScheds.GetFirst(true).Id
                };
            }
		...
		public static long ProvParse(FieldHL7 field, SegmentNameHL7 segName, bool isVerbose)
		{
			...
			if (strProvId != "" && strProvIdRoot != "")
			{
				...
				{//there was a ProvID and a ProvID root, but the root is not the office's root OID for a provider object, check the oidexternals table
					...
					{//add to the list of oid's to add to the oidexternal table if we find a provider
                        ...
                        {
                            IDType = IdentifierType.Provider,
                            rootExternal = strProvIdRoot,
                            IDExternal = strProvId
                        };
			...
			{//AIG is the data type CWE with format ProvNum^LName, FName^^Abbr
		...
		public static PatientRaceOld RaceParseOld(string str)
		{
            return str switch
            {
                "American Indian Or Alaska Native" => PatientRaceOld.AmericanIndian,
                "Asian" => PatientRaceOld.Asian,
                "Native Hawaiian or Other Pacific" => PatientRaceOld.HawaiiOrPacIsland,
                "Black or African American" => PatientRaceOld.AfricanAmerican,
                "White" => PatientRaceOld.White,
                "Hispanic" => PatientRaceOld.HispanicLatino,
                "Other Race" => PatientRaceOld.Other,
                _ => PatientRaceOld.Other,
            };
        }
