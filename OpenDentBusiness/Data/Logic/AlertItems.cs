...
namespace Imedisoft.Data
{
    public partial class AlertItems
	{
		...
		public static void CheckODServiceHeartbeat()
		{
			if (!IsODServiceRunning())
			{
				...
				if (!alerts.Any())
				{
					ExecuteInsert(new AlertItem
					{
						Actions = AlertAction.MarkAsRead,
						ClinicId = null,
		...
		public static void Insert(AlertItem alertItem) => ExecuteInsert(alertItem);
		...
		public static List<List<AlertItem>> GetUniqueAlerts(long userNumCur, long clinicNumCur)
		{
			...
			List<string> listUserAlertLinks = AlertCategoryLinks.Find(x => listAlertCatNums.Contains(x.AlertCategoryId))
			...
			List<string> listAllUserAlertLinks = AlertCategoryLinks.Find(x => listAllAlertCatNums.Contains(x.AlertCategoryId))
				.Select(x => x.Type).ToList();
			//Each inner list is a group of alerts that are duplicates of each other.
		...
		/// </summary>
		public static IEnumerable<AlertItem> RefreshForClinicAndTypes(long clinicId, List<string> alertTypes = null)
		{
			...
			return SelectMany(
				...
				"WHERE `type` IN (" + string.Join(", ", alertTypes.Select(x => '\'' + x + '\'')) + ") " +
				"AND (`user_id` IS NULL OR `user_id` = " + userId + ") " +
				"AND (CASE `type` WHEN '" + AlertType.RadiologyProcedures + "' THEN `object_id` = " + providerId + " " +
		...
		/// </summary>
		public static IEnumerable<AlertItem> RefreshForType(string alertType)
		{
			return SelectMany("SELECT * FROM `alert_items` WHERE `type` = @type",
				new MySqlParameter("type", alertType ?? ""));
		}
		...
		/// </summary>
		public static void DeleteFor(string alertType, List<long> objectIds = null)
		{
