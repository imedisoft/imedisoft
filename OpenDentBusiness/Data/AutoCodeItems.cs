    ...
    public partial class AutoCodeItems
	{
		...
		private class AutoCodeItemCache : DictionaryCache<long, AutoCodeItem>
        {
            protected override IEnumerable<AutoCodeItem> Load() 
				=> SelectMany("SELECT * FROM autocodeitem");
		...
		public static List<AutoCodeItem> GetListForCode(long autoCodeNum) 
			=> cache.Find(x => x.AutoCodeId == autoCodeNum).ToList();
		
		/// <summary>
		/// Only called from ContrChart.listProcButtons_Click.
		/// Called once for each tooth selected and for each autocode item attached to the button.
		/// </summary>
		public static long GetCodeNum(long autoCodeNum, string toothNum, string surf, bool isAdditional, long patNum, int age, bool willBeMissing)
		{
			...
			{
				condList = AutoCodeConds.GetByAutoCodeItem(listForCode[i].Id);
				allCondsMet = true;
