	...
	public class ItransNCpl
	{
		...
		public static string TryCarrierUpdate(bool isAutomatic = true, ItransImportFields fieldsToImport = ItransImportFields.None)
		{
			...
			if (clearinghouse.CommBridge == EclaimsCommBridge.ITRANS && !string.IsNullOrEmpty(clearinghouse.ResponsePath))
			{
				...
				if (isAutomatic && Preferences.GetString(PreferenceName.WebServiceServerName).ToLower() != Dns.GetHostName().ToLower())
				{//Only server can run when isOnlyServer is true.
					return "Update can only run on the web service server " + Preferences.GetString(PreferenceName.WebServiceServerName) +
