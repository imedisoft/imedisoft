		...
		public static DataTable GetLetterMergeInfo(Patient PatCur, LetterMerge letter)
		{
			...
			command += " FROM patient "
				...
				+ "LEFT JOIN recall ON recall.PatNum=patient.PatNum "
					+ "AND (recall.RecallTypeNum=" + POut.Long(Preferences.GetLong(PreferenceName.RecallTypeSpecialProphy))
					+ " OR recall.RecallTypeNum=" + POut.Long(Preferences.GetLong(PreferenceName.RecallTypeSpecialPerio)) + ") "
