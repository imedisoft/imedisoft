...
namespace Imedisoft.CEMT.Forms
{
	...
	{
		/// <summary>
		/// List of connections that are already attached to this group and showing in the grid.
		/// </summary>
		private List<CentralConnection> _listConnections;

		/// <summary>
		/// List of connections in the right grid that are available to attach.
		/// </summary>
		...
		private void FillGrid()
		{
			...
			gridMain.ListGridColumns.Add(new GridColumn("Database", 320));
			gridMain.ListGridColumns.Add(new GridColumn("Note", 300));
		...
		private void FillGridAvail()
		{
			...
			gridAvail.ListGridColumns.Add(new GridColumn("Database", 320));
			gridAvail.ListGridColumns.Add(new GridColumn("Note", 300));
		...
		private void AddButton_Click(object sender, EventArgs e)
		{
			...
			{
				MessageBox.Show("Please select connections first.");
				return;
			}
			...
			for (int i = 0; i < gridAvail.SelectedIndices.Length; i++)
			{
				ConnGroupAttaches.Attach(
					_listConnectionsAvail[gridAvail.SelectedIndices[i]].CentralConnectionNum, ConnectionGroupCur.ConnectionGroupNum);
			}
		...
		private void RemoveButton_Click(object sender, EventArgs e)
		{
			...
			for (int i = 0; i < gridMain.SelectedIndices.Length; i++)
			{
				ConnGroupAttaches.Detach(
					_listConnections[gridMain.SelectedIndices[i]].CentralConnectionNum, ConnectionGroupCur.ConnectionGroupNum);
			}
